const SAVE_KEY="ageOfOlympusSaveV170";
...
function v150AutomaticMarriageTurn(){
 ...
 if(free.length<2||Math.random()>.28)return;
 const a=randomItem(free), choices=free.filter(x=>x.name!==a.name);
 if(!choices.length)return;
 const b=choices.sort((x,y)=>v150RelationshipScore(a,y)-v150RelationshipScore(a,x))[0];
 a.affection=clamp((a.affection||0)+rand(2,7));b.affection=clamp((b.affection||0)+rand(2,7));
 a.trust=clamp((a.trust||45)+rand(1,5));b.trust=clamp((b.trust||45)+rand(1,5));
 if(v150RelationshipScore(a,b)<82||Math.random()>.42)return;
 ...
 addActivity("💍",`Divine Marriage: ${a.name} & ${b.name}`,"The immortals chose one another without player matchmaking. Their Divine House was founded automatically.","success");
