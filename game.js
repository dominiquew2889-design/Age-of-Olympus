...
function stage11CompleteMarriage(a,b,meta={}){
 const u=stage11CreateOrGetUnion(a,b,{status:"Married",year:meta.year??state.year,type:meta.type||"Divine Union",source:meta.source||"Matchmaking"});
 u.status="Married";
 u.history.unshift({year:state.year,text:`Marriage completed.`});

 // Resolve any associated betrothal/match.
 (state.divineMatchmaking?.arrangedMatches||[]).forEach(m=>{
  if((m.a===a&&m.b===b)||(m.a===b&&m.b===a)){m.status="Married";m.accepted=true}
 });

 // Resolve stale matchmaking alerts automatically.
 (state.alerts||[]).forEach(alert=>{
  if(alert.title?.includes("Match Accepted")&&alert.title.includes(a)&&alert.title.includes(b) && !alert.resolved){
   alert.resolved=true;alert.resolution=`Wedding completed: ${a} & ${b}.`;alert.status="Resolved";
  }
 });
...
function attemptStage10Match(aName,bName){
 ...
 if(accepted){
  ...
  createAlert({priority:"urgent",category:"family",icon:"💞",title:`Match Accepted: ${a.name} & ${b.name}`,description:`Both immortals accepted your proposed union. You may now hold a formal divine wedding.`,location:"Mount Olympus",targetId:match.id,years:3});
...
function holdStage10Wedding(matchId){
 ...
 createAlert({priority:"important",category:"family",icon:"💍",title:`Wedding Complete: ${a.name} & ${b.name}`,description:`Their marriage, house, family tree, relationships, and future children are now connected through Olympus Core.`,location:"Mount Olympus",targetId:m.id,years:2});
 runOlympusIntegrityRepair(true);
 renderAll();saveGame(false);
}
...
function syncStage10Dynasties(){
 const dm=state.divineMatchmaking;
 dm.dynasties=[];
 (state.divineRelationships?.houses||[]).forEach(h=>{
  const wedding=dm.weddings.find(w=>h.founders?.includes(w.a)&&h.founders?.includes(w.b));
  if(!wedding)return;
  const children=(state.divineRelationships?.npcImmortalChildren||[]).filter(c=>h.founders.includes(c.parentA)&&h.founders.includes(c.parentB));
  dm.dynasties.push({id:h.id,house:h.name,founders:[...h.founders],type:h.type,children:children.map(c=>c.name),influence:h.influence||50,unity:h.unity||70});
 });
}
...
function renderStage10Matchmaking(){runOlympusIntegrityRepair(true);
 if(!document.getElementById("matchmaking-view")||!state.divineMatchmaking)return;
 synchronizeUnifiedOlympus(true);stage10RepairAllMarriages();syncStage10Dynasties();
 ...
 $("#matchmaking-list").innerHTML=elig.length?elig.map(a=>{
   const canon=state.unifiedOlympus.masterPeople[a.name];
...
function renderDivineHouses(){runOlympusIntegrityRepair(true);synchronizeUnifiedOlympus(true);stage10RepairAllMarriages();
