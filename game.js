const SAVE_KEY="ageOfOlympusSaveV140S10";
...
function renderGodSelection(){
 ...
 if(localStorage.getItem(SAVE_KEY)||localStorage.getItem(PREVIOUS_SAVE_KEY_STAGE10)||localStorage.getItem(PREVIOUS_SAVE_KEY_STAGE9)||localStorage.getItem(PREVIOUS_SAVE_KEY_STAGE8)||localStorage.getItem(PREVIOUS_SAVE_KEY_STAGE7)||localStorage.getItem(PREVIOUS_SAVE_KEY_STAGE6))$("#continue-button").classList.remove("hidden");
}
...
function stage10EligibleImmortals(){
 stage10RepairAllMarriages();
 return (state.divineRelationships?.roster||[]).filter(r=>{
   return r.name!==getGod().name && !stage10SpouseOf(r.name);
 });
}
...
function holdStage10Wedding(matchId){
 ...
 stage10RepairAllMarriages();
 if(stage10SpouseOf(m.a)||stage10SpouseOf(m.b)){showToast("One of the immortals is already married.");return}
 ...
 m.status="Married";
 if(!state.divineMatchmaking.weddings.some(w=>(w.a===m.a&&w.b===m.b)||(w.a===m.b&&w.b===m.a))){
  state.divineMatchmaking.weddings.unshift({id:uid("wedding"),a:m.a,b:m.b,year:state.year,type});
 }

 // This is now the permanent marriage source of truth.
 stage10CanonicalMarriage(m.a,m.b,{year:state.year,type,source:"Player Matchmaking"});

 // Also update legacy relationship marriage storage immediately.
 let legacy=state.divineRelationships.npcMarriages.find(x=>(x.a===m.a&&x.b===m.b)||(x.a===m.b&&x.b===m.a));
 if(!legacy){
  legacy={id:uid("npcMarriage"),a:m.a,b:m.b,year:state.year,type,status:"Married"};
  state.divineRelationships.npcMarriages.unshift(legacy);
 }else legacy.status="Married";

 a.status="Married";b.status="Married";a.autonomousPartner=b.name;b.autonomousPartner=a.name;

 // Repair every connected system before any render occurs.
 stage10RepairAllMarriages();
 synchronizeUnifiedOlympus(true);
 stage10RepairAllMarriages();
 ...
 state.divineMatchmaking.history.unshift({year:state.year,text:`${a.name} married ${b.name} after player matchmaking.`});

 recordChronicleEvent({icon:"💍",title:`Divine Wedding: ${a.name} & ${b.name}`,text:`The marriage is permanently recorded across Unified Olympus, the Family Tree, Divine Houses, Living Souls, and dynasty systems.`,category:"marriages",severity:"mythic",major:true,actor:getGod().name,target:`${a.name} & ${b.name}`,location:"Mount Olympus",tags:["divine-family","gods-titans","marriages"]});
 createAlert({priority:"urgent",category:"family",icon:"💍",title:`Divine Wedding: ${a.name} & ${b.name}`,description:`${a.name} and ${b.name} are now permanently married. Their Divine House has been founded and future immortal children will inherit both bloodlines.`,location:"Mount Olympus",targetId:m.id,years:3});
...
function renderStage10Matchmaking(){
 ...
 $("#immortal-dynasty-list").innerHTML=
   pending.map(m=>`<article class="card wedding-card"><div class="card-row"><div><h3>💍 ${m.a} & ${m.b}</h3><p>Betrothed • Compatibility ${m.compatibility}</p></div><span class="tag gold">Awaiting Wedding</span></div><button class="match-action secondary" data-hold-wedding="${m.id}">Hold Divine Wedding</button></article>`).join("")+
...
function renderDivineHouses(){synchronizeUnifiedOlympus(true);stage10RepairAllMarriages();
...
function renderDivineFamily(){synchronizeUnifiedOlympus(true);const df=state.divineFamily,s=df.spouse;$("#divine-family-status").textContent=s?`Married to ${s.name}`:"Unwed";$("#divine-family-summary").innerHTML=`<p class="eyebrow">LIVING PANTHEON</p><h3>Marriage creates immortal children, jealousy, sibling bonds, inheritance and political consequences</h3><div class="stats"><div class="stat"><strong>${s?1:0}</strong><small>Spouse</small></div><div class="stat"><strong>${df.immortalChildren.length}</strong><small>Immortal Children</small></div><div class="stat"><strong>${state.demigods.length}</strong><small>Demigods</small></div></div>`;
