...
function ensureV140Stage10Data(){
 ...
 if(!dm.lastChildYears)dm.lastChildYears={};
...
function stage10EligibleImmortals(){
 return (state.divineRelationships?.roster||[]).filter(r=>{
   const canon=state.unifiedOlympus?.masterPeople?.[r.name];
   return r.name!==getGod().name && !canon?.spouse;
 });
}
...
function holdStage10Wedding(matchId){
 ...
 synchronizeUnifiedOlympus(true);
 if(state.unifiedOlympus.masterPeople[m.a]?.spouse||state.unifiedOlympus.masterPeople[m.b]?.spouse){showToast("One of the immortals married someone else.");return}
 const roster=state.divineRelationships?.roster||[],a=roster.find(r=>r.name===m.a),b=roster.find(r=>r.name===m.b);if(!a||!b)return;

 m.status="Married";
 state.divineMatchmaking.weddings.unshift({id:uid("wedding"),a:m.a,b:m.b,year:state.year,type:(a.type==="Titan"||b.type==="Titan")?"Olympian-Titan Union":"Divine Union"});
 stage9LinkSpouses(m.a,m.b);

 // Keep old autonomous marriage systems aware too.
 if(!state.divineRelationships.npcMarriages.some(x=>(x.a===m.a&&x.b===m.b)||(x.a===m.b&&x.b===m.a))){
  state.divineRelationships.npcMarriages.unshift({id:uid("npcMarriage"),a:m.a,b:m.b,year:state.year,type:(a.type==="Titan"||b.type==="Titan")?"Mixed/Titan Union":"Olympian Union",status:"Married"});
 }
 a.status="Committed";b.status="Committed";a.autonomousPartner=b.name;b.autonomousPartner=a.name;

 // Create/update family tree nodes and spouse links.
 [a,b].forEach(r=>{
  let node=state.divineFamily.familyTree.find(n=>n.name===r.name);
  if(!node){node={id:uid("family"),name:r.name,icon:r.icon,type:r.type,relation:"Divine Kin",parents:[],spouse:null,children:[]};state.divineFamily.familyTree.push(node)}
 });
 state.divineFamily.familyTree.find(n=>n.name===a.name).spouse=b.name;
 state.divineFamily.familyTree.find(n=>n.name===b.name).spouse=a.name;

 // Create their Divine House immediately.
 const houseName=`House of ${a.name} & ${b.name}`;
 if(!state.divineRelationships.houses.some(h=>h.name===houseName)){
  state.divineRelationships.houses.push({id:uid("divineHouse"),name:houseName,icon:"🏛️",founders:[a.name,b.name],type:(a.type==="Titan"||b.type==="Titan")?"Olympian-Titan House":"Divine House",children:[],demigods:[],influence:50,unity:70,history:[`Founded by arranged marriage in Year ${state.year}.`]});
 }
 ...
 recordChronicleEvent({icon:"💍",title:`Divine Wedding: ${a.name} & ${b.name}`,text:`A formal immortal marriage founded ${houseName}.`,category:"marriages",severity:"mythic",major:true,actor:getGod().name,target:`${a.name} & ${b.name}`,location:"Mount Olympus",tags:["divine-family","gods-titans","marriages"]});
 createAlert({priority:"urgent",category:"family",icon:"💍",title:`Divine Wedding: ${a.name} & ${b.name}`,description:`The marriage is now part of the Unified Family Tree and may produce immortal children over the coming years.`,location:"Mount Olympus",targetId:m.id,years:3});
...
function stage10YearTurn(){
 stage10ChildbirthTurn();stage10MarriagePoliticsTurn();syncStage10Dynasties();synchronizeUnifiedOlympus(false);
}

function renderStage10Matchmaking(){
 if(!document.getElementById("matchmaking-view")||!state.divineMatchmaking)return;
 synchronizeUnifiedOlympus(true);syncStage10Dynasties();
...
function renderDivineHouses(){synchronizeUnifiedOlympus(true);
