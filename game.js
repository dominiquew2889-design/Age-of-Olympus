const SAVE_KEY="ageOfOlympusSaveV210";
const PREVIOUS_SAVE_KEY_V210="ageOfOlympusSaveV200";
const PREVIOUS_SAVE_KEY_V200="ageOfOlympusSaveV190";
const PREVIOUS_SAVE_KEY_V190="ageOfOlympusSaveV180";
const PREVIOUS_SAVE_KEY_V180="ageOfOlympusSaveV170";
const PREVIOUS_SAVE_KEY_V170="ageOfOlympusSaveV160";
const PREVIOUS_SAVE_KEY_V160="ageOfOlympusSaveV150";
const PREVIOUS_SAVE_KEY_V150="ageOfOlympusSaveV140S11";
const PREVIOUS_SAVE_KEY_STAGE11="ageOfOlympusSaveV140S10";
const PREVIOUS_SAVE_KEY_STAGE10="ageOfOlympusSaveV140S9";
const PREVIOUS_SAVE_KEY_STAGE9="ageOfOlympusSaveV140S8";
const PREVIOUS_SAVE_KEY_STAGE8="ageOfOlympusSaveV140S7";
...
function renderGodSelection(){
 ...
 if(localStorage.getItem(SAVE_KEY)||localStorage.getItem(PREVIOUS_SAVE_KEY_V210)||localStorage.getItem(PREVIOUS_SAVE_KEY_V200)||localStorage.getItem(PREVIOUS_SAVE_KEY_V190)||localStorage.getItem(PREVIOUS_SAVE_KEY_V180)||localStorage.getItem(PREVIOUS_SAVE_KEY_V170)||localStorage.getItem(PREVIOUS_SAVE_KEY_V160)||localStorage.getItem(PREVIOUS_SAVE_KEY_V150)||localStorage.getItem(PREVIOUS_SAVE_KEY_STAGE11)||localStorage.getItem(PREVIOUS_SAVE_KEY_STAGE10)||localStorage.getItem(PREVIOUS_SAVE_KEY_STAGE9)||localStorage.getItem(PREVIOUS_SAVE_KEY_STAGE8)||localStorage.getItem(PREVIOUS_SAVE_KEY_STAGE7)||localStorage.getItem(PREVIOUS_SAVE_KEY_STAGE6))$("#continue-button").classList.remove("hidden");
}
function startGame(load=false){if(load){try{const raw=localStorage.getItem(SAVE_KEY)||localStorage.getItem(PREVIOUS_SAVE_KEY_V210)||localStorage.getItem(PREVIOUS_SAVE_KEY_V200)||localStorage.getItem(PREVIOUS_SAVE_KEY_V190)||localStorage.getItem(PREVIOUS_SAVE_KEY_V180)||localStorage.getItem(PREVIOUS_SAVE_KEY_V170)||localStorage.getItem(PREVIOUS_SAVE_KEY_V160)||localStorage.getItem(PREVIOUS_SAVE_KEY_V150)||localStorage.getItem(PREVIOUS_SAVE_KEY_STAGE11)||localStorage.getItem(PREVIOUS_SAVE_KEY_STAGE10)||localStorage.getItem(PREVIOUS_SAVE_KEY_STAGE9)||localStorage.getItem(PREVIOUS_SAVE_KEY_STAGE8)||localStorage.getItem(PREVIOUS_SAVE_KEY_STAGE7)||localStorage.getItem(PREVIOUS_SAVE_KEY_STAGE6);state=JSON.parse(raw);if(!state)throw new Error("No save")}catch(e){localStorage.removeItem(SAVE_KEY);showToast("Save could not be loaded.");return}}else state=newGame(selectedGodId);$("#start-screen").classList.remove("active");$("#game-screen").classList.add("active");renderAll();saveGame(false)}
...
function ensureV210Data(){
 if(!state.v210)state.v210={version:"2.1.0",lastRepairYear:0};
 // Upgrade old queued NPC births without blocking the player.
 [...(state.v170?.pendingBirths||[])].forEach(b=>{if(b.parentA!==getGod().name&&b.parentB!==getGod().name)finalizeV170Birth(b.id,v170SuggestedNames(1)[0],true)});
 if(state.olympusCore){runOlympusIntegrityRepair(true);syncAutonomousDivineFamilies()}
 state.v210.lastRepairYear=state.year;
}
function renderAll(){
 ...
 ensureLivingCityData();ensureV100Data();ensureV110Data();ensureV120Data();ensureV130Data();ensureV130Stage2Data();ensureV130Stage3Data();ensureV130Stage4Data();ensureV130Stage5Data();ensureV140Stage1Data();ensureV140Stage2Data();ensureV140Stage3Data();ensureV140Stage4Data();ensureV140Stage5Data();ensureV140Stage6Data();ensureV140Stage7Data();ensureV140Stage8Data();ensureV140Stage9Data();ensureV140Stage10Data();ensureV140Stage11Data();ensureV150Data();ensureV160Data();ensureV170Data();ensureV180Data();ensureV190Data();ensureV200Data();ensureV210Data();renderWorld();renderLivingChronicle();renderStage6Sagas();renderStage7Extras();renderStage8Extras();renderStage9Extras();renderStage10Extras();renderStage11Extras();renderDivineRelationships();renderDivineHouses();renderDivineFamily();renderFamilyTree();renderAscensionCouncil();renderLivingCitizens();renderPrayers();renderInteractiveTemples();renderLivingWars();renderCreatureEcosystem();renderLegendaryAdventures();renderDynasties();renderAlertsCenter();renderLivingAI();renderUnderworld();renderLivingOlympus();renderV150SocialFeed();renderV160Extras();renderV170Extras();renderV180Extras();renderV190Extras();renderAvatar();renderDivinePolitics();renderTitanCouncil();renderDivineReputation();renderWorldExpansion();renderNaval();renderWonders();renderOlympicGamesV110();renderLibraryOfFate();renderCityDevelopment();renderTraining();renderOlympus();renderFate();renderAges();renderPantheon();renderDemigodFamily();renderKingdoms();renderEconomy();renderArtifacts();renderLegacy();renderTitans();renderCouncil();renderPowersPlaceholder();renderHeroes();renderMonsters();renderQuests();renderThrone();renderTreasury();renderLog();
}

function renderWorld(){
 const controlled=state.cities.filter(c=>c.player>c.rival).length;
 const monsters=state.monsters.filter(m=>m.active).length,critical=(state.alerts||[]).filter(a=>!a.resolved&&a.priority==="critical").length;
 const marriages=(state.olympusCore?.unions||[]).filter(u=>u.status==="Married").length,children=Object.values(state.olympusCore?.people||{}).filter(p=>["Immortal Child","Demigod","Ascended Immortal"].includes(p.type)).length;
 $("#home-title").textContent=`${getGod().name}'s Divine Realm`;
 $("#world-summary").innerHTML=`<p class="eyebrow">YEAR ${state.year} • ${getV180Era?.().name||"Age of Myth"}</p><h3 class="home-realm-title">${getGod().symbol} Rule Greece. Shape the myths. Build a dynasty.</h3><p>Every year cities grow, gods form relationships, monsters move, heroes act, kingdoms make decisions, and new stories enter the Chronicle.</p><div class="stats"><div class="stat"><strong>${controlled}/${state.cities.length}</strong><small>Cities Favor You</small></div><div class="stat"><strong>${monsters}</strong><small>Monster Threats</small></div><div class="stat"><strong>${marriages}</strong><small>Divine Marriages</small></div></div><div class="home-status-line"><span class="home-status-chip">🌳 ${children} descendants</span><span class="home-status-chip">🏆 ${state.heroes.filter(h=>h.recruited).length} heroes</span><span class="home-status-chip">⚔️ ${(state.livingWars||[]).filter(w=>w.status==="Active").length} wars</span><span class="home-status-chip">🔔 ${critical} crises</span></div>`;
 const actions=$("#home-action-grid");if(actions){actions.innerHTML=[
  ["🏛️","Develop a City",`${state.cities.filter(c=>c.activeProjectId).length} projects active`,"cities-view",""],
  ["🏆","Command Heroes",`${state.heroes.filter(h=>h.recruited).length} recruited`,"heroes-view",""],
  ["💍","Divine Families",`${marriages} marriages • ${children} descendants`,"family-tree-view",""],
  [critical?"🚨":"🔔",critical?"Resolve Crisis":"View Alerts",critical?`${critical} critical decisions`:"World is stable","alerts-view",critical?"urgent":""]
 ].map(a=>`<button class="home-action ${a[4]}" data-home-view="${a[3]}"><span>${a[0]}</span><strong>${a[1]}</strong><small>${a[2]}</small></button>`).join("");$$('[data-home-view]').forEach(b=>b.onclick=()=>activateView(b.dataset.homeView))}
 const ticker=$("#world-ticker");if(ticker){const stories=(state.activityFeed||state.logs||[]).slice(0,6);ticker.innerHTML=stories.length?stories.map(x=>`<div class="ticker-item"><strong>${x.icon||"📜"} ${x.title||"World Event"}</strong><br>${x.text||x.description||"The living world continues."}</div>`).join(""):`<div class="ticker-item"><strong>🌅 Greece Awakens</strong><br>Advance the year to begin the living simulation.</div>`}
 const threat=$("#map-threat-pill");if(threat)threat.textContent=monsters?`🐉 ${monsters} Threat${monsters===1?'':'s'}`:"✨ Greece Calm";
 renderMapMarkers();
}
function renderMapMarkers(){
 ...
 state.cities.forEach(c=>{const danger=state.monsters.some(m=>m.active&&m.territory===c.name)||c.unrest>=55;$("#map-markers").insertAdjacentHTML("beforeend",`<div class="map-city-status ${danger?'danger':'safe'}" style="left:${c.x}%;top:${c.y}%">${danger?'⚠ '+Math.round(c.unrest)+'% unrest':'● '+Math.round(c.player)+' influence'}</div>`)});
...
function renderCityDevelopment(){
 ...
 $("#city-development-list").innerHTML=state.cities.map(c=>{
  ...
  return `<article class="building-card ${project?"city-project":""}"><div class="building-head"><div><h3>${c.symbol} ${c.name}</h3><p>${c.trait}</p></div><span class="tag gold">${project?project.name:`${c.buildings.length} Buildings`}</span></div><div class="stats"><div class="stat"><strong>${c.happiness}</strong><small>Happiness</small></div><div class="stat"><strong>${c.culture}</strong><small>Culture</small></div><div class="stat"><strong>${state.citizens[c.name].length}</strong><small>Citizens</small></div></div>${project?`<p>${project.icon} ${project.text}</p><div class="meter project-progress"><span style="width:${Math.max(5,(project.years-c.projectYearsLeft)/project.years*100)}%"></span></div><small class="muted">${c.projectYearsLeft} years remaining</small>`:""}<div class="choice-row"><button class="choice-button good" data-v180-develop-city="${c.id}">🏗️ Develop ${c.name}</button><button class="choice-button bad" data-city-full="${c.id}">🏙️ Enter Districts</button></div><div class="city-district-grid">${districtTemplates.slice(0,6).map(d=>`<button class="action-button district-button" data-city-district="${c.id}|${d.id}">${d.icon} ${d.name}</button>`).join("")}</div></article>`;
 }).join("");
 ...
 $$(`[data-v180-develop-city]`).forEach(b=>b.onclick=()=>openCityBuild(b.dataset.v180DevelopCity));
}
...
function openTempleDistrict(c){
 ...
 $$("[data-temple-action]").forEach(b=>b.addEventListener("click",()=>resolveCityTempleDistrictAction(c,b.dataset.templeAction)));
}
function resolveCityTempleDistrictAction(c,a){
...
function completeCityProject(c,p){
 ...
 addLog(`${p.name} Completed in ${c.name}`,p.text);recordChronicleEvent({icon:p.icon,title:`Major Project Completed: ${p.name}`,text:`${p.name} was completed in ${c.name}. ${p.text}`,category:"cities",severity:"mythic",location:c.name,tags:["cities","development"]});
}
...
function initNavigationV2(){
 ...
 window.__navCategoryMap={
  world:{title:"World",items:[["world-view","🗺️","World Map","Cities, movement & events"],["cities-view","🏗️","Develop City","Build districts & improve settlements"],["kingdoms-view","👑","Kingdoms","Rulers & politics"],["citizens-view","👥","Citizens","Living mortal population"],["living-wars-view","⚔️","Wars","Dynamic warfare"],["creatures-view","🐉","Creatures","Roaming monsters"],["civilizations-view","🌐","Civilizations","Foreign peoples"]]},
  divine:{title:"Divine",items:[["avatar-view","👤","Avatar","Walk the world in divine form"],["relationships-view","❤️","Relationships","Gods, Titans & courtship"],["divine-family-view","💍","Divine Family","Marriage & immortal children"],["family-tree-view","🌳","Family Tree","Bloodlines & siblings"],["divine-houses-view","🏛️","Divine Houses","Immortal dynasties"],["living-olympus-view","☁️","Living Olympus","Visit divine locations"],["pantheon-view","⚕️","Pantheon","Gods & divine offices"],["divine-politics-view","⚖️","Politics","Olympian relationships"],["council-view","🏛️","Council","Votes & proposals"]]},
  ...
  more:{title:"More",items:[["cities-view","🏗️","Cities","Build and develop settlements"],["kingdoms-view","👑","Kingdoms","Rulers, diplomacy & wars"],["citizens-view","👥","Citizens","Living mortal population"],["prayers-view","🙏","Prayers","Mortal petitions"],["temples-view","🏛️","Temples","Priests & ceremonies"],["treasury-view","🪙","Treasury","Gold & resources"],["naval-view","⚓","Naval","Fleets & sea routes"],["wonders-view","🏆","Wonders","Great constructions"],["living-chronicle-view","📖","Chronicle","Year-by-year history"],["alerts-view","🔔","Alerts","Important decisions"],["living-ai-view","🧠","Living AI","Autonomous characters"]]}
 };
 $$(".nav-category").forEach(btn=>btn.onclick=()=>openNavigationSheet(btn.dataset.navCategory));
 $$("[data-nav-home]").forEach(btn=>btn.onclick=()=>{activateView(btn.dataset.navHome);closeNavigationSheet();$$('.nav-category').forEach(x=>x.classList.remove('active'));$$('.nav-home').forEach(x=>x.classList.add('active'))});
...
function openNavigationSheet(category){
 ...
 $$(".nav-home").forEach(b=>b.classList.remove("active"));
...
function pantheonGenerationsYearTurn(){
 const dr=state.divineRelationships;
 // v1.5.0 owns autonomous marriage and births. This legacy turn now only ages descendants.
...
const stage8AmbitionPool=["Become the most influential voice on Olympus","Gain a divine office","Found a powerful dynasty","Acquire a legendary artifact","Mentor a future hero","Defeat a hated rival","Protect a favored city","Restore Titan honor","Win a Council majority","Create a famous bloodline","Earn immortality","Become the greatest hero alive","Rule a mortal kingdom","Fulfill a prophecy"];
const stage8SchemePool=["Gather allies against a rival","Undermine a succession claim","Secretly support a mortal rebellion","Sabotage an ascension vote","Discredit a divine house","Manipulate a prophecy","Recruit followers to a hidden faction","Acquire a sacred artifact before a rival","Turn two allies against one another"];
const stage8FactionTemplates=[{id:"olympian-loyalists",name:"Olympian Loyalists",icon:"⚡",goal:"Preserve Olympian authority",alignment:"Olympus"},{id:"titan-restoration",name:"Titan Restorationists",icon:"⛓️",goal:"Restore ancient Titan influence",alignment:"Titans"},{id:"hero-advocates",name:"Champions of Heroes",icon:"🏆",goal:"Expand hero and demigod power",alignment:"Heroes"},{id:"mortal-guardians",name:"Guardians of Mortals",icon:"🕊️",goal:"Limit destructive divine intervention",alignment:"Mortals"},{id:"fate-keepers",name:"Keepers of Fate",icon:"🧵",goal:"Protect prophecy and sacred destiny",alignment:"Fate"}];
const stage8OfficeTemplates=[{id:"olympus-ruler",name:"Ruler of Olympus",icon:"👑",power:15,description:"Shapes major Council politics."},{id:"law-keeper",name:"Keeper of Divine Law",icon:"⚖️",power:10,description:"Influences judgments and sacred law."},{id:"war-marshal",name:"War Marshal",icon:"⚔️",power:10,description:"Influences warfare and hero deployment."},{id:"prophecy-keeper",name:"Keeper of Prophecy",icon:"🔮",power:9,description:"Influences Fate and oracle events."},{id:"mortal-guardian",name:"Guardian of Mortal Affairs",icon:"🏛️",power:8,description:"Influences cities, prayers, and mortal politics."},{id:"titan-ambassador",name:"Ambassador to the Titans",icon:"⛓️",power:8,description:"Influences Titan relations."}];

const stage9FavorRanks=[{name:"Unfavored",min:0,icon:"🌑"},{name:"Blessed",min:50,icon:"✨"},{name:"Divinely Favored",min:100,icon:"🌟"},{name:"Chosen of Fate",min:200,icon:"🧵"},{name:"Radiant Authority",min:350,icon:"☀️"},{name:"Supreme Divine Favor",min:500,icon:"👑"}];
const stage9FavorDeeds=[{id:"answer-prayer",icon:"🙏",name:"Answer a Worthy Prayer",reward:8,cooldown:1},{id:"family-duty",icon:"🌳",name:"Honor the Divine Family",reward:10,cooldown:2},{id:"city-protection",icon:"🏛️",name:"Protect a Favored City",reward:12,cooldown:2},{id:"sacred-oath",icon:"📜",name:"Keep a Sacred Oath",reward:14,cooldown:3},{id:"titan-peace",icon:"⛓️",name:"Advance Titan Reconciliation",reward:16,cooldown:3},{id:"heroic-duty",icon:"🏆",name:"Sponsor a Heroic Deed",reward:12,cooldown:2},{id:"council-duty",icon:"⚖️",name:"Fulfill a Council Duty",reward:10,cooldown:2},{id:"prophecy-duty",icon:"🧵",name:"Respect the Threads of Fate",reward:15,cooldown:3}];


function ensureStage10MarriageRegistry(){
 if(!state.unifiedOlympus)state.unifiedOlympus={favorDeeds:{},syncLog:[],lastSyncYear:0,masterPeople:{},familyLinks:[],integrity:{family:true,houses:true,children:true,chronicle:true}};
 if(!state.unifiedOlympus.npcMarriages)state.unifiedOlympus.npcMarriages=[];
 if(!state.unifiedOlympus.npcHouses)state.unifiedOlympus.npcHouses=[];
}

function stage10CanonicalMarriage(aName,bName,meta={}){
 ensureStage10MarriageRegistry();
 let m=state.unifiedOlympus.npcMarriages.find(x=>(x.a===aName&&x.b===bName)||(x.a===bName&&x.b===aName));
 if(!m){
  m={id:uid("canonicalMarriage"),a:aName,b:bName,year:meta.year??state.year,type:meta.type||"Divine Union",status:"Married",source:meta.source||"Matchmaking"};
  state.unifiedOlympus.npcMarriages.push(m);
 }else{
  m.status="Married";
...
function stage10RepairAllMarriages(){
 ensureStage10MarriageRegistry();

 // Import Stage 10 weddings into canonical registry.
 (state.divineMatchmaking?.weddings||[]).forEach(w=>{
  stage10CanonicalMarriage(w.a,w.b,{year:w.year,type:w.type,source:"Matchmaking"});
 });

 // Import older NPC marriage system too.
 (state.divineRelationships?.npcMarriages||[]).filter(m=>m.status==="Married").forEach(m=>{
  stage10CanonicalMarriage(m.a,m.b,{year:m.year,type:m.type,source:"Autonomous"});
 });

 // Push every canonical marriage into every connected system.
 state.unifiedOlympus.npcMarriages.filter(m=>m.status==="Married").forEach(m=>{
  const roster=state.divineRelationships?.roster||[];
  const a=roster.find(r=>r.name===m.a), b=roster.find(r=>r.name===m.b);

  // 1. Relationship roster
  if(a){a.status="Married";a.autonomousPartner=m.b}
  if(b){b.status="Married";b.autonomousPartner=m.a}

  // 2. Unified Olympus canonical people
  stage9CanonicalPerson(m.a,{icon:a?.icon||"✨",type:a?.type||"Immortal"}).spouse=m.b;
  stage9CanonicalPerson(m.b,{icon:b?.icon||"✨",type:b?.type||"Immortal"}).spouse=m.a;

  // 3. Divine Family Tree
  let an=state.divineFamily.familyTree.find(n=>n.name===m.a);
  let bn=state.divineFamily.familyTree.find(n=>n.name===m.b);
  if(!an){
   an={id:uid("family"),name:m.a,icon:a?.icon||"✨",type:a?.type||"Immortal",relation:a?.type==="Titan"?"Titan Kin":"Divine Kin",parents:[],spouse:m.b,children:[]};
   state.divineFamily.familyTree.push(an);
  }
  if(!bn){
   bn={id:uid("family"),name:m.b,icon:b?.icon||"✨",type:b?.type||"Immortal",relation:b?.type==="Titan"?"Titan Kin":"Divine Kin",parents:[],spouse:m.a,children:[]};
   state.divineFamily.familyTree.push(bn);
  }
  an.spouse=m.b;bn.spouse=m.a;

  // 4. Old NPC marriage system
  if(!state.divineRelationships.npcMarriages.some(x=>(x.a===m.a&&x.b===m.b)||(x.a===m.b&&x.b===m.a))){
   state.divineRelationships.npcMarriages.push({id:uid("npcMarriage"),a:m.a,b:m.b,year:m.year,type:m.type,status:"Married"});
  }else{
   const old=state.divineRelationships.npcMarriages.find(x=>(x.a===m.a&&x.b===m.b)||(x.a===m.b&&x.b===m.a));
   old.status="Married";
  }

  // 5. Divine House
  const houseName=`House of ${m.a} & ${m.b}`;
  let house=state.divineRelationships.houses.find(h=>h.founders?.includes(m.a)&&h.founders?.includes(m.b));
  if(!house){
   house={id:uid("divineHouse"),name:houseName,icon:"🏛️",founders:[m.a,m.b],type:m.type?.includes("Titan")?"Olympian-Titan House":"Divine House",children:[],demigods:[],influence:50,unity:70,history:[`Founded by marriage in Year ${m.year}.`]};
   state.divineRelationships.houses.push(house);
  }
  house.name=houseName;
  house.founders=[m.a,m.b];

  // 6. Children from either system are attached to both parents + house.
  const kids=(state.divineRelationships?.npcImmortalChildren||[]).filter(c=>(c.parentA===m.a&&c.parentB===m.b)||(c.parentA===m.b&&c.parentB===m.a));
  house.children=kids.map(c=>c.name);
  kids.forEach(c=>{
   c.house=houseName;
   let cn=state.divineFamily.familyTree.find(n=>n.name===c.name);
   if(!cn){
    cn={id:uid("family"),name:c.name,icon:c.icon||"✨",type:"Immortal Child",relation:"Extended Divine Kin",parents:[m.a,m.b],spouse:null,children:[]};
    state.divineFamily.familyTree.push(cn);
   }
   cn.parents=[m.a,m.b];
   if(!an.children.includes(c.name))an.children.push(c.name);
   if(!bn.children.includes(c.name))bn.children.push(c.name);
   stage9CanonicalPerson(c.name,{icon:c.icon||"✨",type:"Immortal Child",status:c.stage||"Immortal"}).house=houseName;
   stage9LinkParentChild(m.a,c.name);stage9LinkParentChild(m.b,c.name);
  });

  // 7. Living Souls
  const pa=getStage8Profile?.(m.a), pb=getStage8Profile?.(m.b);
  if(pa)rememberStage8(m.a,`Married ${m.b} in Year ${m.year}.`,4);
  if(pb)rememberStage8(m.b,`Married ${m.a} in Year ${m.year}.`,4);
 });

 syncStage10Dynasties();
}

function stage10SpouseOf(name){
 ensureStage10MarriageRegistry();
 const m=state.unifiedOlympus.npcMarriages.find(x=>x.status==="Married"&&(x.a===name||x.b===name));
 return m?(m.a===name?m.b:m.a):null;
}



function repairStage11MatchmakingState(){
 if(!state.divineMatchmaking)return;

 const durable=[];
 (state.divineMatchmaking.weddings||[]).forEach(w=>durable.push({a:w.a,b:w.b,year:w.year,type:w.type}));
 (state.divineRelationships?.npcMarriages||[]).filter(m=>m.status==="Married").forEach(m=>durable.push({a:m.a,b:m.b,year:m.year,type:m.type}));
 (state.unifiedOlympus?.npcMarriages||[]).filter(m=>m.status==="Married").forEach(m=>durable.push({a:m.a,b:m.b,year:m.year,type:m.type}));

 const seen=new Set();
 durable.forEach(m=>{
  const key=stage11UnionKey(m.a,m.b);if(seen.has(key))return;
  seen.add(key);
  stage11CompleteMarriage(m.a,m.b,{year:m.year,type:m.type,source:"Save Repair"});
 });

 (state.divineMatchmaking.arrangedMatches||[]).forEach(m=>{
  const key=stage11UnionKey(m.a,m.b);
  if(seen.has(key)){m.status="Married";return}
  if(!m.accepted)return;

  m.status="Betrothed";
  state.alerts=(state.alerts||[]).filter(a=>!(a.targetId===m.id && a.title?.startsWith("Match Accepted:")));
  state.alerts.unshift({
   id:uid("alert"),status:"active",createdYear:m.year||state.year,expiresYear:state.year+3,
   priority:"urgent",category:"family",icon:"💞",
   title:`Match Accepted: ${m.a} & ${m.b}`,
   description:"Both immortals accepted your proposed union. Complete the formal divine wedding to make the marriage permanent.",
   location:"Mount Olympus",targetType:"matchmaking",targetId:m.id,source:"Divine Matchmaking",relatedCount:1,
   choices:[
    {id:"wedding",label:"💍 Hold Divine Wedding",cost:null,amount:0,effect:"holdWedding"},
    {id:"view",label:"💞 Open Matchmaking",cost:null,amount:0,effect:"openMatchmaking"},
    {id:"cancel",label:"💔 Cancel Betrothal",cost:null,amount:0,effect:"cancelBetrothal"}
   ]
  });
 });

 state.alerts=(state.alerts||[]).filter(a=>!a.resolved && a.status!=="Resolved" && a.status!=="resolved");
}


const v150GatheringTemplates=[
 {icon:"🍷",name:"Feast of Olympus",place:"Hall of the Gods"},
 {icon:"🏛️",name:"Council Banquet",place:"Olympian Council"},
 {icon:"🎭",name:"Festival of the Muses",place:"Sacred Gardens"},
 {icon:"🔥",name:"Hearth Gathering",place:"Hestia's Hearth"},
 {icon:"🌙",name:"Moonlit Assembly",place:"Artemis' Grove"},
 {icon:"🌊",name:"Sea Court Reception",place:"Poseidon's Gate"}
];


const v160LodgeDefinitions={
 Zeus:{name:"Storm Lodge",icon:"⚡",focus:"Leadership & Sky Power"},
 Poseidon:{name:"Tide Lodge",icon:"🔱",focus:"Watercraft & Endurance"},
 Athena:{name:"Owl Lodge",icon:"🦉",focus:"Strategy & Wisdom"},
 Ares:{name:"War Lodge",icon:"⚔️",focus:"Combat & Courage"},
 Aphrodite:{name:"Rose Lodge",icon:"🌹",focus:"Charisma & Diplomacy"},
 Hades:{name:"Underworld Lodge",icon:"💀",focus:"Spirit Lore & Resolve"},
 Apollo:{name:"Sun Lodge",icon:"☀️",focus:"Archery, Healing & Prophecy"},
 Artemis:{name:"Moon Lodge",icon:"🌙",focus:"Archery & Survival"},
 Hermes:{name:"Winged Lodge",icon:"🪽",focus:"Speed, Travel & Cunning"},
 Hephaestus:{name:"Forge Lodge",icon:"🔥",focus:"Crafting & Invention"},
 Hera:{name:"Peacock Lodge",icon:"🦚",focus:"Leadership & Family Duty"},
 Dionysus:{name:"Vine Lodge",icon:"🍇",focus:"Creativity & Spirit"},
 Demeter:{name:"Harvest Lodge",icon:"🌾",focus:"Nature & Sustenance"},
 Hecate:{name:"Torch Lodge",icon:"🔮",focus:"Magic & Crossroads"},
 Kronos:{name:"Time Lodge",icon:"⏳",focus:"Titan Legacy & Endurance"},
 Rhea:{name:"Mountain Lodge",icon:"⛰️",focus:"Protection & Ancient Lore"},
 Oceanus:{name:"River Lodge",icon:"🌊",focus:"Water & Titan Heritage"},
 Themis:{name:"Law Lodge",icon:"⚖️",focus:"Justice & Prophecy"},
 Prometheus:{name:"Flame Lodge",icon:"🔥",focus:"Forethought & Mortal Progress"},
 Atlas:{name:"Star Lodge",icon:"🌌",focus:"Strength & Navigation"}
};

const v160CampLocations=[
 {id:"arena",icon:"⚔️",name:"Training Arena",desc:"Practice combat, athletics, and team tactics."},
 {id:"forge",icon:"🔥",name:"Divine Forge",desc:"Craft equipment and learn from sacred smiths."},
 {id:"oracle",icon:"🔮",name:"Oracle Shrine",desc:"Seek prophecies and form demigod quest parties."},
 {id:"woods",icon:"🌲",name:"Sacred Woods",desc:"Train survival, monster knowledge, and tracking."},
 {id:"lake",icon:"🌊",name:"Silver Lake",desc:"Rest, socialize, train water skills, and build friendships."},
 {id:"stables",icon:"🐎",name:"Pegasus Stables",desc:"Practice riding and prepare for distant quests."},
 {id:"healing",icon:"🏥",name:"Healing Temple",desc:"Recover injuries and study divine medicine."},
 {id:"pavilion",icon:"🍽️",name:"Dining Pavilion",desc:"Host camp feasts and strengthen lodge relationships."}
];


const v170GreekNamePool=["Asteria","Theron","Lykaon","Melia","Dorian","Thalia","Callista","Leandros","Evadne","Orion","Nerissa","Cassian","Ione","Phaedra","Damian","Selene","Xanthe","Alcaeus","Iris","Eryx","Theon","Cyras","Maia","Elara","Nikos","Daphne","Perseon","Lysandra","Ariadne","Phoebe","Adrasteia","Evander","Calix","Myrine","Timon","Cyrene","Leontes","Euphemia","Aeson"];
const v170CampRoles=[
 {name:"Lodge Leader",minGlory:70,skill:"leadership",minSkill:60,icon:"👑"},
 {name:"War Games Captain",minGlory:55,skill:"strategy",minSkill:58,icon:"⚔️"},
 {name:"Oracle Candidate",minGlory:45,skill:"monsterLore",minSkill:55,icon:"🔮"},
 {name:"Forge Apprentice",minGlory:35,skill:"divinePower",minSkill:50,icon:"🔥"},
 {name:"Healer",minGlory:30,skill:"leadership",minSkill:48,icon:"🏥"},
 {name:"Scout",minGlory:30,skill:"survival",minSkill:52,icon:"🌲"},
 {name:"Monster Specialist",minGlory:40,skill:"monsterLore",minSkill:62,icon:"🐉"}
];


const v180CityUpgrades=[
 {id:"housing",icon:"🏘️",name:"Expand Housing",cost:35,text:"Increase population capacity and happiness.",apply:c=>{c.population+=35;c.happiness=clamp(c.happiness+5);c.wealth=clamp(c.wealth+2)}},
 {id:"farms",icon:"🌾",name:"Improve Farms",cost:30,text:"Increase food production and city stability.",apply:c=>{state.food+=60;c.population+=15;c.happiness=clamp(c.happiness+4)}},
 {id:"market",icon:"🪙",name:"Expand Markets",cost:40,text:"Increase wealth and yearly economic strength.",apply:c=>{c.wealth=clamp(c.wealth+10);state.gold+=12}},
 {id:"temple",icon:"🏛️",name:"Restore Temples",cost:35,text:"Increase devotion, Faith and player influence.",apply:c=>{c.player=clamp(c.player+7);state.faith+=18;c.happiness=clamp(c.happiness+3)}},
 {id:"walls",icon:"🛡️",name:"Strengthen Fortifications",cost:45,text:"Reduce unrest and improve protection.",apply:c=>{c.protected=true;c.unrest=clamp(c.unrest-12);c.military=clamp((c.military||50)+7)}},
 {id:"culture",icon:"🎭",name:"Fund Culture",cost:30,text:"Improve culture, happiness and Prestige.",apply:c=>{c.culture=clamp(c.culture+12);c.happiness=clamp(c.happiness+7);state.prestige+=4}},
 {id:"military",icon:"⚔️",name:"Train City Guard",cost:40,text:"Increase military strength and reduce unrest.",apply:c=>{c.military=clamp((c.military||50)+12);c.unrest=clamp(c.unrest-5)}},
 {id:"roads",icon:"🛣️",name:"Build Roads",cost:35,text:"Improve commerce, wealth and population growth.",apply:c=>{c.wealth=clamp(c.wealth+7);c.population+=20;c.culture=clamp(c.culture+3)}},
 {id:"sanitation",icon:"💧",name:"Improve Sanitation",cost:30,text:"Improve happiness and long-term population growth.",apply:c=>{c.happiness=clamp(c.happiness+9);c.population+=18}},
 {id:"academy",icon:"📚",name:"Expand Academy",cost:45,text:"Improve culture and local hero learning.",apply:c=>{c.culture=clamp(c.culture+14);state.heroes.filter(h=>h.location===c.name).forEach(h=>h.xp+=15)}},
 {id:"harbor",icon:"⚓",name:"Improve Harbor",cost:45,text:"Increase wealth and strengthen naval access.",apply:c=>{c.wealth=clamp(c.wealth+10);state.gold+=15}}
];

const v180Eras=[
 {id:"founding",name:"Founding Era",min:1,max:5,icon:"💍",text:"Divine marriages, first Houses, early children and city foundations are strongly favored."},
 {id:"rising",name:"Rising Houses",min:6,max:15,icon:"🌳",text:"Bloodlines grow, Camp Elysium fills, cities develop and Divine Houses compete."},
 {id:"heroic",name:"Heroic Era",min:16,max:35,icon:"🏆",text:"Heroes, quests, War Games, monsters and legendary deeds become more prominent."},
 {id:"dynastic",name:"Dynastic Era",min:36,max:70,icon:"👑",text:"Great Houses, succession, factions, offices and generational politics dominate Olympus."},
 {id:"mythic",name:"Mythic Age",min:71,max:9999,icon:"📜",text:"Ancient grudges, legendary descendants and multigenerational sagas shape the world."}
];


const v190FateTemplates=[
 {kind:"hero",icon:"🏆",text:n=>`${n} shall either become a legendary hero or be forgotten before adulthood.`},
 {kind:"monster",icon:"🐉",text:n=>`${n} shall face a monster that no ordinary warrior can survive.`},
 {kind:"betrayal",icon:"🗡️",text:n=>`${n} shall betray blood or break a feud that has lasted generations.`},
 {kind:"ascension",icon:"🌟",text:n=>`${n} may one day stand among the immortals.`},
 {kind:"rule",icon:"👑",text:n=>`${n} shall rule a House, a city, or both.`},
 {kind:"love",icon:"❤️",text:n=>`${n} shall love someone their House considers an enemy.`},
 {kind:"prophecy",icon:"🔮",text:n=>`${n} shall become the center of a prophecy that changes Olympus.`}
];
const v190LegacyRegistry=[
 {id:"olympus-core",name:"Olympus Core",status:"active",text:"Authoritative marriages, genealogy, children, Houses and spouse records."},
 {id:"family-tree",name:"Unified Family Tree",status:"active",text:"Reads genealogy from Olympus Core."},
 {id:"divine-houses",name:"Divine Houses",status:"active",text:"Reads founders and descendants from Olympus Core."},
 {id:"old-matchmaking",name:"Player Matchmaking",status:"retired",text:"Removed. Autonomous romance and marriage now own divine unions."},
 {id:"legacy-npc-marriages",name:"Legacy NPC Marriage Storage",status:"compatibility",text:"Migration-only mirror; Olympus Core remains authoritative."},
 {id:"old-city-build",name:"Old City Build Stub",status:"retired",text:"Replaced by Develop City."},
 {id:"camp-family-data",name:"Camp Family Records",status:"retired",text:"Camp Elysium reads parentage from Olympus Core."},
 {id:"living-souls",name:"Living Souls",status:"active",text:"Memories, ambitions, schemes and character autonomy."},
 {id:"chronicle",name:"Living Chronicle",status:"active",text:"Permanent history, myths, milestones and major outcomes."}
];
function ensureV190Data(){
 if(!state.v190)state.v190={fates:[],houseRelations:[],succession:[],artifactHistory:[],expeditions:[],lifeEvents:[],legacyRegistry:v190LegacyRegistry.map(x=>({...x})),version:"2.0.0"};
 const v=state.v190;["fates","houseRelations","succession","artifactHistory","expeditions","lifeEvents"].forEach(k=>{if(!v[k])v[k]=[]});if(!v.legacyRegistry)v.legacyRegistry=v190LegacyRegistry.map(x=>({...x}));
 patchV190Navigation();cleanupV190LegacySystems();syncV190HouseRelations();syncV190ArtifactHistory();
}
function patchV190Navigation(){const map=window.__navCategoryMap;if(!map)return;if(map.divine&&!map.divine.items.some(x=>x[0]==="fates-view"))map.divine.items.push(["fates-view","🔮","Fates","Destiny, Houses & succession"]);if(map.chronicle)map.chronicle.items=map.chronicle.items.filter(x=>x[0]!=="system-core-view")}
function cleanupV190LegacySystems(){
 state.alerts=(state.alerts||[]).filter(a=>a.targetType!=="matchmaking"&&!a.title?.startsWith("Match Accepted:")&&!a.title?.startsWith("Wedding Awaiting:"));
 if(state.olympusCore){const unions=(state.olympusCore.unions||[]).filter(u=>u.status==="Married");state.divineRelationships.npcMarriages=unions.filter(u=>u.a!==getGod().name&&u.b!==getGod().name).map(u=>({id:u.id,a:u.a,b:u.b,year:u.year,type:u.type,status:"Married"}));if(state.unifiedOlympus)state.unifiedOlympus.npcMarriages=unions.filter(u=>u.a!==getGod().name&&u.b!==getGod().name).map(u=>({id:u.id,a:u.a,b:u.b,year:u.year,type:u.type,status:"Married",source:"v1.9 compatibility mirror"}))}
 if(state.divineMatchmaking){state.divineMatchmaking.arrangedMatches=[];state.divineMatchmaking.weddings=[];state.divineMatchmaking.dynasties=[]}
}
function v190ImportantPeople(){return Object.values(state.olympusCore?.people||{}).filter(p=>["Immortal Child","Demigod","Ascended Immortal","Olympian","Titan","Titaness"].includes(p.type))}
function createV190Fate(person){if(!person||state.v190.fates.some(f=>f.subject===person.name&&f.status==="Active"))return;const t=randomItem(v190FateTemplates);const f={id:uid("fate"),subject:person.name,icon:t.icon,kind:t.kind,thread:t.text(person.name),status:"Active",year:state.year,pressure:0,response:null,outcome:null};state.v190.fates.unshift(f);recordChronicleEvent({icon:"🔮",title:`The Fates Mark ${person.name}`,text:f.thread,category:"legends",severity:"mythic",major:true,target:person.name,tags:["fate","prophecy"]})}
function resolveV190FateChoice(id,choice){const f=state.v190.fates.find(x=>x.id===id);if(!f||f.status!=="Active")return;f.response=choice;f.pressure+=choice==="accept"?2:choice==="defy"?5:choice==="oracle"?3:4;let summary="";if(choice==="accept"){summary=`You accepted the thread woven for ${f.subject}.`;state.favor+=4}if(choice==="defy"){summary=`You openly defied the Fates concerning ${f.subject}.`;state.favor=Math.max(0,state.favor-3);state.prestige+=5}if(choice==="oracle"){summary=`You sought the Oracle for deeper meaning concerning ${f.subject}.`;state.faith+=6}if(choice==="change"){summary=`You began altering the path surrounding ${f.subject}.`;state.prestige+=3}recordAfterAction({icon:"🔮",title:`Fate Decision: ${f.subject}`,summary,category:"legends",target:f.subject,changes:[{label:"Response",value:choice},{label:"Fate Pressure",value:`${f.pressure}`}],severity:choice==="defy"?"danger":"mythic"});renderAll();saveGame(false)}
function v190IsHouseLeader(name){return Object.values(state.olympusCore?.houses||{}).some(h=>h.leader===name)}
function progressV190Fates(){state.v190.fates.filter(f=>f.status==="Active").forEach(f=>{f.pressure+=rand(0,2);const p=state.olympusCore?.people?.[f.subject],camper=state.v160?.campers?.[f.subject],hero=(state.heroes||[]).find(h=>h.name===f.subject&&h.recruited);if(f.kind==="hero"&&(hero||camper?.campGlory>=80)){f.status="Fulfilled";f.outcome=`${f.subject} rose toward legendary heroism.`}else if(f.kind==="ascension"&&p?.type==="Ascended Immortal"){f.status="Fulfilled";f.outcome=`${f.subject} achieved immortality.`}else if(f.kind==="rule"&&v190IsHouseLeader(f.subject)){f.status="Fulfilled";f.outcome=`${f.subject} became a House leader.`}else if(f.pressure>=12&&Math.random()<.22){f.status=f.response==="defy"&&Math.random()<.55?"Broken":"Twisted";f.outcome=f.status==="Broken"?`The foretold path of ${f.subject} was successfully broken.`:`The destiny of ${f.subject} manifested in an unexpected form.`}if(f.status!=="Active")recordChronicleEvent({icon:f.icon,title:`Fate ${f.status}: ${f.subject}`,text:f.outcome,category:"legends",severity:"mythic",major:true,target:f.subject,tags:["fate","prophecy"]})})}
function v190ChildLifeEvents(){
 const descendants=Object.values(state.olympusCore?.people||{}).filter(p=>["Immortal Child","Demigod"].includes(p.type));
 descendants.forEach(p=>{const source=v160AllDemigods().find(d=>d.name===p.name)||(state.divineRelationships?.npcImmortalChildren||[]).find(c=>c.name===p.name)||(state.divineFamily?.immortalChildren||[]).find(c=>c.name===p.name);const age=source?.age;if(age===undefined)return;const milestones=[[5,"Personality Awakens",()=>{p.temperament=p.temperament||randomItem(["Bold","Curious","Compassionate","Proud","Patient","Ambitious","Loyal","Rebellious"])}],[8,"Divine Power Awakens",()=>{p.divinePotential=clamp((p.divinePotential||50)+5)}],[12,"Natural Talent Emerges",()=>{p.naturalTalent=p.naturalTalent||randomItem(["Strategy","Combat","Healing","Prophecy","Leadership","Crafting","Survival","Charisma"])}],[15,"Adolescent Bonds",()=>v190CreateYouthBond(p)],[16,"Destiny Becomes Clearer",()=>{if(!state.v190.fates.some(f=>f.subject===p.name))createV190Fate(p)}],[18,"Adulthood Begins",()=>v190AdultDecision(p)]];milestones.forEach(([target,title,fn])=>{const key=`${p.name}|${target}`;if(age>=target&&!state.v190.lifeEvents.some(e=>e.key===key)){fn();state.v190.lifeEvents.unshift({key,year:state.year,subject:p.name,title,age:target});recordChronicleEvent({icon:"🌱",title:`${p.name}: ${title}`,text:`At age ${target}, ${p.name} reached a defining life milestone.`,category:"divine-family",severity:"success",target:p.name,tags:["childhood","generations"]})}})})
}
function v190CreateYouthBond(p){const peers=Object.values(state.olympusCore?.people||{}).filter(x=>x.name!==p.name&&["Immortal Child","Demigod","Ascended Immortal"].includes(x.type));if(!peers.length)return;const q=randomItem(peers),kind=randomItem(["Friendship","Rivalry","First Crush","Mentorship"]);p.youthBonds=p.youthBonds||[];p.youthBonds.push({name:q.name,kind,year:state.year});if(kind==="Rivalry")createStage8Rivalry(p.name,q.name,"A rivalry formed during adolescence.")}
function v190AdultDecision(p){p.adultPath=randomItem(["Heroism","Divine Politics","Scholarship","Temple Service","Adventure","House Leadership"]);const camper=state.v160?.campers?.[p.name];if(camper&&p.adultPath==="Heroism")camper.campGlory+=10}
function syncV190HouseRelations(){const houses=Object.values(state.olympusCore?.houses||{}),rel=state.v190.houseRelations;houses.forEach((a,i)=>houses.slice(i+1).forEach(b=>{const key=[a.name,b.name].sort().join("||");if(!rel.some(r=>r.key===key))rel.push({key,a:a.name,b:b.name,score:50,status:"Neutral",history:[]})}));rel.forEach(r=>{r.status=r.score>=75?"Allied":r.score>=60?"Friendly":r.score>=40?"Neutral":r.score>=20?"Rival":"Feuding"})}
function v190HousePoliticsTurn(){syncV190HouseRelations();state.v190.houseRelations.forEach(r=>{if(Math.random()<.12){const delta=rand(-8,8);r.score=clamp(r.score+delta);if(Math.abs(delta)>=6)r.history.unshift({year:state.year,text:delta>0?"A diplomatic gesture improved relations.":"A public dispute damaged relations."})}})}
function v190SuccessionTurn(){Object.values(state.olympusCore?.houses||{}).forEach(h=>{if(!h.leader)h.leader=h.founders?.[0]||null;const descendants=[...(h.children||[]),...(h.demigods||[])];if(!h.heir&&descendants.length)h.heir=descendants.slice().sort((a,b)=>(state.olympusCore.people[b]?.divinePotential||50)-(state.olympusCore.people[a]?.divinePotential||50))[0]})}
function v190CloseKin(a,b){const ap=new Set(a.parents||[]),bp=new Set(b.parents||[]);if(a.parents?.includes(b.name)||b.parents?.includes(a.name))return true;return [...ap].some(x=>bp.has(x))}
function v190NextGenerationRomanceTurn(){const adults=Object.values(state.olympusCore?.people||{}).filter(p=>["Immortal Child","Ascended Immortal"].includes(p.type)&&!p.spouse);if(adults.length<2||Math.random()>.18)return;const a=randomItem(adults),cands=adults.filter(b=>b.name!==a.name&&!v190CloseKin(a,b));if(!cands.length)return;const b=randomItem(cands);a.romanceInterest=b.name;b.romanceInterest=a.name;if(Math.random()<.38){stage11CompleteMarriage(a.name,b.name,{year:state.year,type:"Next-Generation Divine Union",source:"v1.9 Descendant Romance"});recordChronicleEvent({icon:"❤️",title:`Next-Generation Marriage: ${a.name} & ${b.name}`,text:`A new branch of the divine family formed when ${a.name} and ${b.name} married.`,category:"marriages",severity:"mythic",major:true,tags:["generations","divine-family"]})}}
function syncV190ArtifactHistory(){[...(state.divineRule?.dynamicArtifacts||[]),...(state.heroArtifacts||[])].forEach(a=>{if(!a)return;let h=state.v190.artifactHistory.find(x=>x.name===a.name);if(!h)state.v190.artifactHistory.push({id:uid("artifactHistory"),name:a.name,icon:a.icon||"🏺",owner:(a.owners||[])[0]||a.owner||null,history:[],createdYear:a.year||state.year})})}
function v190RecordArtifactEvent(name,text,owner=null){let h=state.v190.artifactHistory.find(x=>x.name===name);if(!h){h={id:uid("artifactHistory"),name,icon:"🏺",owner:null,history:[],createdYear:state.year};state.v190.artifactHistory.push(h)}if(owner)h.owner=owner;h.history.unshift({year:state.year,text})}
function v190MonsterExpedition(){const heroes=(state.heroes||[]).filter(h=>h.recruited);if(!heroes.length){showToast("Recruit or graduate at least one hero first.");return}const monster=randomItem(monsterTemplates||[{name:"Hydra",strength:70,weakness:"Fire"}]);const party=[...heroes].sort((a,b)=>(b.reputation||0)-(a.reputation||0)).slice(0,Math.min(3,heroes.length));const power=party.reduce((s,h)=>s+(h.level||1)*12+(h.reputation||0),0)+Math.random()*80,difficulty=(monster.strength||65)+Math.random()*70,success=power>=difficulty;const event={id:uid("expedition"),year:state.year,monster:monster.name,party:party.map(h=>h.name),success,artifact:null};party.forEach(h=>{h.xp=(h.xp||0)+(success?35:15);h.reputation=(h.reputation||0)+(success?8:2)});if(success&&Math.random()<.45){const artifact=`${monster.name}-Bane of Year ${state.year}`;event.artifact=artifact;v190RecordArtifactEvent(artifact,`Created after ${party[0].name} and companions defeated ${monster.name}.`,party[0].name);if(state.divineRule?.dynamicArtifacts)state.divineRule.dynamicArtifacts.unshift({id:uid("dynRelic"),name:artifact,icon:"🗡️",power:"Monster Slayer",owners:[party[0].name],history:[`Forged from the victory over ${monster.name}.`],year:state.year})}state.v190.expeditions.unshift(event);recordAfterAction({icon:"🐉",title:`Monster Expedition: ${monster.name}`,summary:success?`${party.map(h=>h.name).join(", ")} defeated ${monster.name}.`:`The party survived but failed to defeat ${monster.name}.`,category:"heroes",changes:[{label:"Party",value:party.map(h=>h.name).join(", ")},{label:"Outcome",value:success?"Victory":"Failure"},{label:"Artifact",value:event.artifact||"None"}],severity:success?"mythic":"danger"});renderAll();saveGame(false)}
function v190AutonomousWorldTurn(){const people=v190ImportantPeople();if(people.length&&Math.random()<.2){const p=randomItem(people),profile=getStage8Profile?.(p.name);if(profile){profile.ambitionProgress=clamp((profile.ambitionProgress||0)+rand(2,7));if(Math.random()<.12)rememberStage8(p.name,`Pursued ${profile.ambition.toLowerCase()} without player intervention.`,2)}}}
function v190YearTurn(){if(Math.random()<.18){const candidates=v190ImportantPeople().filter(p=>!state.v190.fates.some(f=>f.subject===p.name&&f.status==="Active"));if(candidates.length)createV190Fate(randomItem(candidates))}progressV190Fates();v190ChildLifeEvents();v190HousePoliticsTurn();v190SuccessionTurn();v190NextGenerationRomanceTurn();v190AutonomousWorldTurn();cleanupV190LegacySystems();syncV190ArtifactHistory()}
function renderV190Fates(){
 if(!document.getElementById("fates-view")||!state.v190)return;syncV190HouseRelations();cleanupV190LegacySystems();const active=state.v190.fates.filter(f=>f.status==="Active");
 $("#fates-count").textContent=`${active.length} Active`;$("#fates-summary").innerHTML=`<p class="eyebrow">CLOTHO • LACHESIS • ATROPOS</p><h3>Destiny follows specific people across childhood, Camp Elysium, heroism, Houses and immortality</h3><div class="stats"><div class="stat"><strong>${active.length}</strong><small>Active Threads</small></div><div class="stat"><strong>${state.v190.fates.filter(f=>f.status==="Fulfilled").length}</strong><small>Fulfilled</small></div><div class="stat"><strong>${state.v190.fates.filter(f=>f.status==="Broken").length}</strong><small>Broken</small></div></div>`;
 $("#fate-thread-list").innerHTML=state.v190.fates.length?state.v190.fates.slice(0,16).map(f=>`<article class="card fate-card ${f.status!=="Active"?"fate-resolved":""}"><div class="card-row"><div><h3>${f.icon} ${f.subject}</h3><p>Thread woven in Year ${f.year}</p></div><span class="tag gold">${f.status}</span></div><p class="fate-thread">“${f.thread}”</p>${f.outcome?`<p>${f.outcome}</p>`:""}${f.status==="Active"?`<div class="fate-choice-grid"><button class="fate-choice" data-fate-id="${f.id}" data-fate-choice="accept">✨ Accept Fate</button><button class="fate-choice" data-fate-id="${f.id}" data-fate-choice="change">🧵 Change Fate</button><button class="fate-choice" data-fate-id="${f.id}" data-fate-choice="oracle">🔮 Seek Oracle</button><button class="fate-choice" data-fate-id="${f.id}" data-fate-choice="defy">⚡ Defy Fates</button></div>`:""}</article>`).join(""):'<article class="card"><p>No active threads have been revealed yet.</p></article>';
 $$('[data-fate-id]').forEach(b=>b.onclick=()=>resolveV190FateChoice(b.dataset.fateId,b.dataset.fateChoice));
 const houses=Object.values(state.olympusCore?.houses||{});$("#house-politics-list").innerHTML=houses.length?houses.map(h=>`<article class="card house-politics-card"><div class="card-row"><div><h3>🏛️ ${h.name}</h3><p>Leader: ${h.leader||h.founders?.[0]||"None"} • Heir: ${h.heir||"Undecided"}</p></div><span class="tag gold">${(h.children?.length||0)+(h.demigods?.length||0)} Descendants</span></div><div class="house-politics-grid"><div class="house-politics-stat"><strong>${h.influence||50}</strong><small>Influence</small></div><div class="house-politics-stat"><strong>${h.unity||70}</strong><small>Unity</small></div><div class="house-politics-stat"><strong>${h.leader||"None"}</strong><small>House Head</small></div></div><p>${state.v190.houseRelations.filter(r=>r.a===h.name||r.b===h.name).slice(0,3).map(r=>`${r.a===h.name?r.b:r.a}: ${r.status}`).join(" • ")||"No major House relations yet."}</p></article>`).join(""):'<article class="card"><p>No Divine Houses exist yet.</p></article>';
 $("#legacy-cleanup-list").innerHTML=(state.v190.legacyRegistry||[]).map(x=>`<article class="card legacy-clean-card ${x.status==="retired"?"retired":""}"><div class="card-row"><div><h3>${x.status==="active"?"✅":x.status==="retired"?"🗑️":"🔁"} ${x.name}</h3><p>${x.text}</p></div><span class="tag gold">${x.status.toUpperCase()}</span></div></article>`).join("");
}
function renderV190Extras(){renderV190Fates()}

function ensureV180Data(){
 if(!state.v180)state.v180={cityUpgradeHistory:[],systemHealth:{},lastAuditYear:0,version:"1.8.0"};
 if(!state.v180.cityUpgradeHistory)state.v180.cityUpgradeHistory=[];if(!state.v180.systemHealth)state.v180.systemHealth={};
 patchV180Navigation();ensureV180CityData();auditV180Systems(true);
}
function patchV180Navigation(){
 const map=window.__navCategoryMap;if(!map)return;
 if(map.divine&&!map.divine.items.some(x=>x[0]==="avatar-view"))map.divine.items.unshift(["avatar-view","👤","Avatar","Walk the world in divine form"]);
 if(map.world){
  const city=map.world.items.find(x=>x[0]==="cities-view");if(city){city[1]="🏗️";city[2]="Develop City";city[3]="Build districts & improve settlements"}
 }
 if(map.chronicle&&!map.chronicle.items.some(x=>x[0]==="systems-overview-view"))map.chronicle.items.push(["systems-overview-view","⚙️","Systems Overview","Era pacing & system health"]);
}
function ensureV180CityData(){
 state.cities.forEach(c=>{
  if(!c.v180Upgrades)c.v180Upgrades={};
  v180CityUpgrades.forEach(u=>{if(c.v180Upgrades[u.id]===undefined)c.v180Upgrades[u.id]=0});
  if(c.military===undefined)c.military=50;if(c.wealth===undefined)c.wealth=50;if(c.unrest===undefined)c.unrest=20;
 });
}
function getV180Era(){return v180Eras.find(e=>state.year>=e.min&&state.year<=e.max)||v180Eras[v180Eras.length-1]}
function v180EraProgress(){const e=getV180Era();if(e.max>1000)return 100;return clamp(Math.round((state.year-e.min)/Math.max(1,e.max-e.min)*100),0,100)}

function openCityBuild(id){
 const c=state.cities.find(x=>x.id===id);if(!c)return;ensureV180CityData();
 showModal(`<p class="eyebrow">DEVELOP CITY • VERSION 1.8.0</p><h2>${c.symbol} ${c.name}</h2><p class="muted">Choose an immediate development or begin a major multi-year project.</p><div class="city-upgrade-grid">${v180CityUpgrades.map(u=>`<button class="city-upgrade-button" data-v180-city-upgrade="${u.id}"><strong>${u.icon} ${u.name}</strong><small>${u.cost} Gold • Level ${c.v180Upgrades[u.id]}<br>${u.text}</small></button>`).join("")}</div><div class="choice-row"><button id="v180-major-project" class="choice-button good">🏗️ Major City Project</button><button id="v180-enter-districts" class="choice-button bad">🏙️ Enter Districts</button></div>`);
 $$('[data-v180-city-upgrade]').forEach(b=>b.onclick=()=>performV180CityUpgrade(c,b.dataset.v180CityUpgrade));
 $('#v180-major-project').onclick=()=>chooseCityProject(c);
 $('#v180-enter-districts').onclick=()=>openLivingCity(c.id);
}
function performV180CityUpgrade(c,id){
 const u=v180CityUpgrades.find(x=>x.id===id);if(!u)return;
 const level=c.v180Upgrades[id]||0,cost=u.cost+level*8;if(!spend('gold',cost))return;
 const before={population:c.population,happiness:c.happiness,culture:c.culture,wealth:c.wealth,military:c.military,unrest:c.unrest,faith:state.faith,prestige:state.prestige};
 u.apply(c);c.v180Upgrades[id]=level+1;
 state.v180.cityUpgradeHistory.unshift({year:state.year,city:c.name,id,name:u.name,level:c.v180Upgrades[id]});state.v180.cityUpgradeHistory=state.v180.cityUpgradeHistory.slice(0,80);
 recordAfterAction({icon:u.icon,title:`Developed ${c.name}: ${u.name}`,summary:u.text,category:'cities',location:c.name,changes:[{label:'Upgrade Level',value:`${c.v180Upgrades[id]}`},{label:'Population',before:before.population,after:c.population},{label:'Happiness',before:before.happiness,after:c.happiness},{label:'Wealth',before:before.wealth,after:c.wealth}],severity:'success'});
 recordChronicleEvent({icon:u.icon,title:`${c.name} Development: ${u.name}`,text:`${c.name} completed level ${c.v180Upgrades[id]} of ${u.name}.`,category:'cities',severity:'success',location:c.name,tags:['cities','development']});
 closeModal();renderAll();saveGame(false);
}

function v180EarlyMarriageTurn(){ /* v2.1: retired — v150AutomaticMarriageTurn owns NPC marriage progression. */ }

function v180FastAgingTurn(){
 // Add one EXTRA year of childhood growth after the legacy +1 aging turns.
 (state.divineRelationships?.npcImmortalChildren||[]).forEach(c=>{c.age++;c.stage=c.age<3?'Divine Infant':c.age<8?'Divine Child':c.age<14?'Divine Youth':c.age<20?'Young God':'Mature Immortal';c.power+=rand(0,2);c.wisdom+=rand(0,2)});
 (state.divineRelationships?.npcDemigods||[]).forEach(d=>{d.age++;if(d.age>=8&&d.status==='Demigod Child')d.status='Young Demigod';if(d.age>=16&&d.status!=='Immortal')d.status='Adult Demigod';if(d.age>=16){d.fame+=rand(0,2);d.reputation+=rand(0,2)}});
 (state.divineFamily?.immortalChildren||[]).forEach(c=>{c.age++;const st=[...immortalChildStages].reverse().find(x=>c.age>=x.minAge);if(st)c.stage=st.id;c.power+=rand(0,2);c.wisdom+=rand(0,1)});
 (state.demigods||[]).forEach(d=>{if(d.status!=='Immortal')d.age++});
 syncAutonomousDivineFamilies();syncV160Campers();syncV170Identity();
}

function auditV180Systems(silent=false){
 const health={
  core:!!state.olympusCore,
  family:!!state.divineFamily&&!!state.divineRelationships,
  houses:Array.isArray(state.divineRelationships?.houses),
  camp:!!state.v160&&!!state.v160.campers,
  heirs:!!state.v170,
  avatar:!!state.avatar,
  cities:state.cities.every(c=>c.v180Upgrades),
  chronicle:!!state.livingChronicle,
  alerts:Array.isArray(state.alerts),
  favor:typeof state.favor==='number'
 };
 state.v180.systemHealth=health;state.v180.lastAuditYear=state.year;
 if(state.olympusCore)runOlympusIntegrityRepair(true);
 syncV160Campers();syncV160Bloodlines();syncV170Identity();checkV170Milestones();
 if(!silent)addActivity('⚙️','Olympus Systems Audit',Object.values(health).every(Boolean)?'All major systems are connected and operating.':'The game repaired and refreshed connected systems.',Object.values(health).every(Boolean)?'success':'normal');
 return health;
}
function v180SystemPolishTurn(){
 const era=getV180Era();
 // Early era emphasizes city growth, later eras strengthen corresponding systems without new duplicate databases.
 if(era.id==='founding'&&Math.random()<.35){const c=randomItem(state.cities);c.population+=rand(4,12);c.happiness=clamp(c.happiness+rand(0,3))}
 if(era.id==='rising'&&Math.random()<.25){const houses=state.divineRelationships?.houses||[];if(houses.length){const h=randomItem(houses);h.influence=clamp((h.influence||50)+2);h.unity=clamp((h.unity||70)+1)}}
 if(era.id==='heroic'&&Math.random()<.25){const h=randomItem((state.heroes||[]).filter(x=>x.recruited));if(h){h.xp+=15;h.reputation=(h.reputation||0)+2}}
 if(era.id==='dynastic'&&Math.random()<.2)stage8FactionTurn();
 if(state.year-state.v180.lastAuditYear>=2)auditV180Systems(true);
}
function v180YearTurn(){v180EarlyMarriageTurn();v180FastAgingTurn();v180SystemPolishTurn()}

function renderV180Overview(){
 if(!document.getElementById('systems-overview-view')||!state.v180)return;
 const era=getV180Era(),health=auditV180Systems(true),ok=Object.values(health).filter(Boolean).length,total=Object.keys(health).length;
 $('#era-name-pill').textContent=`${era.icon} ${era.name}`;
 $('#systems-overview-summary').innerHTML=`<p class="eyebrow">ERA PACING</p><h3>${era.icon} ${era.name} • Year ${state.year}</h3><p>${era.text}</p><div class="era-progress"><span style="width:${v180EraProgress()}%"></span></div><div class="stats"><div class="stat"><strong>${ok}/${total}</strong><small>Systems Healthy</small></div><div class="stat"><strong>${state.divineRelationships?.houses?.length||0}</strong><small>Divine Houses</small></div><div class="stat"><strong>${Object.keys(state.v160?.campers||{}).length}</strong><small>Campers</small></div></div>`;
 $('#systems-overview-list').innerHTML=Object.entries(health).map(([k,v])=>`<article class="card systems-health-card ${v?'good':'warn'}"><div class="card-row"><div><h3>${v?'✅':'⚠️'} ${k.replace(/\b\w/g,x=>x.toUpperCase())}</h3><p>${v?'Connected and operating':'Repair needed'}</p></div><span class="tag gold">${v?'ONLINE':'CHECK'}</span></div></article>`).join('')+`<button class="card-button" id="v180-run-audit">⚙️ Run Full Systems Audit</button>`;
 const btn=$('#v180-run-audit');if(btn)btn.onclick=()=>{auditV180Systems(false);renderAll();saveGame(false)};
}
function renderV180Extras(){renderV180Overview()}

function ensureV170Data(){
 if(!state.v170)state.v170={pendingBirths:[],familyMilestones:[],usedNames:[],birthHistory:[],graduations:[],version:"1.7.0"};
 const v=state.v170;
 ["pendingBirths","familyMilestones","usedNames","birthHistory","graduations"].forEach(k=>{if(!v[k])v[k]=[]});
 patchV170Navigation();syncV170UsedNames();syncV170Identity();checkV170Milestones();
 const queued=[...(state.v170.pendingBirths||[])];queued.forEach(b=>{const npc=/Autonomous|Founding|NPC/i.test(b.source||"")&&b.parentA!==getGod().name&&b.parentB!==getGod().name;if(npc)finalizeV170Birth(b.id,v170SuggestedNames(1)[0],true)});
 setTimeout(()=>showNextV170BirthNaming(),300);
}
function patchV170Navigation(){
 const map=window.__navCategoryMap;if(!map)return;
 if(map.divine&&!map.divine.items.some(x=>x[0]==="heirs-view"))map.divine.items.push(["heirs-view","👶","Heirs","Children, titles & milestones"]);
}
function syncV170UsedNames(){
 const names=new Set();
 Object.keys(state.olympusCore?.people||{}).forEach(n=>names.add(n.trim().toLowerCase()));
 (state.v170.birthHistory||[]).forEach(b=>b.name&&names.add(b.name.trim().toLowerCase()));
 state.v170.usedNames=[...names];
}
function v170NameAvailable(name){
 const clean=(name||"").trim();if(clean.length<2)return false;syncV170UsedNames();
 return !state.v170.usedNames.includes(clean.toLowerCase());
}
function v170SuggestedNames(count=6){
 syncV170UsedNames();
 const unused=v170GreekNamePool.filter(n=>!state.v170.usedNames.includes(n.toLowerCase())).sort(()=>Math.random()-.5);
 const result=unused.slice(0,count);
 while(result.length<count){
  const c=`Theon ${rand(10,999)}`;
  if(v170NameAvailable(c)&&!result.includes(c))result.push(c);
 }
 return result;
}
function v170InheritedIdentity(parentA,parentB,type){
 const domains=[...new Set([...(v160ParentDomains(parentA)||[]),...(v160ParentDomains(parentB)||[])])].slice(0,4);
 const gift=v160BloodlineGift(parentA||"",parentB||"",domains);
 return {
  domains,gift,
  temperament:randomItem(["Bold","Curious","Compassionate","Proud","Patient","Ambitious","Loyal","Rebellious","Clever","Gentle"]),
  talent:randomItem(["Strategy","Combat","Healing","Prophecy","Leadership","Crafting","Survival","Charisma","Monster Lore","Divine Power"]),
  weakness:randomItem(["Pride","Impulsiveness","Jealousy","Fear of Failure","Recklessness","Stubbornness","Naivety","Isolation"]),
  potential:clamp((type==="Immortal Child"?65:50)+domains.length*4+rand(-5,15)),
  icon:type==="Immortal Child"?randomItem(["✨","🌟","⚡","🌙","🔥","🌊","🦉","🌸"]):"👤"
 };
}
function queueV170Birth(config){
 const identity=v170InheritedIdentity(config.parentA,config.parentB||config.mortalParent,config.type);
 const pending={id:uid("pendingBirth"),createdYear:state.year,type:config.type,parentA:config.parentA,parentB:config.parentB||null,mortalParent:config.mortalParent||null,city:config.city||"Mount Olympus",house:config.house||null,domain:config.domain||identity.domains[0]||"Divine Power",identity,source:config.source||"Birth",originalData:config.originalData||{}};
 state.v170.pendingBirths.push(pending);saveGame(false);
 const autonomous=/Autonomous|Founding|NPC/i.test(pending.source||"") && pending.parentA!==getGod().name && pending.parentB!==getGod().name;
 if(autonomous){const autoName=v170SuggestedNames(1)[0];setTimeout(()=>finalizeV170Birth(pending.id,autoName,true),20)}
 else setTimeout(()=>showNextV170BirthNaming(),50);
 return pending;
}
function showNextV170BirthNaming(){
 if(document.querySelector(".birth-name-overlay"))return;
 const b=state.v170?.pendingBirths?.[0];if(!b)return;
 const suggestions=v170SuggestedNames(6);
 const o=document.createElement("div");o.className="birth-name-overlay";
 o.innerHTML=`<div class="birth-name-panel"><div class="birth-name-icon">${b.identity.icon}</div><p class="eyebrow">${b.type==="Immortal Child"?"IMMORTAL BIRTH":"DEMIGOD BIRTH"} • YEAR ${state.year}</p><h2>Name the Child</h2><p>${b.parentA}${b.parentB?` & ${b.parentB}`:""}${b.mortalParent?` & mortal ${b.mortalParent}`:""}</p><div class="birth-detail-grid"><div class="birth-detail"><strong>${b.house||"Divine Bloodline"}</strong><br>House</div><div class="birth-detail"><strong>${b.identity.domains.join(" • ")||b.domain}</strong><br>Inherited Domains</div><div class="birth-detail"><strong>${b.identity.gift.icon} ${b.identity.gift.name}</strong><br>Bloodline Gift</div><div class="birth-detail"><strong>${b.identity.temperament}</strong><br>Temperament</div><div class="birth-detail"><strong>${b.identity.talent}</strong><br>Natural Talent</div><div class="birth-detail"><strong>${b.identity.potential}</strong><br>Divine Potential</div></div><input id="birth-name-input" class="birth-name-input" maxlength="24" placeholder="Enter a unique name"><div class="birth-suggestion-row">${suggestions.map(n=>`<button class="birth-suggestion" data-birth-suggestion="${n}">${n}</button>`).join("")}</div><p id="birth-name-error" class="muted"></p><div class="choice-row"><button id="birth-generate-more" class="choice-button bad">🎲 More Names</button><button id="birth-confirm-name" class="choice-button good">✨ Name Child</button></div></div>`;
 document.body.appendChild(o);const input=$("#birth-name-input");input.focus();
 $$("[data-birth-suggestion]").forEach(btn=>btn.onclick=()=>{input.value=btn.dataset.birthSuggestion;$("#birth-name-error").textContent=""});
 $("#birth-generate-more").onclick=()=>{o.remove();showNextV170BirthNaming()};
 $("#birth-confirm-name").onclick=()=>{
  const name=input.value.trim();
  if(!v170NameAvailable(name)){$("#birth-name-error").textContent=name.length<2?"Enter at least 2 characters.":"That name already exists in Olympus history. Choose another.";return}
  o.remove();finalizeV170Birth(b.id,name);
 };
}
function v170SiblingReactions(childName,parentA,parentB){
 const core=state.olympusCore;
 const siblings=Object.values(core?.people||{}).filter(p=>p.name!==childName&&p.parents?.some(x=>x===parentA||x===parentB));
 const reactions=[];
 siblings.forEach(s=>{
  const reaction=randomItem(["Excited","Protective","Jealous","Curious","Indifferent","Threatened"]);
  s.siblingReactions=s.siblingReactions||{};s.siblingReactions[childName]={reaction,year:state.year};reactions.push({name:s.name,reaction});
 });
 return reactions;
}
function finalizeV170Birth(id,name,automatic=false){
 const idx=state.v170.pendingBirths.findIndex(x=>x.id===id);if(idx<0)return;
 const b=state.v170.pendingBirths[idx],identity=b.identity;
 if(b.type==="Immortal Child"){
  const child={id:uid("npcImmortalChild"),name,icon:identity.icon,parentA:b.parentA,parentB:b.parentB,domain:b.domain,age:0,stage:"Divine Infant",power:rand(20,42),wisdom:rand(20,42),loyalty:65,house:b.house||`House of ${b.parentA} & ${b.parentB}`,memories:[`Born and named in Year ${state.year}.`],inheritedDomains:identity.domains,bloodlineGift:identity.gift,temperament:identity.temperament,naturalTalent:identity.talent,weakness:identity.weakness,divinePotential:identity.potential};
  state.divineRelationships.npcImmortalChildren.push(child);stage11Person(name,{icon:child.icon,type:"Immortal Child",status:"Divine Infant"});stage11LinkParentChild(b.parentA,name);if(b.parentB)stage11LinkParentChild(b.parentB,name);
 }else{
  const child={id:uid("npcDemigod"),name,icon:"👤",divineParent:b.parentA,mortalParent:b.mortalParent||b.parentB,city:b.city,age:0,status:"Demigod Child",fame:0,reputation:10,corruption:0,domainPotential:b.domain||identity.domains[0]||"Divine Power",memories:[`Born and named in ${b.city} in Year ${state.year}.`],inheritedDomains:identity.domains,bloodlineGift:identity.gift,temperament:identity.temperament,naturalTalent:identity.talent,weakness:identity.weakness,divinePotential:identity.potential};
  state.divineRelationships.npcDemigods.push(child);stage11Person(name,{icon:"👤",type:"Demigod",status:"Demigod Child"});stage11LinkParentChild(b.parentA,name);
 }
 runV150FamilySync();syncV160Campers();syncV160Bloodlines();syncV170Identity();
 const reactions=v170SiblingReactions(name,b.parentA,b.parentB||b.mortalParent);
 state.v170.birthHistory.unshift({year:state.year,name,type:b.type,parentA:b.parentA,parentB:b.parentB||b.mortalParent,house:b.house,identity,reactions});
 state.v170.pendingBirths.splice(idx,1);syncV170UsedNames();
 addActivity(identity.icon,`${b.type==="Immortal Child"?"Immortal":"Demigod"} Child Named: ${name}`,`${name} has entered the world with ${identity.gift.name}.`,"success");
 recordChronicleEvent({icon:identity.icon,title:`Birth of ${name}`,text:`${name}, child of ${b.parentA}${b.parentB?` and ${b.parentB}`:b.mortalParent?` and mortal ${b.mortalParent}`:""}, ${automatic?"was named by the family":"was named by the player"}. Inherited domains: ${identity.domains.join(", ")}.`,category:"births",severity:b.type==="Immortal Child"?"mythic":"success",major:b.type==="Immortal Child",actor:b.parentA,target:name,location:b.city,tags:["divine-family","bloodline","births"]});
 checkV170Milestones();renderAll();saveGame(false);setTimeout(()=>showNextV170BirthNaming(),150);
}
function syncV170Identity(){
 const core=state.olympusCore;if(!core)return;
 Object.values(core.people).forEach(p=>{
  if(!["Immortal Child","Demigod","Ascended Immortal"].includes(p.type))return;
  if(!p.inheritedDomains)p.inheritedDomains=[...new Set((p.parents||[]).flatMap(v160ParentDomains))].slice(0,4);
  if(!p.bloodlineGift)p.bloodlineGift=v160BloodlineGift(p.parents?.[0]||"",p.parents?.[1]||"",p.inheritedDomains);
  if(!p.temperament)p.temperament=randomItem(["Bold","Curious","Compassionate","Proud","Patient","Ambitious","Loyal","Rebellious"]);
  if(!p.naturalTalent)p.naturalTalent=randomItem(["Strategy","Combat","Healing","Prophecy","Leadership","Crafting","Survival","Charisma"]);
  if(!p.weakness)p.weakness=randomItem(["Pride","Impulsiveness","Jealousy","Recklessness","Stubbornness","Naivety"]);
  if(!p.divinePotential)p.divinePotential=clamp(50+(p.parents?.length||0)*8+(p.inheritedDomains?.length||0)*4+rand(-5,12));
  v170AssignEpithet(p);
 });
}
function v170AssignEpithet(p){
 if(p.epithet)return;
 const hero=(state.heroes||[]).find(h=>h.name===p.name),camper=state.v160?.campers?.[p.name];
 let score=(p.divinePotential||0)+(hero?.reputation||0)+(camper?.campGlory||0);if(score<105)return;
 const pool=[
  ["the Lionhearted",p.naturalTalent==="Combat"||p.temperament==="Bold"],
  ["the Far-Seer",p.naturalTalent==="Prophecy"||p.inheritedDomains?.includes("Prophecy")],
  ["the Oathkeeper",p.temperament==="Loyal"||p.temperament==="Patient"],
  ["the Storm-Born",p.inheritedDomains?.some(d=>/Storm|Sky/.test(d))],
  ["the Golden",p.divinePotential>=90],
  ["the Wise",p.naturalTalent==="Strategy"],
  ["the Flame-Touched",p.inheritedDomains?.some(d=>/Fire|Forge/.test(d))]
 ];
 p.epithet=(pool.find(x=>x[1])||["the Rising",true])[0];
 recordChronicleEvent({icon:p.icon||"✨",title:`Epithet Earned: ${p.name} ${p.epithet}`,text:`The deeds and inherited gifts of ${p.name} have earned a permanent epithet.`,category:"legends",severity:"mythic",major:true,target:p.name,tags:["bloodline","legends"]});
}
function v170AssignCampRoles(){
 Object.values(state.v160?.campers||{}).forEach(c=>{
  const best=v170CampRoles.find(r=>c.campGlory>=r.minGlory&&c[r.skill]>=r.minSkill);
  if(best&&c.campRole!==best.name){
   c.campRole=best.name;c.campRoleIcon=best.icon;c.memories.unshift(`Year ${state.year}: Became ${best.name} at Camp Elysium.`);
   recordChronicleEvent({icon:best.icon,title:`Camp Role: ${c.name} — ${best.name}`,text:`${c.name} earned a leadership role at Camp Elysium.`,category:"heroes",severity:"success",target:c.name,location:"Camp Elysium",tags:["camp","demigods"]});
  }
 });
}
function v170GraduationTurn(){
 Object.values(state.v160?.campers||{}).forEach(c=>{
  if(c.graduated)return;const d=v160AllDemigods().find(x=>x.name===c.name);if(!d||d.age<18||c.campGlory<75||c.quests<1)return;
  c.graduated=true;c.status="Camp Graduate";state.v170.graduations.unshift({year:state.year,name:c.name,lodge:c.lodge,glory:c.campGlory,quests:c.quests,warGames:c.warGames});
  let hero=(state.heroes||[]).find(h=>h.name===c.name);
  if(!hero){hero={id:uid("hero"),name:c.name,portrait:c.lodgeIcon,location:d.city||"Camp Elysium",recruited:true,level:1,xp:0,reputation:Math.max(20,d.reputation||0),fame:Math.max(15,d.fame||0),skills:[],memories:[`Graduated Camp Elysium in Year ${state.year}.`]};state.heroes.push(hero)}else hero.recruited=true;
  recordChronicleEvent({icon:"🏆",title:`Camp Graduation: ${c.name}`,text:`${c.name} graduated from ${c.lodge} after ${c.quests} quests and ${c.warGames} War Games. New status: Hero Candidate.`,category:"heroes",severity:"mythic",major:true,target:c.name,location:"Camp Elysium",tags:["camp","heroes","demigods"]});
  addActivity("🏆",`Camp Graduation: ${c.name}`,`${c.name} is now a Hero Candidate.`,"success");
 });
}
function v170MilestoneExists(key){return state.v170.familyMilestones.some(m=>m.key===key)}
function v170AddMilestone(key,title,text,icon="🏛️"){
 if(v170MilestoneExists(key))return;state.v170.familyMilestones.unshift({key,title,text,icon,year:state.year});
 recordChronicleEvent({icon,title:`Family Milestone: ${title}`,text,category:"legends",severity:"mythic",major:true,tags:["bloodline","dynasty","family"]});
}
function checkV170Milestones(){
 Object.values(state.olympusCore?.houses||{}).forEach(h=>{
  const all=[...(h.children||[]),...(h.demigods||[])];
  if((h.children||[]).length>=1)v170AddMilestone(`${h.name}-first-immortal`,`First Immortal Child of ${h.name}`,`${h.name} welcomed its first immortal child.`,"✨");
  if((h.demigods||[]).length>=1)v170AddMilestone(`${h.name}-first-demigod`,`First Demigod Branch of ${h.name}`,`${h.name} established its first mortal-born demigod branch.`,"👶");
  if(all.length>=10)v170AddMilestone(`${h.name}-ten`,`Ten Descendants of ${h.name}`,`${h.name} has grown to at least ten descendants.`,"🌳");
  if(all.some(n=>(state.heroes||[]).some(hero=>hero.name===n&&hero.recruited)))v170AddMilestone(`${h.name}-hero`,`First Hero of ${h.name}`,`${h.name} produced its first recognized hero.`,"🏆");
  if(all.some(n=>state.olympusCore.people[n]?.type==="Ascended Immortal"))v170AddMilestone(`${h.name}-ascended`,`Ascended Descendant of ${h.name}`,`A descendant of ${h.name} achieved full immortality.`,"🌟");
 });
}
function v170YearTurn(){syncV170Identity();v170AssignCampRoles();v170GraduationTurn();checkV170Milestones();setTimeout(()=>showNextV170BirthNaming(),100)}
function renderV170Heirs(){
 if(!document.getElementById("heirs-view")||!state.v170)return;
 syncV170Identity();checkV170Milestones();
 const heirs=Object.values(state.olympusCore?.people||{}).filter(p=>["Immortal Child","Demigod","Ascended Immortal"].includes(p.type));
 $("#heirs-count").textContent=`${heirs.length} Descendants`;
 $("#heirs-summary").innerHTML=`<p class="eyebrow">GENERATIONAL IDENTITY</p><h3>${heirs.length} descendants carry inherited domains, talents, weaknesses, gifts and earned titles</h3><div class="stats"><div class="stat"><strong>${state.v170.birthHistory.length}</strong><small>Named Births</small></div><div class="stat"><strong>${heirs.filter(p=>p.epithet).length}</strong><small>Earned Epithets</small></div><div class="stat"><strong>${state.v170.graduations.length}</strong><small>Camp Graduates</small></div></div>`;
 $("#heirs-list").innerHTML=heirs.length?heirs.map(p=>{const camper=state.v160?.campers?.[p.name];const reactions=p.siblingReactions?Object.entries(p.siblingReactions).slice(0,4):[];return `<article class="card heir-card"><div class="card-row"><div class="council-head"><span class="relationship-avatar">${p.icon||"✨"}</span><div><h3>${p.name} ${p.epithet?`<span class="epithet">${p.epithet}</span>`:""}</h3><p>${p.type} • ${p.house||"No House"}</p></div></div><span class="tag gold">Potential ${p.divinePotential||50}</span></div><div class="heir-grid"><div class="heir-stat"><strong>${(p.parents||[]).join(" & ")||"Unknown"}</strong><small>Parents</small></div><div class="heir-stat"><strong>${(p.inheritedDomains||[]).join(" • ")||"Awakening"}</strong><small>Domains</small></div><div class="heir-stat"><strong>${p.temperament||"Developing"}</strong><small>Temperament</small></div><div class="heir-stat"><strong>${p.naturalTalent||"Unknown"}</strong><small>Talent</small></div><div class="heir-stat"><strong>${p.weakness||"Unknown"}</strong><small>Weakness</small></div><div class="heir-stat"><strong>${camper?.campRole||"None"}</strong><small>Camp Role</small></div></div><div class="bloodline-gift"><strong>${p.bloodlineGift?.icon||"✨"} ${p.bloodlineGift?.name||"Divine Spark"}</strong><br>${p.bloodlineGift?.desc||""}</div>${reactions.length?`<div class="sibling-reaction"><strong>Sibling Reactions</strong><br>${reactions.map(([n,r])=>`${n}: ${r.reaction}`).join(" • ")}</div>`:""}</article>`}).join(""):'<article class="card"><p>No descendants yet.</p></article>';
 $("#family-milestone-list").innerHTML=state.v170.familyMilestones.length?state.v170.familyMilestones.map(m=>`<article class="card milestone-card"><div class="card-row"><div><h3>${m.icon} ${m.title}</h3><p>Year ${m.year}</p></div><span class="tag gold">Achieved</span></div><p>${m.text}</p></article>`).join(""):'<article class="card"><h3>No milestones yet</h3><p>As Divine Houses grow, their first children, heroes, ascensions, and large descendant counts will become permanent achievements.</p></article>';
}
function renderV170Extras(){renderV170Heirs()}

function ensureV160Data(){
 if(!state.v160)state.v160={campers:{},campEvents:[],lodgeGlory:{},warGameWins:{},bloodlines:{},lastWarGameYear:0,lastOracleYear:0,version:"1.6.0"};
 const v=state.v160;
 if(!v.campers)v.campers={};if(!v.campEvents)v.campEvents=[];if(!v.lodgeGlory)v.lodgeGlory={};if(!v.warGameWins)v.warGameWins={};if(!v.bloodlines)v.bloodlines={};
 patchV160Navigation();
 syncV160Campers();
 syncV160Bloodlines();
}

function patchV160Navigation(){
 const map=window.__navCategoryMap;if(!map)return;
 if(map.divine&&!map.divine.items.some(x=>x[0]==="bloodlines-view"))map.divine.items.push(["bloodlines-view","🧬","Bloodlines","Inheritance & Great Houses"]);
 if(map.heroes&&!map.heroes.items.some(x=>x[0]==="camp-elysium-view"))map.heroes.items.unshift(["camp-elysium-view","🏕️","Camp Elysium","Interactive demigod sanctuary"]);
}

function v160AllDemigods(){
 const map=new Map();
 (state.demigods||[]).forEach(d=>map.set(d.name,d));
 (state.divineRelationships?.npcDemigods||[]).forEach(d=>{if(!map.has(d.name))map.set(d.name,d)});
 return [...map.values()];
}

function v160DivineParentOf(d){
 if(d.divineParent)return d.divineParent;
 const core=state.olympusCore?.people?.[d.name];
 if(core){
  const divine=core.parents.find(p=>{
   const t=state.olympusCore.people[p]?.type||"";
   return /Olympian|Titan|Titaness|Immortal|God/i.test(t);
  });
...
function v160LodgeFor(parent){
 const def=v160LodgeDefinitions[parent];
 if(def)return {...def,parent};
 return {name:`${parent} Lodge`,icon:"🏛️",focus:"Divine Heritage & Heroic Training",parent};
}

function syncV160Campers(){
 const v=state.v160;
 v160AllDemigods().forEach(d=>{
  const parent=v160DivineParentOf(d),lodge=v160LodgeFor(parent);
  if(!v.campers[d.name]){
   v.campers[d.name]={
    name:d.name,demigodId:d.id,divineParent:parent,lodge:lodge.name,lodgeIcon:lodge.icon,
    swordsmanship:rand(15,35),archery:rand(15,35),strategy:rand(15,35),divinePower:rand(15,35),
    survival:rand(15,35),athletics:rand(15,35),monsterLore:rand(10,30),leadership:rand(10,30),
    campGlory:0,quests:0,warGames:0,status:"Camper",friends:[],rivals:[],memories:[`Joined Camp Elysium in Year ${state.year}.`]
   };
  }
  const c=v.campers[d.name];
  c.divineParent=parent;c.lodge=lodge.name;c.lodgeIcon=lodge.icon;
  d.campLodge=lodge.name;d.campStatus=c.status;d.campGlory=c.campGlory;
  if(v.lodgeGlory[lodge.name]===undefined)v.lodgeGlory[lodge.name]=0;
  if(v.warGameWins[lodge.name]===undefined)v.warGameWins[lodge.name]=0;
 });
}

function v160ParentDomains(parent){
 const roster=(state.divineRelationships?.roster||[]).find(r=>r.name===parent);
 const profile=getStage8Profile?.(parent);
 const base={
  Zeus:["Storms","Sky","Kingship"],Poseidon:["Sea","Tides","Earth"],Athena:["Wisdom","Strategy","Craft"],
  Ares:["War","Courage","Battle"],Aphrodite:["Love","Beauty","Desire"],Hades:["Underworld","Shadows","Wealth"],
  Apollo:["Light","Prophecy","Healing"],Artemis:["Moon","Hunt","Wilds"],Hermes:["Travel","Messages","Trade"],
  Hephaestus:["Forge","Fire","Invention"],Hera:["Family","Queenship","Marriage"],Hecate:["Magic","Crossroads","Night"],
  Prometheus:["Fire","Forethought","Mortal Progress"],Kronos:["Time","Harvest","Ancient Rule"]
 };
 return base[parent]||[profile?.ambition?.includes("prophecy")?"Prophecy":"Divine Power",roster?.type==="Titan"?"Titan Blood":"Olympian Blood"];
}

function v160BloodlineGift(parentA,parentB,domains=[]){
 const joined=[parentA,parentB,...domains].join(" ").toLowerCase();
 if(joined.includes("sea")&&joined.includes("storm"))return {name:"Tempest Blood",icon:"🌩️",desc:"Exceptional command of storms and sea."};
 if(joined.includes("wisdom")&&joined.includes("prophecy"))return {name:"Radiant Foresight",icon:"🔮",desc:"Rare flashes of strategic prophecy."};
 if(joined.includes("war")&&joined.includes("love"))return {name:"Heart of Battle",icon:"❤️‍🔥",desc:"Charisma and courage intensify under pressure."};
 if(joined.includes("underworld")&&joined.includes("moon"))return {name:"Veilwalker",icon:"🌘",desc:"Natural affinity for spirits, shadows, and night."};
 if(joined.includes("fire")&&joined.includes("craft"))return {name:"Sacred Forgeheart",icon:"🔥",desc:"Instinctive mastery of divine crafting."};
 if(joined.includes("titan"))return {name:"Ancient Blood",icon:"⛓️",desc:"Unusually strong endurance inherited from Titan ancestry."};
 return {name:"Divine Spark",icon:"✨",desc:"A balanced inheritance of immortal potential."};
}

function syncV160Bloodlines(){
 const core=state.olympusCore;if(!core)return;
 const v=state.v160;
 v.bloodlines={};

 Object.values(core.houses||{}).forEach(h=>{
  const descendants=[...(h.children||[]),...(h.demigods||[])];
  const founders=h.founders||[];
  const domains=[...new Set(founders.flatMap(v160ParentDomains))];
  const gift=v160BloodlineGift(founders[0]||"",founders[1]||"",domains);
  const artifacts=(state.divineRule?.dynamicArtifacts||[]).filter(a=>a.owners?.some(o=>founders.includes(o))).length;
  const heroCount=descendants.filter(n=>(state.heroes||[]).some(hero=>hero.name===n&&hero.recruited)).length;
  const prestige=Math.round((h.influence||50)+(h.unity||70)*.4+descendants.length*6+heroCount*10+artifacts*8);
  v.bloodlines[h.name]={
   house:h.name,founders:[...founders],descendants,domains,gift,
   prestige,influence:h.influence||50,unity:h.unity||70,wealth:Math.round(30+descendants.length*5+prestige*.15),
   authority:Math.round(prestige*.55+(h.influence||50)*.45),
   legacy:prestige>=130?"Great House":prestige>=90?"Rising Dynasty":"Young Bloodline"
  };
 });

 // Individual descendants inherit a compact profile from actual Core parents.
 Object.values(core.people).forEach(p=>{
  if(!["Immortal Child","Demigod","Ascended Immortal"].includes(p.type))return;
  const parents=p.parents||[],domains=[...new Set(parents.flatMap(v160ParentDomains))];
  p.inheritedDomains=domains.slice(0,4);
  p.bloodlineGift=v160BloodlineGift(parents[0]||"",parents[1]||"",domains);
  p.divinePotential=p.divinePotential||clamp(45+parents.length*8+domains.length*4+rand(-5,12));
 });
}

function v160TrainCamper(name,skill){
 const c=state.v160.campers[name];if(!c)return;
 const amount=rand(4,8);
 if(c[skill]===undefined)return;
 c[skill]=clamp(c[skill]+amount);c.campGlory+=2;c.memories.unshift(`Year ${state.year}: Trained ${skill} at Camp Elysium.`);
 state.v160.lodgeGlory[c.lodge]=(state.v160.lodgeGlory[c.lodge]||0)+1;
 const d=v160AllDemigods().find(x=>x.name===name);if(d){d.fame=(d.fame||0)+1;d.reputation=(d.reputation||0)+1}
 recordAfterAction({icon:c.lodgeIcon,title:`Camp Training: ${name}`,summary:`${name} improved ${skill} at Camp Elysium.`,category:"heroes",target:name,location:"Camp Elysium",changes:[{label:skill,value:`${c[skill]}`,tone:"positive"},{label:"Camp Glory",value:`${c.campGlory}`},{label:"Lodge",value:c.lodge}],severity:"success"});
 renderAll();saveGame(false);
}

function v160SpeakToCamper(name){
 const c=state.v160.campers[name];if(!c)return;
 showModal(`<p class="eyebrow">CAMP CONVERSATION</p><h2>${c.lodgeIcon} ${c.name}</h2><p>${c.divineParent}'s child • ${c.lodge}</p><div class="camp-action-grid"><button class="camp-action" data-camp-talk="encourage">❤️ Encourage</button><button class="camp-action" data-camp-talk="challenge">⚔️ Challenge</button><button class="camp-action" data-camp-talk="family">🌳 Discuss Family</button><button class="camp-action" data-camp-talk="quest">🗺️ Ask About Quests</button></div>`);
 $$("[data-camp-talk]").forEach(b=>b.onclick=()=>{
  const a=b.dataset.campTalk;
  if(a==="encourage"){c.leadership=clamp(c.leadership+4);c.campGlory+=2}
  if(a==="challenge"){c.athletics=clamp(c.athletics+3);c.swordsmanship=clamp(c.swordsmanship+3)}
  if(a==="family"){const d=v160AllDemigods().find(x=>x.name===name);if(d)d.familyTrust=clamp((d.familyTrust||50)+5)}
  if(a==="quest")c.monsterLore=clamp(c.monsterLore+3);
  c.memories.unshift(`Year ${state.year}: Spoke with ${getGod().name} about ${a}.`);
  closeModal();renderAll();saveGame(false);
 });
}

function v160CampLocationAction(id){
 const loc=v160CampLocations.find(x=>x.id===id);if(!loc)return;
 const campers=Object.values(state.v160.campers);if(!campers.length){showToast("No demigods are currently at Camp Elysium.");return}
 if(id==="arena"){campers.forEach(c=>{c.swordsmanship=clamp(c.swordsmanship+2);c.athletics=clamp(c.athletics+2)})}
 if(id==="forge"){campers.forEach(c=>c.divinePower=clamp(c.divinePower+1));state.gold+=5}
 if(id==="woods"){campers.forEach(c=>{c.survival=clamp(c.survival+2);c.monsterLore=clamp(c.monsterLore+2)})}
 if(id==="lake"){campers.forEach(c=>c.leadership=clamp(c.leadership+1))}
 if(id==="stables"){campers.forEach(c=>c.athletics=clamp(c.athletics+2))}
 if(id==="healing"){campers.forEach(c=>c.status="Camper")}
 if(id==="pavilion"){Object.keys(state.v160.lodgeGlory).forEach(l=>state.v160.lodgeGlory[l]+=1)}
 if(id==="oracle"){v160OracleQuest()}
 state.v160.campEvents.unshift({id:uid("campEvent"),year:state.year,icon:loc.icon,title:`Visited ${loc.name}`,text:`Camp Elysium held activities at ${loc.name}.`,type:id});
 addActivity(loc.icon,`Camp Elysium: ${loc.name}`,loc.desc,"success");renderAll();saveGame(false);
}

function v160WarGames(){
 const lodges=v160LodgeStats().filter(l=>l.campers.length);if(lodges.length<2){showToast("At least two lodges need campers for War Games.");return}
 const [a,b]=lodges.slice().sort(()=>Math.random()-.5).slice(0,2);
 const power=l=>l.campers.reduce((s,n)=>{const c=state.v160.campers[n];return s+c.swordsmanship+c.strategy+c.athletics+c.leadership},0)/Math.max(1,l.campers.length);
 const pa=power(a)+Math.random()*60,pb=power(b)+Math.random()*60,winner=pa>=pb?a:b,loser=winner===a?b:a;
 state.v160.lodgeGlory[winner.name]=(state.v160.lodgeGlory[winner.name]||0)+15;
 state.v160.warGameWins[winner.name]=(state.v160.warGameWins[winner.name]||0)+1;
 winner.campers.forEach(n=>{const c=state.v160.campers[n];c.campGlory+=6;c.warGames++;c.strategy=clamp(c.strategy+2)});
 loser.campers.forEach(n=>{const c=state.v160.campers[n];c.campGlory+=2;c.warGames++});
 const text=`${winner.name} defeated ${loser.name} in the Camp Elysium War Games.`;
 state.v160.campEvents.unshift({id:uid("campEvent"),year:state.year,icon:"⚔️",title:"Camp War Games",text,type:"war-game",winner:winner.name});
 recordChronicleEvent({icon:"⚔️",title:`War Games: ${winner.name} Victorious`,text,category:"heroes",severity:"mythic",location:"Camp Elysium",tags:["camp","demigods"]});
 recordAfterAction({icon:"⚔️",title:`War Games Winner: ${winner.name}`,summary:text,category:"heroes",location:"Camp Elysium",changes:[{label:"Lodge Glory",value:`${state.v160.lodgeGlory[winner.name]}`,tone:"positive"},{label:"War Game Wins",value:`${state.v160.warGameWins[winner.name]}`},{label:"Defeated",value:loser.name}],severity:"mythic"});
 renderAll();saveGame(false);
}

function v160OracleQuest(){
 const campers=Object.values(state.v160.campers);if(!campers.length)return;
 const party=[...campers].sort((a,b)=>(b.campGlory+b.monsterLore)-(a.campGlory+a.monsterLore)).slice(0,Math.min(3,campers.length));
 const monster=randomItem(monsterTemplates||[{name:"Hydra"},{name:"Minotaur"},{name:"Chimera"}]);
 const city=randomItem(state.cities).name;
 const title=`Oracle Quest: ${monster.name} near ${city}`;
 party.forEach(c=>{c.quests++;c.monsterLore=clamp(c.monsterLore+2);c.memories.unshift(`Year ${state.year}: Chosen for ${title}.`)});
 state.v160.campEvents.unshift({id:uid("campQuest"),year:state.year,icon:"🔮",title,text:`The Oracle chose ${party.map(c=>c.name).join(", ")} to face ${monster.name} near ${city}.`,type:"oracle",party:party.map(c=>c.name)});
 state.sideQuests.push({id:uid("side"),city,title,rewardGold:80,rewardXP:65,status:"Available",campParty:party.map(c=>c.name)});
 recordChronicleEvent({icon:"🔮",title,text:`Campers chosen: ${party.map(c=>c.name).join(", ")}.`,category:"heroes",severity:"mythic",location:"Camp Elysium",tags:["camp","prophecy","heroes"]});
}

function v160LodgeStats(){
 const groups={};
 Object.values(state.v160.campers).forEach(c=>{
  if(!groups[c.lodge])groups[c.lodge]={name:c.lodge,icon:c.lodgeIcon,parent:c.divineParent,campers:[],glory:state.v160.lodgeGlory[c.lodge]||0,wins:state.v160.warGameWins[c.lodge]||0};
  groups[c.lodge].campers.push(c.name);
 });
 return Object.values(groups).sort((a,b)=>(b.glory+b.wins*10)-(a.glory+a.wins*10));
}

function v160CampYearTurn(){
 syncV160Campers();
 const campers=Object.values(state.v160.campers);
 campers.forEach(c=>{
  if(Math.random()<.25)c[Math.random()<.5?"strategy":"monsterLore"]=clamp(c[Math.random()<.5?"strategy":"monsterLore"]+1);
  if(c.campGlory>=60)c.status="Senior Camper";
  if(c.campGlory>=100)c.status="Camp Champion";
 });
 if(state.year-state.v160.lastWarGameYear>=3&&Math.random()<.35){state.v160.lastWarGameYear=state.year;v160WarGames()}
 if(state.year-state.v160.lastOracleYear>=3&&Math.random()<.25){state.v160.lastOracleYear=state.year;v160OracleQuest()}
}

function v160BloodlineYearTurn(){
 syncV160Bloodlines();
 Object.values(state.v160.bloodlines).forEach(b=>{
  if(b.unity<35&&Math.random()<.08){
   recordChronicleEvent({icon:"⚔️",title:`House Feud: ${b.house}`,text:`Low unity has created a succession dispute inside ${b.house}.`,category:"divine-family",severity:"danger",major:true,tags:["bloodline","dynasty"]});
  }
 });
}

function v160YearTurn(){v160CampYearTurn();v160BloodlineYearTurn();syncV160Campers();syncV160Bloodlines()}

function renderV160Bloodlines(){
 if(!document.getElementById("bloodlines-view")||!state.v160)return;
 syncV160Bloodlines();
 const bloodlines=Object.values(state.v160.bloodlines).sort((a,b)=>b.prestige-a.prestige);
 $("#bloodline-count").textContent=`${bloodlines.length} Bloodlines`;
 $("#bloodline-summary").innerHTML=`<p class="eyebrow">DIVINE INHERITANCE</p><h3>Great Houses now pass domains, gifts, prestige and political legacy through generations</h3><div class="stats"><div class="stat"><strong>${bloodlines.length}</strong><small>Divine Houses</small></div><div class="stat"><strong>${Object.values(state.olympusCore?.people||{}).filter(p=>p.bloodlineGift).length}</strong><small>Gifted Descendants</small></div><div class="stat"><strong>${bloodlines.filter(b=>b.legacy==="Great House").length}</strong><small>Great Houses</small></div></div>`;
 $("#great-house-ranking").innerHTML=bloodlines.length?bloodlines.map((b,i)=>`<article class="card great-house-card"><div class="card-row"><div><p class="eyebrow">#${i+1} • ${b.legacy.toUpperCase()}</p><h3>🏛️ ${b.house}</h3><p>Founders: ${b.founders.join(" & ")}</p></div><span class="tag gold">${b.prestige} Prestige</span></div><div class="house-rank-grid"><div class="house-rank-stat"><strong>${b.influence}</strong><small>Influence</small></div><div class="house-rank-stat"><strong>${b.unity}</strong><small>Unity</small></div><div class="house-rank-stat"><strong>${b.authority}</strong><small>Authority</small></div><div class="house-rank-stat"><strong>${b.wealth}</strong><small>Wealth</small></div><div class="house-rank-stat"><strong>${b.descendants.length}</strong><small>Descendants</small></div></div><div class="bloodline-gift"><strong>${b.gift.icon} ${b.gift.name}</strong><br>${b.gift.desc}</div></article>`).join(""):'<article class="card"><h3>No dynasties yet</h3><p>Autonomous divine marriages will establish Great Houses over time.</p></article>';

 const descendants=Object.values(state.olympusCore?.people||{}).filter(p=>["Immortal Child","Demigod","Ascended Immortal"].includes(p.type));
 $("#bloodline-descendant-list").innerHTML=descendants.length?descendants.map(p=>`<article class="card bloodline-card"><div class="card-row"><div class="council-head"><span class="relationship-avatar">${p.icon||"✨"}</span><div><h3>${p.name}</h3><p>${p.type} • ${p.house||"No House"}</p></div></div><span class="tag gold">Potential ${p.divinePotential||50}</span></div><p>Parents: ${(p.parents||[]).join(" & ")||"Unknown"}<br>Inherited Domains: ${(p.inheritedDomains||[]).join(" • ")||"Awakening"}</p><div class="bloodline-gift"><strong>${p.bloodlineGift?.icon||"✨"} ${p.bloodlineGift?.name||"Divine Spark"}</strong><br>${p.bloodlineGift?.desc||"Divine potential is still developing."}</div></article>`).join(""):'<article class="card"><p>No descendants are ready for bloodline analysis.</p></article>';
}

function renderV160Camp(){
 if(!document.getElementById("camp-elysium-view")||!state.v160)return;
 syncV160Campers();
 const campers=Object.values(state.v160.campers),lodges=v160LodgeStats();
 $("#camp-count").textContent=`${campers.length} Campers`;
 $("#camp-summary").innerHTML=`<p class="eyebrow">DEMIGOD SANCTUARY</p><h3>${campers.length} demigods live and train across ${lodges.length} divine lodges</h3><p>Every camper's lodge is determined automatically by their real divine parent in Olympus Core.</p><div class="choice-row"><button class="choice-button good" onclick="v160WarGames()">⚔️ Hold War Games</button><button class="choice-button bad" onclick="v160OracleQuest();renderAll();saveGame(false)">🔮 Consult Oracle</button></div>`;

 $("#camp-lodge-list").innerHTML=lodges.length?lodges.map(l=>`<article class="card camp-lodge-card"><div class="card-row"><div><p class="eyebrow">${l.parent.toUpperCase()} BLOODLINE</p><h3 class="lodge-banner">${l.icon} ${l.name}</h3><p>${v160LodgeFor(l.parent).focus}</p></div><span class="tag gold">${l.glory} Glory</span></div><div class="lodge-grid"><div class="lodge-stat"><strong>${l.campers.length}</strong><small>Campers</small></div><div class="lodge-stat"><strong>${l.wins}</strong><small>War Game Wins</small></div><div class="lodge-stat"><strong>${l.parent}</strong><small>Divine Parent</small></div></div><p class="muted">${l.campers.join(", ")}</p></article>`).join(""):'<article class="card"><p>No demigods have arrived at camp yet.</p></article>';

 $("#camp-location-list").innerHTML=v160CampLocations.map(l=>`<article class="card camp-location-card"><div class="card-row"><div class="council-head"><span class="camp-map-icon">${l.icon}</span><div><h3>${l.name}</h3><p>${l.desc}</p></div></div></div><button class="card-button" data-camp-location="${l.id}">Visit ${l.name}</button></article>`).join("");
 $$("[data-camp-location]").forEach(b=>b.onclick=()=>v160CampLocationAction(b.dataset.campLocation));

 $("#camp-camper-list").innerHTML=campers.length?campers.map(c=>`<article class="card camper-card"><div class="card-row"><div><h3>${c.lodgeIcon} ${c.name}</h3><p>Child of ${c.divineParent} • ${c.lodge}</p></div><div>${c.campRole?`<span class="camp-role-badge">${c.campRoleIcon||"🏅"} ${c.campRole}</span>`:""} <span class="tag gold">${c.status}</span></div></div><div class="camper-grid"><div class="camper-stat"><strong>${c.campGlory}</strong><small>Camp Glory</small></div><div class="camper-stat"><strong>${c.quests}</strong><small>Quests</small></div><div class="camper-stat"><strong>${c.warGames}</strong><small>War Games</small></div></div><div class="camp-skill-row"><div class="camp-skill"><strong>${c.swordsmanship}</strong>Sword</div><div class="camp-skill"><strong>${c.archery}</strong>Archery</div><div class="camp-skill"><strong>${c.strategy}</strong>Strategy</div><div class="camp-skill"><strong>${c.divinePower}</strong>Power</div><div class="camp-skill"><strong>${c.survival}</strong>Survival</div><div class="camp-skill"><strong>${c.athletics}</strong>Athletics</div><div class="camp-skill"><strong>${c.monsterLore}</strong>Monster Lore</div><div class="camp-skill"><strong>${c.leadership}</strong>Leadership</div></div><div class="camp-action-grid"><button class="camp-action" data-train-camper="${c.name}" data-train-skill="swordsmanship">⚔️ Sword Training</button><button class="camp-action" data-train-camper="${c.name}" data-train-skill="strategy">🦉 Strategy</button><button class="camp-action" data-train-camper="${c.name}" data-train-skill="divinePower">✨ Divine Power</button><button class="camp-action" data-talk-camper="${c.name}">💬 Speak</button></div></article>`).join(""):'<article class="card"><h3>No campers yet</h3><p>Every new demigod will automatically be assigned a lodge when they enter Camp Elysium.</p></article>';
 $$("[data-train-camper]").forEach(b=>b.onclick=()=>v160TrainCamper(b.dataset.trainCamper,b.dataset.trainSkill));
 $$("[data-talk-camper]").forEach(b=>b.onclick=()=>v160SpeakToCamper(b.dataset.talkCamper));

 $("#camp-event-list").innerHTML=state.v160.campEvents.length?state.v160.campEvents.slice(0,15).map(e=>`<article class="card camp-event-card ${e.type||""}"><div class="card-row"><div><h3>${e.icon} ${e.title}</h3><p>Year ${e.year}</p></div></div><p>${e.text}</p></article>`).join(""):'<article class="card"><h3>Camp Chronicle Awaiting Stories</h3><p>War Games, Oracle quests, training, and camp events will be recorded here.</p></article>';
}

function renderV160Extras(){renderV160Bloodlines();renderV160Camp()}

function ensureV150Data(){
 if(!state.v150)state.v150={socialStories:[],lastGatheringYear:0,retiredMatchmaking:true,version:"1.5.0"};
 if(!state.v150.socialStories)state.v150.socialStories=[];
 state.v150.retiredMatchmaking=true;
 retireMatchmakingV150();
 runV150FamilySync();
}

function retireMatchmakingV150(){
 const map=window.__navCategoryMap;
 if(map?.divine)map.divine.items=map.divine.items.filter(x=>x[0]!=="matchmaking-view");
 // Preserve old data for migration/history, but it is no longer an active gameplay system.
 (state.alerts||[]).forEach(a=>{
  if(a.targetType==="matchmaking"||a.title?.startsWith("Match Accepted:")||a.title?.startsWith("Wedding Awaiting:")){
   a.resolved=true;a.status="resolved";a.outcome="Retired by v1.5.0 autonomous family migration.";
  }
 });
 state.alerts=(state.alerts||[]).filter(a=>!a.resolved&&a.status!=="resolved"&&a.status!=="Resolved");
}

function runV150FamilySync(){
 // Import every durable old marriage once, then let Olympus Core own the truth.
 const durable=[];
 (state.divineRelationships?.npcMarriages||[]).filter(m=>m.status==="Married"||m.status==="Committed").forEach(m=>durable.push(m));
 (state.divineMatchmaking?.weddings||[]).forEach(w=>durable.push({...w,status:"Married"}));
 (state.unifiedOlympus?.npcMarriages||[]).filter(m=>m.status==="Married").forEach(m=>durable.push(m));
 const seen=new Set();
 durable.forEach(m=>{
  if(!m.a||!m.b)return;const key=stage11UnionKey(m.a,m.b);if(seen.has(key))return;seen.add(key);
  stage11CompleteMarriage(m.a,m.b,{year:m.year||state.year,type:m.type||"Divine Union",source:"v1.5 Autonomous Family Migration"});
 });
 runOlympusIntegrityRepair(true);
 syncAutonomousDivineFamilies();
 if(state.v160){syncV160Campers();syncV160Bloodlines()}
}

function v150RelationshipScore(a,b){
 let score=35+(a.affection||0)*.35+(b.affection||0)*.35+(a.trust||45)*.25+(b.trust||45)*.25+(a.attraction||40)*.2+(b.attraction||40)*.2;
 const pa=getStage8Profile?.(a.name),pb=getStage8Profile?.(b.name);
 if(pa&&pb&&pa.factionId&&pa.factionId===pb.factionId)score+=10;
 if((a.type==="Titan")!==(b.type==="Titan"))score-=Math.round((state.titanWar||50)/14);
 return score+Math.random()*20;
}

function v150AutomaticMarriageTurn(){
 const roster=(state.divineRelationships?.roster||[]).filter(r=>r.name!==getGod().name);
 let free=roster.filter(r=>!state.olympusCore?.people?.[r.name]?.spouse);
 if(free.length<2)return;
 // v2.1: relationships advance every year; a compatible unmarried pair normally forms a union.
 const attempts=state.year<=6?2:1;
 for(let attempt=0;attempt<attempts;attempt++){
  free=roster.filter(r=>!state.olympusCore?.people?.[r.name]?.spouse);if(free.length<2)break;
  const ranked=[];
  free.forEach(a=>free.forEach(b=>{if(a.name>=b.name)return;ranked.push({a,b,score:v150RelationshipScore(a,b)})}));
  ranked.sort((x,y)=>y.score-x.score);const pair=ranked[Math.min(attempt,ranked.length-1)];if(!pair)break;
  const {a,b}=pair;a.affection=clamp((a.affection||35)+rand(6,12));b.affection=clamp((b.affection||35)+rand(6,12));a.trust=clamp((a.trust||45)+rand(4,9));b.trust=clamp((b.trust||45)+rand(4,9));
  const mature=state.year>=2,threshold=state.year<=5?58:68;if(!mature||v150RelationshipScore(a,b)<threshold)continue;
  const type=(a.type==='Titan'||a.type==='Titaness'||b.type==='Titan'||b.type==='Titaness')?'Olympian-Titan Union':'Divine Union';
  stage11CompleteMarriage(a.name,b.name,{year:state.year,type,source:'v2.1 Living Pantheon'});
  state.divineRelationships.npcFamilyEvents.unshift({year:state.year,type:'Marriage',text:`${a.name} and ${b.name} married.`});
  addActivity('💍',`Divine Marriage: ${a.name} & ${b.name}`,'Their relationship became a permanent Divine House.','success');
  recordChronicleEvent({icon:'💍',title:`Divine Marriage: ${a.name} & ${b.name}`,text:`Their living relationship founded House of ${a.name} & ${b.name}.`,category:'marriages',severity:'mythic',major:true,actor:a.name,target:b.name,location:'Mount Olympus',tags:['living-pantheon','divine-family']});
 }
}
function v150AutomaticImmortalBirthTurn(){
 const unions=(state.olympusCore?.unions||[]).filter(u=>u.status==="Married");
 unions.forEach(u=>{
  if(Math.random()>.28)return;
  const existing=(state.divineRelationships.npcImmortalChildren||[]).filter(c=>stage11UnionKey(c.parentA,c.parentB)===stage11UnionKey(u.a,u.b));
  const queued=(state.v170?.pendingBirths||[]).filter(c=>c.type==="Immortal Child"&&stage11UnionKey(c.parentA,c.parentB)===stage11UnionKey(u.a,u.b));
  if(existing.length+queued.length>=3||Math.random()<existing.length*.16)return;
  const a=(state.divineRelationships.roster||[]).find(r=>r.name===u.a),b=(state.divineRelationships.roster||[]).find(r=>r.name===u.b);if(!a||!b)return;
  queueV170Birth({type:"Immortal Child",parentA:u.a,parentB:u.b,house:`House of ${u.a} & ${u.b}`,city:"Mount Olympus",domain:deriveAutonomousChildDomain(a,b),source:"Autonomous Divine Birth"});
 });
}
function v150AutomaticDemigodTurn(){
 const roster=(state.divineRelationships?.roster||[]).filter(r=>r.type!=="Titan"&&r.type!=="Titaness");
 roster.forEach(g=>{
  if(Math.random()>.045)return;
  const existing=(state.divineRelationships.npcDemigods||[]).filter(d=>d.divineParent===g.name).length;
  const queued=(state.v170?.pendingBirths||[]).filter(d=>d.type==="Demigod"&&d.parentA===g.name).length;
  if(existing+queued>=5||Math.random()<existing*.1)return;
  const mortal=randomItem(autonomousMortalRomanceNames),city=randomItem(state.cities).name;
  queueV170Birth({type:"Demigod",parentA:g.name,mortalParent:mortal,city,domain:randomItem(divineDomainPool),source:"Autonomous Demigod Birth"});
 });
}
function v150LivingOlympusGatheringTurn(){
 if(Math.random()>.32)return;
 const roster=state.divineRelationships?.roster||[];if(roster.length<2)return;
 const event=randomItem(v150GatheringTemplates),people=[...roster].sort(()=>Math.random()-.5).slice(0,Math.min(roster.length,rand(2,5)));
 const a=people[0],b=people[1];
 const outcomes=[
  `${a.name} and ${b.name} grew closer during the gathering.`,
  `${a.name} challenged ${b.name}, beginning a new rivalry.`,
  `${people.map(p=>p.name).join(", ")} debated the future of Olympus.`,
  `${a.name} publicly praised ${b.name}, improving their standing among the immortals.`,
  `Rumors spread through Olympus after ${a.name} and ${b.name} were seen speaking privately.`
 ];
 const text=randomItem(outcomes);
 if(text.includes("grew closer")||text.includes("praised")){a.affection=clamp((a.affection||0)+5);b.affection=clamp((b.affection||0)+5);a.trust=clamp((a.trust||45)+3);b.trust=clamp((b.trust||45)+3)}
 const story={id:uid("olympusSocial"),year:state.year,icon:event.icon,title:event.name,place:event.place,text,attendees:people.map(p=>p.name)};
 state.v150.socialStories.unshift(story);state.v150.socialStories=state.v150.socialStories.slice(0,60);
 addActivity(event.icon,event.name,text,"normal");
 if(Math.random()<.45)recordChronicleEvent({icon:event.icon,title:event.name,text,category:"olympus",severity:"normal",location:event.place,tags:["living-olympus","social"]});
}

function v150YearTurn(){
 v150AutomaticMarriageTurn();
 v150AutomaticImmortalBirthTurn();
 v150AutomaticDemigodTurn();
 v150LivingOlympusGatheringTurn();
 runV150FamilySync();
}

function renderV150SocialFeed(){
 const el=document.getElementById("olympus-social-feed");if(!el||!state.v150)return;
 const stories=state.v150.socialStories||[];
 const pill=document.getElementById("olympus-social-count");if(pill)pill.textContent=`${stories.length} Stories`;
 el.innerHTML=stories.length?stories.slice(0,12).map(s=>`<article class="card living-social-card"><div class="card-row"><div><h3>${s.icon} ${s.title}</h3><p>${s.place} • Year ${s.year}</p></div><span class="tag gold">${s.attendees.length} Attended</span></div><p>${s.text}</p><div class="dynasty-children">${s.attendees.map(n=>`<span class="dynasty-child-chip">✨ ${n}</span>`).join("")}</div></article>`).join(""):'<article class="card"><h3>Olympus is gathering...</h3><p>As the years advance, feasts, festivals, rivalries, rumors, alliances, and romances will be recorded here automatically.</p></article>';
}

function ensureV140Stage11Data(){
 if(!state.olympusCore)state.olympusCore={unions:[],people:{},houses:{},events:[],repairLog:[],lastRepairYear:0,version:11};
 const c=state.olympusCore;
 if(!c.unions)c.unions=[];if(!c.people)c.people={};if(!c.houses)c.houses={};if(!c.events)c.events=[];if(!c.repairLog)c.repairLog=[];
 patchStage11Navigation();
 migrateStage11Core();
 runOlympusIntegrityRepair(true);
 repairStage11MatchmakingState();
 bindStage11InteractionRouter();
}

function patchStage11Navigation(){
 const map=window.__navCategoryMap;if(!map)return;
 /* v1.9.0: technical Olympus Core page retired from normal navigation. */
}

function stage11Person(name,defaults={}){
 if(!name)return null;
 const core=state.olympusCore;
 if(!core.people[name])core.people[name]={name,icon:defaults.icon||"👤",type:defaults.type||"Unknown",spouse:null,parents:[],children:[],house:null,status:defaults.status||"Active"};
 const p=core.people[name];
 if(defaults.icon)p.icon=defaults.icon;
 if(defaults.type)p.type=defaults.type;
 if(defaults.status)p.status=defaults.status;
 return p;
}

function stage11UnionKey(a,b){return [a,b].sort().join("||")}

function stage11CreateOrGetUnion(a,b,meta={}){
 if(!a||!b||a===b)return null;
 const key=stage11UnionKey(a,b);
 let u=state.olympusCore.unions.find(x=>x.key===key);
 if(!u){
  u={id:uid("union"),key,a,b,status:meta.status||"Married",year:meta.year??state.year,type:meta.type||"Divine Union",source:meta.source||"Unified Core",history:[]};
  state.olympusCore.unions.push(u);
 }
 if(meta.status)u.status=meta.status;
 if(meta.type)u.type=meta.type;
 if(meta.source)u.source=meta.source;
 return u;
}

function stage11LinkParentChild(parent,child){
 if(!parent||!child||parent===child)return;
 const p=stage11Person(parent),c=stage11Person(child);
 if(!p.children.includes(child))p.children.push(child);
 if(!c.parents.includes(parent))c.parents.push(parent);
}

function migrateStage11Core(){
 const core=state.olympusCore;

 // Preserve already-completed Core marriages before rebuilding.
 // Older Stage 11 code cleared them here, which erased a wedding
 // immediately after stage11CompleteMarriage() created it.
 const preservedUnions=(core.unions||[])
  .filter(u=>u.status==="Married")
  .map(u=>({...u,history:[...(u.history||[])]}));

 core.unions=[];core.people={};core.houses={};

 preservedUnions.forEach(u=>stage11CreateOrGetUnion(u.a,u.b,{
  status:"Married",
  year:u.year,
  type:u.type,
  source:u.source||"Olympus Core"
 }));

// Build people from every known system.
 const player=stage11Person(getGod().name,{icon:stage9GetPlayerIcon?.()||getGod().icon||getGod().symbol||"⚡",type:"Olympian",status:"Player God"});

 (state.divineRelationships?.roster||[]).forEach(r=>stage11Person(r.name,{icon:r.icon||"✨",type:r.type||"Immortal",status:r.status||"Active"}));
 (state.divineFamily?.familyTree||[]).forEach(n=>{
  const p=stage11Person(n.name,{icon:n.icon||"✨",type:n.type||"Divine Kin"});
  (n.parents||[]).forEach(par=>stage11LinkParentChild(par,n.name));
  (n.children||[]).forEach(ch=>stage11LinkParentChild(n.name,ch));
  if(n.spouse)stage11CreateOrGetUnion(n.name,n.spouse,{status:"Married",source:"Family Tree"});
 });
 if(state.divineFamily?.spouse)stage11CreateOrGetUnion(getGod().name,state.divineFamily.spouse.name,{status:"Married",year:state.divineFamily.spouse.marriedYear,type:state.divineFamily.spouse.type==="Titan"?"Olympian-Titan Union":"Divine Union",source:"Player Marriage"});

 (state.divineMatchmaking?.weddings||[]).forEach(w=>stage11CreateOrGetUnion(w.a,w.b,{status:"Married",year:w.year,type:w.type,source:"Matchmaking"}));
 (state.divineRelationships?.npcMarriages||[]).filter(m=>m.status==="Married"||m.status==="Committed").forEach(m=>stage11CreateOrGetUnion(m.a,m.b,{status:"Married",year:m.year,type:m.type,source:"NPC Marriage"}));
 (state.unifiedOlympus?.npcMarriages||[]).filter(m=>m.status==="Married").forEach(m=>stage11CreateOrGetUnion(m.a,m.b,{status:"Married",year:m.year,type:m.type,source:"Stage 10 Canonical"}));

 (state.divineFamily?.immortalChildren||[]).forEach(c=>{
  stage11Person(c.name,{icon:c.icon||"✨",type:"Immortal Child",status:c.stage||"Immortal"});
  (c.parents||[getGod().name,state.divineFamily.spouse?.name]).filter(Boolean).forEach(p=>stage11LinkParentChild(p,c.name));
 });
 (state.demigods||[]).forEach(d=>{
  stage11Person(d.name,{icon:d.icon||"👤",type:"Demigod",status:d.status||"Demigod"});
  stage11LinkParentChild(d.divineParent||getGod().name,d.name);
  if(d.mortalParent)stage11LinkParentChild(d.mortalParent,d.name);
 });
 (state.divineRelationships?.npcImmortalChildren||[]).forEach(c=>{
  stage11Person(c.name,{icon:c.icon||"✨",type:"Immortal Child",status:c.stage||"Immortal"});
  stage11LinkParentChild(c.parentA,c.name);stage11LinkParentChild(c.parentB,c.name);
 });
 (state.divineRelationships?.npcDemigods||[]).forEach(d=>{
  stage11Person(d.name,{icon:d.icon||"👤",type:"Demigod",status:d.status||"Demigod"});
  stage11LinkParentChild(d.divineParent,d.name);if(d.mortalParent)stage11LinkParentChild(d.mortalParent,d.name);
 });

 // Union spouse pointers.
 core.unions.filter(u=>u.status==="Married").forEach(u=>{
  stage11Person(u.a).spouse=u.b;
  stage11Person(u.b).spouse=u.a;
 });

 // Build houses ONLY from active married unions.
 core.unions.filter(u=>u.status==="Married").forEach(u=>{
  const houseName=`House of ${u.a} & ${u.b}`;
  const children=Object.values(core.people).filter(p=>p.parents.includes(u.a)&&p.parents.includes(u.b)&&p.type==="Immortal Child").map(p=>p.name);
  const demigods=Object.values(core.people).filter(p=>p.type==="Demigod"&&(p.parents.includes(u.a)||p.parents.includes(u.b))).map(p=>p.name);
  core.houses[houseName]={name:houseName,founders:[u.a,u.b],type:u.type?.includes("Titan")?"Olympian-Titan House":"Divine House",children,demigods,unionId:u.id,influence:50+children.length*5+demigods.length*3,unity:70};
  [...children,...demigods].forEach(n=>stage11Person(n).house=houseName);
  stage11Person(u.a).house=houseName;stage11Person(u.b).house=houseName;
 });
}

function stage11PushCoreToLegacy(){
 const core=state.olympusCore;

 // Clear / rebuild duplicate marriage storages from core.
 state.divineRelationships.npcMarriages=core.unions
  .filter(u=>u.status==="Married"&&u.a!==getGod().name&&u.b!==getGod().name)
  .map(u=>({id:u.id,a:u.a,b:u.b,year:u.year,type:u.type,status:"Married"}));

 if(state.unifiedOlympus){
  state.unifiedOlympus.npcMarriages=core.unions
   .filter(u=>u.status==="Married"&&u.a!==getGod().name&&u.b!==getGod().name)
   .map(u=>({id:u.id,a:u.a,b:u.b,year:u.year,type:u.type,status:"Married",source:"Stage 11 Core"}));
 }

 // Relationships roster.
 (state.divineRelationships.roster||[]).forEach(r=>{
  const p=core.people[r.name];
  if(p?.spouse){r.status="Married";r.autonomousPartner=p.spouse}
  else if(r.status==="Married"||r.status==="Committed"){r.status="Acquaintance";r.autonomousPartner=null}
 });

 // Rebuild family tree spouse/children/parents from core.
 Object.values(core.people).forEach(p=>{
  let n=state.divineFamily.familyTree.find(x=>x.name===p.name);
  const familyEligible=["Olympian","Titan","Titaness","Immortal Child","Demigod","Ascended Immortal","Divine Kin"].includes(p.type)||p.name===getGod().name;
  if(!n&&familyEligible){
   n={id:uid("family"),name:p.name,icon:p.icon||"✨",type:p.type,relation:p.name===getGod().name?"Self":p.type==="Titan"?"Titan Kin":"Divine Kin",parents:[],spouse:null,children:[]};
   state.divineFamily.familyTree.push(n);
  }
  if(n){
   n.icon=p.icon||"✨";n.parents=[...p.parents];n.children=[...p.children];n.spouse=p.spouse;
  }
 });

 // Rebuild houses from core instead of old duplicate house logic.
 state.divineRelationships.houses=Object.values(core.houses).map(h=>({
  id:h.unionId,name:h.name,icon:"🏛️",founders:[...h.founders],type:h.type,children:[...h.children],demigods:[...h.demigods],influence:h.influence,unity:h.unity,history:[`Unified by Olympus Core in Year ${state.year}.`]
 }));

 // Keep matchmaking weddings aligned with core.
 if(state.divineMatchmaking){
  state.divineMatchmaking.weddings=core.unions
   .filter(u=>u.status==="Married"&&u.a!==getGod().name&&u.b!==getGod().name)
   .map(u=>({id:u.id,a:u.a,b:u.b,year:u.year,type:u.type}));
 }

 // Player family links.
 (state.demigods||[]).forEach(d=>{
  const p=core.people[d.name];if(!p)return;
  d.divineSiblings=core.people[getGod().name]?.children.filter(n=>n!==d.name)||[];
  d.divineStepParent=state.divineFamily.spouse?.name||null;
  d.divineHouse=p.house||null;
 });
 (state.divineFamily?.immortalChildren||[]).forEach(c=>{
  const p=core.people[c.name];if(!p)return;
  c.demigodSiblings=(core.people[getGod().name]?.children||[]).filter(n=>core.people[n]?.type==="Demigod");
  c.divineHouse=p.house||null;
 });
}

function runOlympusIntegrityRepair(initial=false){
 const before=JSON.stringify({
  unions:state.olympusCore?.unions?.length||0,
  tree:state.divineFamily?.familyTree?.length||0,
  houses:state.divineRelationships?.houses?.length||0
 });
 migrateStage11Core();
 stage11PushCoreToLegacy();
 const core=state.olympusCore;
 const issues=[];

 core.unions.filter(u=>u.status==="Married").forEach(u=>{
  if(core.people[u.a]?.spouse!==u.b)issues.push(`${u.a} spouse mismatch`);
  if(core.people[u.b]?.spouse!==u.a)issues.push(`${u.b} spouse mismatch`);
  const hn=`House of ${u.a} & ${u.b}`;
  if(!core.houses[hn])issues.push(`${hn} missing`);
 });
 Object.values(core.people).forEach(p=>{
  p.children.forEach(c=>{if(!core.people[c]?.parents.includes(p.name))issues.push(`${p.name}/${c} parent-child mismatch`)});
 });

 core.lastRepairYear=state.year;
 core.repairLog.unshift({year:state.year,issues:issues.length,text:issues.length?`Repair completed with ${issues.length} remaining issue(s).`:`All connected systems synchronized successfully.`});
 core.repairLog=core.repairLog.slice(0,30);

 if(!initial&&state.livingChronicle){
  recordChronicleEvent({icon:"🔧",title:"Olympus Core Integrity Check",text:issues.length?`The world core found ${issues.length} unresolved integrity issue(s).`:"Marriage, genealogy, houses, and character records were synchronized.",category:"legends",severity:issues.length?"danger":"success"});
 }
 return issues;
}

function stage11CompleteMarriage(a,b,meta={}){
 const year=meta.year??state.year;
 const type=meta.type||"Divine Union";

 // 1. Create the authoritative Core union.
 const u=stage11CreateOrGetUnion(a,b,{status:"Married",year,type,source:meta.source||"Matchmaking"});
 u.status="Married";u.year=year;u.type=type;u.history=u.history||[];
 u.history.unshift({year:state.year,text:"Marriage completed."});

 // 2. Persist the wedding BEFORE any integrity rebuild can run.
 state.divineMatchmaking=state.divineMatchmaking||{arrangedMatches:[],weddings:[],dynasties:[],lastChildYears:{},history:[]};
 state.divineMatchmaking.weddings=state.divineMatchmaking.weddings||[];
 let wedding=state.divineMatchmaking.weddings.find(x=>stage11UnionKey(x.a,x.b)===stage11UnionKey(a,b));
 if(!wedding){
  wedding={id:u.id,a,b,year,type};
  state.divineMatchmaking.weddings.unshift(wedding);
 }else{
  wedding.year=year;wedding.type=type;
 }

 (state.divineMatchmaking.arrangedMatches||[]).forEach(m=>{
  if(stage11UnionKey(m.a,m.b)===stage11UnionKey(a,b)){
   m.status="Married";m.accepted=true;m.weddingYear=year;
  }
 });

 // 3. Persist to NPC marriage storage BEFORE repair.
 state.divineRelationships.npcMarriages=state.divineRelationships.npcMarriages||[];
 let legacy=state.divineRelationships.npcMarriages.find(x=>stage11UnionKey(x.a,x.b)===stage11UnionKey(a,b));
 if(!legacy){
  state.divineRelationships.npcMarriages.unshift({id:u.id,a,b,year,type,status:"Married"});
 }else{
  legacy.status="Married";legacy.year=year;legacy.type=type;
 }

 // 4. Persist to Stage 9/10 compatibility storage BEFORE repair.
 if(state.unifiedOlympus){
  state.unifiedOlympus.npcMarriages=state.unifiedOlympus.npcMarriages||[];
  let old=state.unifiedOlympus.npcMarriages.find(x=>stage11UnionKey(x.a,x.b)===stage11UnionKey(a,b));
  if(!old){
   state.unifiedOlympus.npcMarriages.unshift({id:u.id,a,b,year,type,status:"Married",source:"Stage 11 Core"});
  }else{
   old.status="Married";old.year=year;old.type=type;old.source="Stage 11 Core";
  }
 }

 // 5. Persist spouse links into Family Tree BEFORE repair.
 [a,b].forEach((name,i)=>{
  const spouse=i===0?b:a;
  let node=state.divineFamily.familyTree.find(n=>n.name===name);
  const roster=(state.divineRelationships?.roster||[]).find(r=>r.name===name);
  if(!node){
   node={id:uid("family"),name,icon:roster?.icon||"✨",type:roster?.type||"Immortal",relation:roster?.type==="Titan"?"Titan Kin":"Divine Kin",parents:[],spouse,children:[]};
   state.divineFamily.familyTree.push(node);
  }
  node.spouse=spouse;
 });

 // 6. Relationship roster becomes Married immediately.
 (state.divineRelationships?.roster||[]).forEach(r=>{
  if(r.name===a){r.status="Married";r.autonomousPartner=b}
  if(r.name===b){r.status="Married";r.autonomousPartner=a}
 });

 // 7. Safe rebuild: the marriage now exists in every durable source.
 runOlympusIntegrityRepair(true);

 // 8. Move Match Accepted alerts to History.
 const affected=(state.alerts||[]).filter(alert=>
  alert.title?.includes("Match Accepted") &&
  alert.title.includes(a) &&
  alert.title.includes(b)
 );
 affected.forEach(alert=>{
  alert.resolved=true;alert.status="resolved";
  alert.outcome=`Wedding completed: ${a} & ${b}.`;
  alert.choice="Hold Divine Wedding";alert.resolvedYear=state.year;
  state.alertHistory.unshift(alert);
 });
 state.alerts=(state.alerts||[]).filter(alert=>!affected.includes(alert));
 state.alertHistory=state.alertHistory.slice(0,150);

 return u;
}
function stage11CancelBetrothal(a,b){
 const m=(state.divineMatchmaking?.arrangedMatches||[]).find(x=>(x.a===a&&x.b===b)||(x.a===b&&x.b===a));
 if(m)m.status="Cancelled";
 (state.alerts||[]).forEach(alert=>{
  if(alert.title?.includes("Match Accepted")&&alert.title.includes(a)&&alert.title.includes(b) && !alert.resolved){
   alert.resolved=true;alert.resolution=`Betrothal cancelled.`;alert.status="Resolved";
  }
 });
}

function stage11NormalizeAlerts(){
 (state.alerts||[]).forEach(a=>{
  if(a.resolved){a.status="Resolved";return}
  if(a.priority==="critical"||a.priority==="urgent")a.status="Requires Action";
  else a.status="New";
 });
}

function bindStage11InteractionRouter(){
 if(window.__stage11RouterBound)return;
 window.__stage11RouterBound=true;

 document.addEventListener("click",ev=>{
  const el=ev.target.closest("[data-stage11-action]");
  if(!el)return;
  const action=el.dataset.stage11Action;

  if(action==="repair-core"){runOlympusIntegrityRepair(false);renderAll();saveGame(false)}
  if(action==="hold-wedding"){holdStage10Wedding(el.dataset.matchId)}
  if(action==="cancel-betrothal"){stage11CancelBetrothal(el.dataset.a,el.dataset.b);renderAll();saveGame(false)}
 },true);
}

function stage11YearTurn(){
 stage11NormalizeAlerts();
 runOlympusIntegrityRepair(true);
}

function renderStage11Core(){
 if(!document.getElementById("system-core-view")||!state.olympusCore)return;
 const core=state.olympusCore;
 const issues=runOlympusIntegrityRepair(true);
 $("#core-integrity-pill").textContent=issues.length?`${issues.length} Issues`:"Synchronized";
 $("#core-summary").innerHTML=`<p class="eyebrow">UNIFIED WORLD CORE</p><h3>${Object.keys(core.people).length} characters • ${core.unions.filter(u=>u.status==="Married").length} marriages • ${Object.keys(core.houses).length} divine houses</h3><p>Marriage, genealogy, children, houses, relationships, and family-tree data rebuild from one shared Olympus Core.</p><button class="card-button" data-stage11-action="repair-core">🔧 Run Integrity Repair</button>`;
 $("#core-integrity-list").innerHTML=[
  {name:"Divine Unions",ok:core.unions.every(u=>u.status!=="Married"||(core.people[u.a]?.spouse===u.b&&core.people[u.b]?.spouse===u.a)),value:core.unions.filter(u=>u.status==="Married").length},
  {name:"Genealogy",ok:Object.values(core.people).every(p=>p.children.every(c=>core.people[c]?.parents.includes(p.name))),value:Object.values(core.people).reduce((s,p)=>s+p.children.length,0)},
  {name:"Divine Houses",ok:core.unions.filter(u=>u.status==="Married").every(u=>core.houses[`House of ${u.a} & ${u.b}`]),value:Object.keys(core.houses).length},
  {name:"Alerts",ok:true,value:(state.alerts||[]).filter(a=>!a.resolved).length},
  {name:"Chronicle",ok:!!state.livingChronicle,value:state.livingChronicle?.entries?.length||0},
  {name:"Living Souls",ok:!!state.livingSouls,value:state.livingSouls?.profiles?.length||0}
 ].map(x=>`<article class="card core-card ${x.ok?"ok":"warn"}"><div class="card-row"><div><h3>${x.ok?"✅":"⚠️"} ${x.name}</h3><p>${x.ok?"Operating normally":"Needs repair"}</p></div><span class="tag gold">${x.value}</span></div></article>`).join("")+
 (core.repairLog.slice(0,5).map(l=>`<div class="core-fix-log"><strong>Year ${l.year}</strong> — ${l.text}</div>`).join(""));
}

function renderStage11Extras(){renderStage11Core()}

function ensureV140Stage10Data(){
 if(!state.divineMatchmaking)state.divineMatchmaking={arrangedMatches:[],weddings:[],dynasties:[],lastChildYears:{},history:[]};
 const dm=state.divineMatchmaking;
 ["arrangedMatches","weddings","dynasties","history"].forEach(k=>{if(!dm[k])dm[k]=[]});
 if(!dm.lastChildYears)dm.lastChildYears={};ensureStage10MarriageRegistry();
 patchStage10Navigation();
 synchronizeUnifiedOlympus(true);
 stage10RepairAllMarriages();
 syncStage10Dynasties();
}

function patchStage10Navigation(){
 const map=window.__navCategoryMap;if(!map?.divine)return;
 map.divine.items=map.divine.items.filter(x=>x[0]!=="matchmaking-view");
}

function stage10EligibleImmortals(){
 if(state.olympusCore)runOlympusIntegrityRepair(true);
 return (state.divineRelationships?.roster||[]).filter(r=>r.name!==getGod().name&&!state.olympusCore?.people?.[r.name]?.spouse);
}

function stage10Compatibility(a,b){
 let score=50;
 score += Math.round(((a.trust||45)+(b.trust||45))/10);
 score += Math.round(((a.affection||0)+(b.affection||0))/12);
 score += Math.round(((a.attraction||50)+(b.attraction||50))/14);
 if(a.type==="Titan"&&b.type==="Titan")score+=8;
 if(a.type!=="Titan"&&b.type!=="Titan")score+=5;
 if((a.type==="Titan")!==(b.type==="Titan"))score-=Math.round((state.titanWar||50)/12);
 const pa=getStage8Profile?.(a.name),pb=getStage8Profile?.(b.name);
 if(pa&&pb){
  if(pa.factionId===pb.factionId)score+=8;
  if(pa.personality==="Jealous"||pb.personality==="Jealous")score-=5;
  const rivalry=state.livingSouls?.rivalries?.find(r=>(r.a===a.name&&r.b===b.name)||(r.a===b.name&&r.b===a.name));
  if(rivalry)score-=Math.round((rivalry.intensity||0)/5);
 }
 return clamp(score);
}

function openStage10Matchmaker(name){
 const a=(state.divineRelationships?.roster||[]).find(r=>r.name===name);if(!a)return;
 const options=stage10EligibleImmortals().filter(r=>r.name!==a.name);
 showModal(`<p class="eyebrow">ARRANGE DIVINE MATCH</p><h2>${a.icon} ${a.name}</h2><p class="muted">Choose another god or Titan to encourage a union.</p><div class="action-grid">${options.map(b=>{const c=stage10Compatibility(a,b);return `<button class="action-button" data-stage10-partner="${b.name}">${b.icon} ${b.name}<br><small>${b.type} • Compatibility ${c}</small></button>`}).join("")}</div>`);
 $$("[data-stage10-partner]").forEach(btn=>btn.onclick=()=>{closeModal();attemptStage10Match(a.name,btn.dataset.stage10Partner)});
}

function attemptStage10Match(aName,bName){
 const roster=state.divineRelationships?.roster||[];
 const a=roster.find(r=>r.name===aName),b=roster.find(r=>r.name===bName);if(!a||!b)return;
 synchronizeUnifiedOlympus(true);
 if(state.unifiedOlympus.masterPeople[a.name]?.spouse||state.unifiedOlympus.masterPeople[b.name]?.spouse){showToast("One of these immortals is already married.");return}

 const compatibility=stage10Compatibility(a,b);
 const diplomacy=(state.prestige||0)*.05+(state.favor||0)*.04;
 const factionBonus=getStage8Profile?.(a.name)?.factionId===getStage8Profile?.(b.name)?.factionId?8:0;
 const titanPenalty=(a.type==="Titan")!==(b.type==="Titan")?Math.round((state.titanWar||50)/10):0;
 const score=compatibility*.72+diplomacy+factionBonus-titanPenalty+Math.random()*22;
 const accepted=score>=60;

 const match={id:uid("arrangedMatch"),a:a.name,b:b.name,year:state.year,compatibility,score:Math.round(score),accepted,status:accepted?"Betrothed":"Rejected"};
 state.divineMatchmaking.arrangedMatches.unshift(match);

 if(accepted){
  a.affection=clamp((a.affection||0)+8);b.affection=clamp((b.affection||0)+8);
  a.trust=clamp((a.trust||45)+6);b.trust=clamp((b.trust||45)+6);
  createAlert({
   priority:"urgent",category:"family",icon:"💞",
   title:`Match Accepted: ${a.name} & ${b.name}`,
   description:`Both immortals accepted your proposed union. Complete the formal divine wedding to make the marriage permanent.`,
   location:"Mount Olympus",targetType:"matchmaking",targetId:match.id,source:"Divine Matchmaking",years:3,
   choices:[
    {id:"wedding",label:"💍 Hold Divine Wedding",cost:null,amount:0,effect:"holdWedding"},
    {id:"view",label:"💞 Open Matchmaking",cost:null,amount:0,effect:"openMatchmaking"},
    {id:"cancel",label:"💔 Cancel Betrothal",cost:null,amount:0,effect:"cancelBetrothal"}
  ...
  recordChronicleEvent({icon:"💞",title:`Divine Betrothal: ${a.name} & ${b.name}`,text:`The player god successfully arranged a betrothal between ${a.name} and ${b.name}.`,category:"marriages",severity:"mythic",major:true,actor:getGod().name,target:`${a.name} & ${b.name}`,tags:["gods-titans","divine-family"]});
  showStage10ProposalResult(a,b,true,match);
 } else {
  a.trust=clamp((a.trust||45)-2);b.trust=clamp((b.trust||45)-2);
  createAlert({priority:"important",category:"family",icon:"💔",title:`Match Rejected: ${a.name} & ${b.name}`,description:`The proposed union was rejected. Improve politics and relationships before trying again.`,location:"Mount Olympus",targetId:match.id,years:3});
  showStage10ProposalResult(a,b,false,match);
 }
 renderAll();saveGame(false);
}

function showStage10ProposalResult(a,b,accepted,match){
 let o=document.querySelector(".cinematic-overlay");if(o)o.remove();
 o=document.createElement("div");o.className="cinematic-overlay";
 o.innerHTML=`<div class="cinematic-panel ${accepted?"marriage-result-accepted":"marriage-result-rejected"}"><div class="cinematic-visual">${accepted?"💞":"💔"}</div><p class="eyebrow">DIVINE MATCHMAKING</p><h2>${accepted?"Betrothal Accepted!":"Match Rejected"}</h2><p class="muted">${a.name} + ${b.name} • Compatibility ${match.compatibility}</p>${accepted?`<button id="stage10-wedding-now" class="primary-button">💍 Hold Divine Wedding</button>`:""}<button id="stage10-close-result" class="primary-button">${accepted?"Decide Later":"Return"}</button></div>`;
 document.body.appendChild(o);
 if(accepted)$("#stage10-wedding-now").onclick=()=>{o.remove();holdStage10Wedding(match.id)};
 $("#stage10-close-result").onclick=()=>o.remove();
}

function holdStage10Wedding(matchId){
 const m=state.divineMatchmaking.arrangedMatches.find(x=>x.id===matchId);if(!m||!m.accepted)return;
 const roster=state.divineRelationships?.roster||[],a=roster.find(r=>r.name===m.a),b=roster.find(r=>r.name===m.b);if(!a||!b)return;

 const existing=state.olympusCore?.unions?.find(u=>u.status==="Married"&&(u.a===m.a||u.b===m.a||u.a===m.b||u.b===m.b));
 if(existing&&stage11UnionKey(existing.a,existing.b)!==stage11UnionKey(m.a,m.b)){showToast("One of the immortals is already married.");return}

 const type=(a.type==="Titan"||b.type==="Titan")?"Olympian-Titan Union":"Divine Union";
 stage11CompleteMarriage(m.a,m.b,{year:state.year,type,source:"Player Matchmaking"});

 if((a.type==="Titan")!==(b.type==="Titan")){
  state.titanWar=clamp((state.titanWar||50)-7);
  if(state.titanCouncil)state.titanCouncil.rebellionStrength=clamp((state.titanCouncil.rebellionStrength||50)-4);
 }
 state.prestige+=10;state.favor+=5;

 recordChronicleEvent({icon:"💍",title:`Divine Wedding: ${a.name} & ${b.name}`,text:`The unified Divine Union system permanently recorded this marriage and founded their Divine House.`,category:"marriages",severity:"mythic",major:true,actor:getGod().name,target:`${a.name} & ${b.name}`,location:"Mount Olympus",tags:["divine-family","gods-titans","marriages"]});
 addActivity("💍",`Wedding Complete: ${a.name} & ${b.name}`,`Their marriage, Divine House, Family Tree, relationships, and future children are now permanently connected through Olympus Core.`,"success");
 runOlympusIntegrityRepair(true);
 renderAll();saveGame(false);
}
function deriveStage10ChildDomain(a,b){
 if(typeof deriveAutonomousChildDomain==="function")return deriveAutonomousChildDomain(a,b);
 return randomItem(divineDomainPool||["Fate","War","Sea","Wisdom","Stars","Storms"]);
}

function stage10CreateImmortalChild(aName,bName){
 const roster=state.divineRelationships?.roster||[],a=roster.find(r=>r.name===aName),b=roster.find(r=>r.name===bName);if(!a||!b)return;
 const name=typeof randomUniqueAutonomousChildName==="function"?randomUniqueAutonomousChildName():`Theon ${rand(2,99)}`;
 const domain=deriveStage10ChildDomain(a,b);
 ...
 state.divineRelationships.npcImmortalChildren.push(child);

 // Canonical and tree links.
 stage9CanonicalPerson(child.name,{icon:child.icon,type:"Immortal Child",status:"Divine Infant"}).house=child.house;
 stage9LinkParentChild(a.name,child.name);stage9LinkParentChild(b.name,child.name);

 [a.name,b.name].forEach(parentName=>{
  let node=state.divineFamily.familyTree.find(n=>n.name===parentName);
  if(node&&!node.children.includes(child.name))node.children.push(child.name);
 });
 if(!state.divineFamily.familyTree.some(n=>n.name===child.name)){
  state.divineFamily.familyTree.push({id:uid("family"),name:child.name,icon:child.icon,type:"Immortal Child",relation:"Extended Divine Kin",parents:[a.name,b.name],spouse:null,children:[]});
 }

 const house=state.divineRelationships.houses.find(h=>h.name===child.house);
 if(house&&!house.children.includes(child.name))house.children.push(child.name);

 state.divineMatchmaking.lastChildYears[`${a.name}|${b.name}`]=state.year;
 state.divineMatchmaking.history.unshift({year:state.year,text:`${child.name}, immortal child of ${a.name} and ${b.name}, was born.`});
 if(state.livingChronicle?.campaignStats)state.livingChronicle.campaignStats.births++;
 recordChronicleEvent({icon:child.icon,title:`Immortal Birth: ${child.name}`,text:`Child of ${a.name} and ${b.name}, born into ${child.house} with affinity for ${domain}.`,category:"births",severity:"mythic",major:true,actor:a.name,target:child.name,location:"Mount Olympus",tags:["divine-family","gods-titans","births"]});
 createAlert({priority:"important",category:"family",icon:child.icon,title:`Immortal Child Born: ${child.name}`,description:`${a.name} and ${b.name} welcomed an immortal child of ${domain}. The child is fully linked to the Unified Family Tree and Divine House.`,location:"Mount Olympus",targetId:child.id,years:3});
 synchronizeUnifiedOlympus(false);
 stage10RepairAllMarriages();
 syncStage10Dynasties();
}

function stage10ChildbirthTurn(){
 (state.divineMatchmaking.weddings||[]).forEach(w=>{
  const key=`${w.a}|${w.b}`,last=state.divineMatchmaking.lastChildYears[key]??w.year;
  if(state.year-last<2)return;
  const existing=state.divineRelationships?.npcImmortalChildren?.filter(c=>(c.parentA===w.a&&c.parentB===w.b)||(c.parentA===w.b&&c.parentB===w.a)).length||0;
  const chance=Math.max(.05,.16-existing*.025);
  if(Math.random()<chance)stage10CreateImmortalChild(w.a,w.b);
 });
}

function syncStage10Dynasties(){
 const dm=state.divineMatchmaking;
 dm.dynasties=[];
 const coreHouses=state.olympusCore?.houses?Object.values(state.olympusCore.houses):[];
 coreHouses.forEach(h=>{
  const wedding=(dm.weddings||[]).find(w=>stage11UnionKey(w.a,w.b)===stage11UnionKey(h.founders[0],h.founders[1]));
  if(!wedding)return;
  dm.dynasties.push({id:h.unionId,house:h.name,founders:[...h.founders],type:h.type,children:[...(h.children||[])],influence:h.influence||50,unity:h.unity||70});
 });
}
function stage10MarriagePoliticsTurn(){
 if(Math.random()>.12)return;
 const betrothed=(state.divineMatchmaking.arrangedMatches||[]).filter(m=>m.status==="Betrothed");
 if(betrothed.length){
  const m=randomItem(betrothed);
  createAlert({priority:"important",category:"family",icon:"💍",title:`Wedding Awaiting: ${m.a} & ${m.b}`,description:`The betrothed immortals are waiting for a formal divine wedding.`,location:"Mount Olympus",targetId:m.id,years:2});
 }
}

function stage10YearTurn(){ /* v1.5.0: retired player matchmaking; autonomous families own marriage/birth simulation. */
}

function renderStage10Matchmaking(){
 if(!document.getElementById("matchmaking-view")||!state.divineMatchmaking)return;
 runOlympusIntegrityRepair(true);
 syncStage10Dynasties();
 const elig=stage10EligibleImmortals();
 $("#matchmaking-count").textContent=`${elig.length} Eligible`;
 $("#matchmaking-summary").innerHTML=`<p class="eyebrow">PLAYER-DIRECTED DYNASTIES</p><h3>Encourage gods and Titans to marry, then watch their immortal families grow through the years</h3><p>Marriage acceptance depends on compatibility, politics, Favor, Prestige, factions, Titan relations, and rivalries.</p>`;

 $("#matchmaking-list").innerHTML=elig.length?elig.map(a=>{
   const canon=state.olympusCore?.people?.[a.name]||state.unifiedOlympus?.masterPeople?.[a.name];
   const best=elig.filter(b=>b.name!==a.name).map(b=>({b,score:stage10Compatibility(a,b)})).sort((x,y)=>y.score-x.score)[0];
   return `<article class="card match-card ${a.type==="Titan"?"titan-match":""}"><div class="card-row"><div class="council-head"><span class="relationship-avatar">${a.icon}</span><div><h3>${a.name}</h3><p>${a.type} • ${a.personality||"Immortal"} • ${a.location||"Olympus"}</p></div></div><span class="tag gold">${canon?.spouse?`Spouse: ${canon.spouse}`:"Unmarried"}</span></div><div class="matchmaking-grid"><div class="match-stat"><strong>${a.affection||0}</strong><small>Affection</small></div><div class="match-stat"><strong>${a.trust||45}</strong><small>Trust</small></div><div class="match-stat"><strong>${best?best.score:"—"}</strong><small>Best Match</small></div></div><button class="match-action" data-arrange-match="${a.name}">💞 Arrange a Match</button></article>`;
 }).join(""):'<article class="card"><h3>No unmarried immortals are currently available</h3><p>New generations can eventually create more eligible immortals.</p></article>';
 $$("[data-arrange-match]").forEach(b=>b.onclick=()=>openStage10Matchmaker(b.dataset.arrangeMatch));

 const pending=(state.divineMatchmaking.arrangedMatches||[]).filter(m=>m.status==="Betrothed");
 const dyn=state.divineMatchmaking.dynasties||[];
 $("#immortal-dynasty-list").innerHTML=
   pending.map(m=>`<article class="card wedding-card"><div class="card-row"><div><h3>💍 ${m.a} & ${m.b}</h3><p>Betrothed • Compatibility ${m.compatibility}</p></div><span class="tag gold">Awaiting Wedding</span></div><div class="wedding-grid"><button class="match-action secondary" data-stage11-action="hold-wedding" data-match-id="${m.id}">💍 Hold Divine Wedding</button><button class="match-action" data-stage11-action="cancel-betrothal" data-a="${m.a}" data-b="${m.b}">Cancel Betrothal</button></div></article>`).join("")+
   (dyn.length?dyn.map(d=>`<article class="card dynasty-card"><div class="card-row"><div><h3>🏛️ ${d.house}</h3><p>${d.type} • Founders ${d.founders.join(" & ")}</p></div><span class="tag gold">${d.children.length} Children</span></div><div class="dynasty-grid"><div class="dynasty-stat"><strong>${d.influence}</strong><small>Influence</small></div><div class="dynasty-stat"><strong>${d.unity}</strong><small>Unity</small></div><div class="dynasty-stat"><strong>${d.children.length}</strong><small>Immortal Children</small></div></div><div class="dynasty-children">${d.children.map(c=>`<span class="dynasty-child-chip">✨ ${c}</span>`).join("")||'<span class="muted">No immortal children yet</span>'}</div></article>`).join(""):'<article class="card"><h3>No arranged immortal dynasties yet</h3><p>Successful divine weddings will appear here and grow through future generations.</p></article>');
 $$("[data-hold-wedding]").forEach(b=>b.onclick=()=>holdStage10Wedding(b.dataset.holdWedding));
}

function renderStage10Extras(){renderStage10Matchmaking()}

function ensureV140Stage9Data(){if(!state.unifiedOlympus)state.unifiedOlympus={favorDeeds:{},masterPeople:{},syncLog:[]};state.unifiedOlympus.favorDeeds||={};state.unifiedOlympus.masterPeople||={};state.unifiedOlympus.syncLog||=[];patchStage9Navigation();synchronizeUnifiedOlympus(true)}
function patchStage9Navigation(){const m=window.__navCategoryMap;if(m?.divine&&!m.divine.items.some(x=>x[0]==="divine-favor-view"))m.divine.items.unshift(["divine-favor-view","✨","Divine Favor","Earn sacred Favor"])}
function stage9GetPlayerIcon(){const g=getGod();return g?.icon||g?.symbol||g?.emoji||"⚡"}
function stage9Canonical(name,d={}){if(!name)return null;const m=state.unifiedOlympus.masterPeople;if(!m[name])m[name]={name,icon:d.icon||"👤",type:d.type||"Unknown",parents:[],children:[],spouse:null,house:null};const p=m[name];if(d.icon&&d.icon!=="undefined")p.icon=d.icon;if(d.type)p.type=d.type;return p}
function stage9Parent(parent,child){if(!parent||!child||parent===child)return;const p=stage9Canonical(parent),c=stage9Canonical(child);if(!p.children.includes(child))p.children.push(child);if(!c.parents.includes(parent))c.parents.push(parent)}
function stage9Spouse(a,b){if(!a||!b)return;stage9Canonical(a).spouse=b;stage9Canonical(b).spouse=a}
function synchronizeUnifiedOlympus(initial=false){const u=state.unifiedOlympus;u.masterPeople={};stage9Canonical(getGod().name,{icon:stage9GetPlayerIcon(),type:"Olympian"});(state.divineRelationships?.roster||[]).forEach(r=>stage9Canonical(r.name,{icon:r.icon||"✨",type:r.type||"Immortal"}));(state.divineFamily?.familyTree||[]).forEach(n=>{stage9Canonical(n.name,{icon:n.name===getGod().name?stage9GetPlayerIcon():(n.icon||"✨"),type:n.type||"Divine Kin"});(n.parents||[]).forEach(p=>stage9Parent(p,n.name));(n.children||[]).forEach(c=>stage9Parent(n.name,c));if(n.spouse)stage9Spouse(n.name,n.spouse)});if(state.divineFamily?.spouse)stage9Spouse(getGod().name,state.divineFamily.spouse.name);(state.divineFamily?.immortalChildren||[]).forEach(c=>{const p=stage9Canonical(c.name,{icon:c.icon||"✨",type:"Immortal Child"});(c.parents||[getGod().name,state.divineFamily.spouse?.name]).filter(Boolean).forEach(x=>stage9Parent(x,c.name));p.house=c.divineHouse||null});(state.demigods||[]).forEach(d=>{const p=stage9Canonical(d.name,{icon:d.icon||"👤",type:"Demigod"});stage9Parent(d.divineParent||getGod().name,d.name);if(d.mortalParent)stage9Parent(d.mortalParent,d.name);p.house=d.divineHouse||null});(state.divineRelationships?.npcImmortalChildren||[]).forEach(c=>{stage9Canonical(c.name,{icon:c.icon||"✨",type:"Immortal Child"});stage9Parent(c.parentA,c.name);stage9Parent(c.parentB,c.name)});(state.divineRelationships?.npcDemigods||[]).forEach(d=>{stage9Canonical(d.name,{icon:d.icon||"👤",type:"Demigod"});stage9Parent(d.divineParent,d.name);stage9Parent(d.mortalParent,d.name)});(state.divineRelationships?.houses||[]).forEach(h=>{(h.founders||[]).forEach(n=>stage9Canonical(n).house=h.name);(h.children||[]).forEach(n=>stage9Canonical(n).house=h.name);(h.demigods||[]).forEach(n=>stage9Canonical(n).house=h.name)});const all=Object.values(u.masterPeople),find=n=>state.divineFamily.familyTree.find(x=>x.name===n);all.forEach(p=>{let n=find(p.name);if(!n&&["Olympian","Titan","Titaness","Immortal Child","Demigod","Ascended Immortal"].includes(p.type)){n={id:uid("family"),name:p.name,icon:p.icon||"✨",type:p.type,relation:p.name===getGod().name?"Self":"Divine Kin",parents:[],spouse:null,children:[]};state.divineFamily.familyTree.push(n)}if(n){n.icon=p.name===getGod().name?stage9GetPlayerIcon():(p.icon||"✨");n.parents=[...p.parents];n.children=[...p.children];n.spouse=p.spouse}});const kids=u.masterPeople[getGod().name]?.children||[];(state.demigods||[]).forEach(d=>{d.divineSiblings=kids.filter(x=>x!==d.name);d.divineStepParent=state.divineFamily.spouse?.name||null});(state.divineFamily?.immortalChildren||[]).forEach(c=>c.demigodSiblings=kids.filter(x=>x!==c.name&&u.masterPeople[x]?.type==="Demigod"));(state.divineRelationships?.houses||[]).forEach(h=>{h.children=all.filter(p=>p.house===h.name&&p.type==="Immortal Child").map(p=>p.name);h.demigods=all.filter(p=>p.house===h.name&&p.type==="Demigod").map(p=>p.name)});if(!initial)u.syncLog.unshift({year:state.year,text:`Synchronized ${all.length} characters.`})}
function stage9FavorRank(){return [...stage9FavorRanks].reverse().find(r=>state.favor>=r.min)||stage9FavorRanks[0]}
function stage9FavorNextRank(){return stage9FavorRanks.find(r=>r.min>state.favor)||null}
function stage9FavorReady(d){const last=state.unifiedOlympus.favorDeeds[d.id]??-999;return state.year-last>=d.cooldown}
function useStage9FavorDeed(id){const d=stage9FavorDeeds.find(x=>x.id===id);if(!d||!stage9FavorReady(d))return;let bonus=0,summary=d.name;if(id==="family-duty"){const s=state.divineFamily?.spouse;if(s){s.affection=clamp((s.affection||60)+4);s.jealousy=clamp((s.jealousy||0)-4)};(state.demigods||[]).forEach(c=>c.familyTrust=clamp((c.familyTrust||50)+3));summary="You honored and strengthened the divine family."}if(id==="city-protection"){const c=randomItem(state.cities);c.happiness=clamp((c.happiness||50)+5);c.player=clamp((c.player||50)+4);summary=`You protected ${c.name}.`}if(id==="sacred-oath"){state.divineReputation.honorable=(state.divineReputation.honorable||0)+3;state.prestige+=4;summary="You upheld a sacred oath."}if(id==="titan-peace"){state.titanWar=clamp((state.titanWar||50)-6);summary="You advanced peace with the Titans."}if(id==="heroic-duty"){const h=randomItem((state.heroes||[]).filter(x=>x.recruited));if(h){h.reputation=(h.reputation||0)+5;summary=`You sponsored ${h.name}.`}}if(id==="council-duty"){state.prestige+=6;summary="You fulfilled a Council duty."}if(id==="prophecy-duty"){const p=state.fateSagas?.prophecies?.find(x=>x.status==="Unfulfilled");if(p){p.pressure+=1;summary=`You honored the prophecy surrounding ${p.subject}.`}}if(id==="answer-prayer"){const p=state.livingWorld?.prayers?.find(x=>x.status==="Pending");if(p){p.status="Answered";bonus=4;summary=`You answered ${p.citizen}'s prayer.`}}const before=state.favor;state.favor+=d.reward+bonus;state.unifiedOlympus.favorDeeds[d.id]=state.year;recordAfterAction({icon:d.icon,title:`Favor Earned: ${d.name}`,summary,category:"gods-titans",changes:[{label:"Favor",before,after:state.favor,tone:"positive"},{label:"Rank",value:stage9FavorRank().name}],severity:"success"});renderAll();saveGame(false)}
function renderDivineFavor(){if(!document.getElementById("divine-favor-view")||!state.unifiedOlympus)return;const r=stage9FavorRank(),n=stage9FavorNextRank(),pct=n?Math.min(100,Math.round((state.favor-r.min)/(n.min-r.min)*100)):100;$("#favor-rank-pill").textContent=`${r.icon} ${r.name}`;$("#favor-summary").innerHTML=`<p class="eyebrow">SACRED PROGRESSION</p><h3>${state.favor} Favor • ${r.name}</h3><p>Earn Favor through family, prayer, cities, prophecy, heroes, Council service, and Titan diplomacy.</p><div class="favor-progress"><div style="width:${pct}%"></div></div>`;$("#favor-deed-list").innerHTML=stage9FavorDeeds.map(d=>`<article class="card favor-deed-card ${stage9FavorReady(d)?"":"locked"}"><div class="card-row"><div><h3>${d.icon} ${d.name}</h3><p>Cooldown ${d.cooldown} year(s)</p></div><span class="tag gold">+${d.reward}</span></div><button class="favor-deed-button" data-favor-deed="${d.id}" ${stage9FavorReady(d)?"":"disabled"}>${stage9FavorReady(d)?"Perform Sacred Deed":"Cooling Down"}</button></article>`).join("");$$("[data-favor-deed]").forEach(b=>b.onclick=()=>useStage9FavorDeed(b.dataset.favorDeed));$("#favor-rank-list").innerHTML=stage9FavorRanks.map(x=>`<article class="card favor-rank-card ${r.name===x.name?"active":""}"><h3>${x.icon} ${x.name}</h3><p>${x.min} Favor required</p></article>`).join("")}
function stage9YearTurn(){synchronizeUnifiedOlympus(false);if((state.divineReputation?.honorable||0)>=50&&Math.random()<.15)state.favor+=2}
function renderStage9Extras(){renderDivineFavor()}

function ensureV140Stage8Data(){
 if(!state.livingSouls)state.livingSouls={filter:"all",profiles:[],factions:[],offices:[],messages:[],rivalries:[],legends:[]};const ls=state.livingSouls;["profiles","factions","offices","messages","rivalries","legends"].forEach(k=>{if(!ls[k])ls[k]=[]});if(!ls.factions.length)stage8FactionTemplates.forEach(f=>ls.factions.push({...f,members:[],influence:rand(25,55),leader:null,history:[]}));if(!ls.offices.length)stage8OfficeTemplates.forEach(o=>ls.offices.push({...o,holder:null,support:50,history:[]}));syncStage8Profiles();syncStage8Factions();patchStage8Navigation()
}
function patchStage8Navigation(){const map=window.__navCategoryMap;if(!map)return;if(map.divine&&!map.divine.items.some(x=>x[0]==="living-souls-view"))map.divine.items.push(["living-souls-view","🧠","Living Souls","Ambitions, memories & schemes"]);if(map.divine&&!map.divine.items.some(x=>x[0]==="factions-view"))map.divine.items.push(["factions-view","🕸️","Factions","Alliances & political blocs"]);if(map.divine&&!map.divine.items.some(x=>x[0]==="divine-offices-view"))map.divine.items.push(["divine-offices-view","👑","Divine Offices","Olympian appointments"]);if(map.divine&&!map.divine.items.some(x=>x[0]==="messages-view"))map.divine.items.push(["messages-view","📨","Messages","Private conversations"])}
function syncStage8Profiles(){const ls=state.livingSouls,seen=new Set();const add=(id,name,icon,type,ref,location="Greece")=>{if(!name||seen.has(name))return;seen.add(name);let p=ls.profiles.find(x=>x.name===name);if(!p){p={id:id||uid("soulProfile"),name,icon:icon||"👤",type,location,personality:randomItem(["Ambitious","Loyal","Proud","Compassionate","Cunning","Honorable","Jealous","Patient","Bold","Curious"]),ambition:randomItem(stage8AmbitionPool),ambitionProgress:rand(0,25),memories:[],grudges:[],loyalties:[],scheme:null,factionId:null,legendScore:0,title:null};ls.profiles.push(p)}p.refId=ref?.id||p.refId;p.location=location||p.location;p.icon=icon||p.icon;p.type=type||p.type;p.legendScore=Math.max(p.legendScore||0,(ref?.reputation||0)+(ref?.fame||0)+(ref?.level||0)*5)};(state.divineRelationships?.roster||[]).forEach(r=>add(`immortal-${r.id}`,r.name,r.icon,r.type==="Titan"?"titan":"god",r,r.location));(state.heroes||[]).filter(h=>h.recruited).forEach(h=>add(`hero-${h.id}`,h.name,h.portrait||h.icon,"hero",h,h.location));(state.demigods||[]).forEach(d=>add(`demigod-${d.id}`,d.name,d.icon||"👤","demigod",d,d.city||"Greece"));(state.divineRelationships?.npcDemigods||[]).forEach(d=>add(`npcdemigod-${d.id}`,d.name,d.icon||"👤","demigod",d,d.city||"Greece"));(state.kingdoms||[]).forEach(k=>add(`ruler-${k.id||k.name}`,k.ruler,"👑","ruler",k,k.name));state.divineFamily?.immortalChildren?.forEach(c=>add(`child-${c.id}`,c.name,c.icon||"✨","god",c,"Mount Olympus"))}
function getStage8Profile(name){return state.livingSouls?.profiles?.find(p=>p.name===name)}
function rememberStage8(name,text,weight=1){const p=getStage8Profile(name);if(!p)return;p.memories.unshift({year:state.year,text,weight});p.memories=p.memories.slice(0,18);if(weight>=3)p.legendScore+=2}
function createStage8Rivalry(a,b,cause){if(!a||!b||a===b)return;let r=state.livingSouls.rivalries.find(x=>(x.a===a&&x.b===b)||(x.a===b&&x.b===a));if(!r){r={id:uid("rivalry"),a,b,intensity:25,causes:[],year:state.year};state.livingSouls.rivalries.push(r)}r.intensity=clamp(r.intensity+rand(5,15));r.causes.unshift({year:state.year,text:cause});rememberStage8(a,`Rivalry with ${b}: ${cause}`,3);rememberStage8(b,`Rivalry with ${a}: ${cause}`,3)}
function syncStage8Factions(){const ls=state.livingSouls;ls.profiles.forEach(p=>{if(p.factionId)return;let f=p.type==="titan"?ls.factions.find(x=>x.id==="titan-restoration"):p.type==="hero"||p.type==="demigod"?ls.factions.find(x=>x.id==="hero-advocates"):randomItem(ls.factions.filter(x=>x.id!=="titan-restoration"));if(f&&Math.random()<.75){p.factionId=f.id;if(!f.members.includes(p.name))f.members.push(p.name)}});ls.factions.forEach(f=>{f.members=f.members.filter(n=>ls.profiles.some(p=>p.name===n));f.leader=f.members.slice().sort((a,b)=>(getStage8Profile(b)?.legendScore||0)-(getStage8Profile(a)?.legendScore||0))[0]||null})}
function stage8AmbitionTurn(){state.livingSouls.profiles.forEach(p=>{if(Math.random()<.35)p.ambitionProgress=clamp(p.ambitionProgress+rand(1,6));if(p.ambitionProgress>=100){const old=p.ambition;p.ambitionProgress=0;p.legendScore+=10;recordChronicleEvent({icon:p.icon,title:`Ambition Achieved: ${p.name}`,text:`${p.name} achieved the ambition to ${old.toLowerCase()}.`,category:p.type==="hero"?"heroes":"gods-titans",severity:"mythic",major:true,target:p.name,tags:["legends"]});rememberStage8(p.name,`Achieved a defining ambition: ${old}.`,4);p.ambition=randomItem(stage8AmbitionPool)}})}
function stage8SchemeTurn(){const ps=state.livingSouls.profiles;ps.forEach(p=>{if(!p.scheme&&/Ambitious|Cunning|Jealous|Proud/.test(p.personality)&&Math.random()<.08){const t=randomItem(ps.filter(x=>x.name!==p.name));if(t)p.scheme={id:uid("scheme"),type:randomItem(stage8SchemePool),target:t.name,progress:0,secrecy:rand(45,85),startedYear:state.year,status:"Active"}}if(p.scheme?.status==="Active"){p.scheme.progress=clamp(p.scheme.progress+rand(4,14));if(Math.random()*100>p.scheme.secrecy&&Math.random()<.18)createStage8Message(p.name,`${p.name} may be involved in a scheme: ${p.scheme.type}.`,"scheme-warning",true);if(p.scheme.progress>=100){const success=Math.random()<.65;if(success){createStage8Rivalry(p.name,p.scheme.target,`A scheme succeeded: ${p.scheme.type}.`);p.legendScore+=4}else p.legendScore=Math.max(0,p.legendScore-2);recordChronicleEvent({icon:"🕷️",title:success?`Scheme Succeeds: ${p.name}`:`Scheme Exposed: ${p.name}`,text:success?`${p.name} acted against ${p.scheme.target}: ${p.scheme.type}.`:`A plot against ${p.scheme.target} failed and became public.`,category:"gods-titans",severity:success?"danger":"mythic",major:true,actor:p.name,target:p.scheme.target,tags:["legends"]});p.scheme=null}}})}
function createStage8Message(sender,text,type="personal",urgent=false){if(state.livingSouls.messages.filter(m=>m.status==="Waiting").length>=8)return;const p=getStage8Profile(sender),m={id:uid("message"),sender,icon:p?.icon||"📨",text,type,urgent,status:"Waiting",year:state.year};state.livingSouls.messages.unshift(m);createAlert({priority:urgent?"urgent":"important",category:"gods",icon:m.icon,title:`Message from ${sender}`,description:text,location:p?.location||"Mount Olympus",targetId:m.id,years:3})}
function stage8MessageTurn(){if(Math.random()>.30)return;const p=randomItem(state.livingSouls.profiles);if(!p)return;const others=state.livingSouls.profiles.filter(x=>x.name!==p.name),rival=others.length?randomItem(others).name:"a rival";createStage8Message(p.name,randomItem([`I need your support with my ambition: ${p.ambition}.`,`I want your advice about my faction and its rivals.`,`A rumor about my family is spreading.`,`I am considering a major decision and want your blessing.`,`I believe ${rival} is plotting against me.`]),"personal",Math.random()<.2)}
function resolveStage8Message(id,tone){const m=state.livingSouls.messages.find(x=>x.id===id);if(!m)return;const p=getStage8Profile(m.sender);if(tone==="supportive"&&p){p.ambitionProgress=clamp(p.ambitionProgress+8);p.loyalties.push(getGod().name);rememberStage8(p.name,"Received a supportive response from the player god.",2)}if(tone==="commanding"&&p)rememberStage8(p.name,"Was commanded directly by the player god.",2);if(tone==="threatening"&&p){p.grudges.push(getGod().name);createStage8Rivalry(p.name,getGod().name,"A threatening private message.")}if(tone==="deceptive")state.divineReputation.deceitful=(state.divineReputation.deceitful||0)+2;m.status="Resolved";m.outcome=tone;recordAfterAction({icon:m.icon,title:`Message Answered: ${m.sender}`,summary:`You answered ${m.sender} in a ${tone} tone.`,category:"gods-titans",target:m.sender,changes:[{label:"Tone",value:tone},{label:"Ambition",value:p?.ambition||"Unknown"},{label:"Progress",value:p?`${p.ambitionProgress}%`:"Unknown"}],severity:tone==="threatening"?"danger":"success"});renderAll();saveGame(false)}
function stage8FactionTurn(){const ls=state.livingSouls;ls.factions.forEach(f=>f.influence=clamp(f.influence+rand(-3,5)));if(Math.random()<.12&&ls.factions.length>=2){const [a,b]=ls.factions.slice().sort(()=>Math.random()-.5).slice(0,2);if(a&&b)recordChronicleEvent({icon:"🕸️",title:"Faction Conflict",text:`${a.name} and ${b.name} are clashing over ${randomItem(["Council authority","Titan policy","hero rights","mortal intervention","prophecy"])}.`,category:"council",severity:"mythic",major:true,tags:["gods-titans"]})}}
function stage8OfficeTurn(){const ls=state.livingSouls;ls.offices.forEach(o=>{if(!o.holder){const cands=ls.profiles.filter(p=>p.type==="god"||p.type==="titan");if(cands.length&&Math.random()<.22){const c=randomItem(cands);o.holder=c.name;o.support=rand(45,70);o.history.unshift(`Year ${state.year}: ${c.name} assumed the office.`);recordChronicleEvent({icon:o.icon,title:`Divine Office Filled: ${o.name}`,text:`${c.name} became ${o.name}.`,category:"council",severity:"mythic",major:true,target:c.name})}}else if(Math.random()<.08){o.support=clamp(o.support+rand(-12,8));if(o.support<20){const old=o.holder;o.holder=null;recordChronicleEvent({icon:o.icon,title:`Office Lost: ${o.name}`,text:`${old} lost political control of the office.`,category:"council",severity:"danger",major:true,target:old})}}})}
function supportStage8Office(id,name){const o=state.livingSouls.offices.find(x=>x.id===id),p=getStage8Profile(name);if(!o||!p||!spend("prestige",10))return;o.holder=p.name;o.support=clamp(o.support+20);p.ambitionProgress=clamp(p.ambitionProgress+12);recordAfterAction({icon:o.icon,title:`Office Appointment: ${p.name}`,summary:`You backed ${p.name} for ${o.name}.`,category:"council",target:p.name,changes:[{label:"Office",value:o.name},{label:"Support",value:`${o.support}`},{label:"Prestige",value:`${state.prestige}`}],severity:"mythic"});renderAll();saveGame(false)}
function stage8ConflictTurn(){const rs=state.livingSouls.rivalries.filter(r=>r.intensity>=65);if(!rs.length||Math.random()>.14)return;const r=randomItem(rs),a=getStage8Profile(r.a),b=getStage8Profile(r.b);if(!a||!b)return;const title=`The Conflict of ${r.a} and ${r.b}`,text=`Years of rivalry escalated after: ${r.causes[0]?.text||"a long series of grievances"}.`,saga=createStage6Saga({kind:"feud",icon:"⚔️",title,participants:[r.a,r.b],summary:text});addSagaChapter(saga,"Open Conflict",text,"⚔️","gods-titans");createAlert({priority:"urgent",category:"gods",icon:"⚔️",title,description:text,location:"Mount Olympus",targetId:saga.id,years:2});r.intensity=clamp(r.intensity+5)}
function stage8ChildDevelopmentTurn(){[...(state.demigods||[]),...(state.divineFamily?.immortalChildren||[])].forEach(c=>{const s=c.age<3?"Infant":c.age<8?"Child":c.age<14?"Adolescent":c.age<20?"Young Adult":"Adult";if(c.stage8LifeStage!==s){c.stage8LifeStage=s;recordChronicleEvent({icon:c.icon||"👶",title:`${c.name} Becomes a ${s}`,text:`A new stage of life begins for ${c.name}.`,category:"divine-family",target:c.name,tags:["births"]})}})}
function stage8MortalPoliticsTurn(){const ds=state.divineRule?.mortalDynasties||[];if(ds.length>=2&&Math.random()<.10){const [a,b]=ds.slice().sort(()=>Math.random()-.5).slice(0,2);recordChronicleEvent({icon:"👑",title:"Dynastic Conflict",text:`${a.name} and ${b.name} clash over ${randomItem(["competing divine ancestry claims","a disputed royal marriage","a broken alliance","claims to sacred land"])}.`,category:"wars",severity:"danger",major:true,tags:["divine-family","wars"]})}}
function stage8LegendTurn(){const ls=state.livingSouls;ls.profiles.forEach(p=>{if(!p.title&&p.legendScore>=90){p.title=randomItem(["the Unbroken","the Golden","the Far-Seer","the Storm-Born","the Oathkeeper","the Lionhearted","the Twice-Blessed"]);ls.legends.push({id:uid("legend"),name:p.name,title:p.title,year:state.year,deeds:p.memories.slice(0,5),type:p.type});recordChronicleEvent({icon:p.icon,title:`Legend Born: ${p.name} ${p.title}`,text:`${p.name} has become a living legend.`,category:"legends",severity:"mythic",major:true,target:p.name,tags:["legends"]})}})}
function stage8DeathLegacyTurn(){(state.souls||[]).forEach(s=>{const p=getStage8Profile(s.name);if(!p||p.funeralRecorded)return;if((p.legendScore||0)>=70||(s.glory||0)>=70){p.funeralRecorded=true;recordChronicleEvent({icon:"🕯️",title:`Great Funeral of ${s.name}`,text:`Heroes, descendants, and worshippers honor ${s.name}'s deeds.`,category:"deaths",severity:"mythic",major:true,target:s.name,location:s.city,tags:["legends","underworld"]});if(Math.random()<.55){state.divineRule.ancestorSpirits=state.divineRule.ancestorSpirits||[];if(!state.divineRule.ancestorSpirits.some(a=>a.name===s.name))state.divineRule.ancestorSpirits.push({id:uid("ancestor"),name:s.name,icon:s.icon||"👻",city:s.city,glory:s.glory,advice:"legacy",year:state.year,heroCult:true});addLibraryEntry("people",`Cult of ${s.name}`,`Mortals established a hero cult after the great funeral of ${s.name}.`)}}})}
function stage8YearTurn(){syncStage8Profiles();syncStage8Factions();stage8AmbitionTurn();stage8SchemeTurn();stage8MessageTurn();stage8FactionTurn();stage8OfficeTurn();stage8ConflictTurn();stage8ChildDevelopmentTurn();stage8MortalPoliticsTurn();stage8LegendTurn();stage8DeathLegacyTurn()}
function renderLivingSouls(){if(!document.getElementById("living-souls-view")||!state.livingSouls)return;syncStage8Profiles();const ls=state.livingSouls,f=ls.filter||"all",shown=f==="all"?ls.profiles:ls.profiles.filter(p=>p.type===f);$("#living-soul-count").textContent=`${ls.profiles.length} Characters`;$("#living-soul-summary").innerHTML=`<p class="eyebrow">CHARACTER AI</p><h3>${ls.profiles.length} important characters now have persistent memories, ambitions, loyalties, grudges, and independent schemes</h3><div class="stats"><div class="stat"><strong>${ls.profiles.filter(p=>p.scheme).length}</strong><small>Active Schemes</small></div><div class="stat"><strong>${ls.rivalries.length}</strong><small>Rivalries</small></div><div class="stat"><strong>${ls.legends.length}</strong><small>Living Legends</small></div></div>`;$$('.soul-filter').forEach(b=>b.classList.toggle('active',b.dataset.soulFilter===f));$$('.soul-filter').forEach(b=>b.onclick=()=>{ls.filter=b.dataset.soulFilter;renderLivingSouls()});$("#living-soul-list").innerHTML=shown.map(p=>`<article class="card soul-card ${p.scheme?"scheme-active":""} ${p.title?"legendary":""}"><div class="card-row"><div class="council-head"><span class="relationship-avatar">${p.icon}</span><div><h3>${p.name}</h3><p>${p.type} • ${p.personality} • ${p.location}</p></div></div>${p.title?`<span class="tag gold">${p.title}</span>`:""}</div><div class="soul-grid"><div class="soul-stat"><strong>${p.ambitionProgress}%</strong><small>Ambition</small></div><div class="soul-stat"><strong>${p.legendScore}</strong><small>Legend</small></div><div class="soul-stat"><strong>${p.memories.length}</strong><small>Memories</small></div><div class="soul-stat"><strong>${p.grudges.length}</strong><small>Grudges</small></div></div><div class="ambition-banner"><strong>🎯 Ambition:</strong> ${p.ambition}</div>${p.scheme?`<div class="scheme-banner"><strong>🕷️ Scheme:</strong> ${p.scheme.type} • Target ${p.scheme.target} • ${p.scheme.progress}%</div>`:""}<div class="memory-stack">${p.memories.slice(0,3).map(m=>`<div class="memory-line"><strong>Year ${m.year}</strong> ${m.text}</div>`).join("")||'<div class="memory-line">No defining memories yet.</div>'}</div><button class="card-button" data-soul-talk="${p.name}">Speak With ${p.name}</button></article>`).join("");$$('[data-soul-talk]').forEach(b=>b.onclick=()=>openStage8Conversation(b.dataset.soulTalk))}
function openStage8Conversation(name){const p=getStage8Profile(name);if(!p)return;showModal(`<p class="eyebrow">PERSONAL CONVERSATION</p><h2>${p.icon} ${p.name}</h2><p class="muted">${p.personality}. Ambition: ${p.ambition}.</p><div class="message-choice-grid"><button class="message-choice" data-stage8-tone="supportive">❤️ Supportive</button><button class="message-choice" data-stage8-tone="diplomatic">🤝 Diplomatic</button><button class="message-choice" data-stage8-tone="commanding">👑 Commanding</button><button class="message-choice" data-stage8-tone="threatening">⚡ Threatening</button><button class="message-choice" data-stage8-tone="deceptive">🐍 Deceptive</button><button class="message-choice" data-stage8-tone="personal">🌳 Ask About Goals</button></div>`);$$('[data-stage8-tone]').forEach(b=>b.onclick=()=>resolveStage8Conversation(p,b.dataset.stage8Tone))}
function resolveStage8Conversation(p,tone){let summary="";if(tone==="supportive"){p.ambitionProgress=clamp(p.ambitionProgress+7);summary=`You encouraged ${p.name}'s ambitions.`}if(tone==="diplomatic")summary=`You carefully discussed politics with ${p.name}.`;if(tone==="commanding"){p.ambitionProgress=clamp(p.ambitionProgress+3);summary=`You reminded ${p.name} of your divine authority.`}if(tone==="threatening"){p.grudges.push(getGod().name);summary=`You threatened ${p.name}, creating lasting resentment.`;createStage8Rivalry(p.name,getGod().name,"Threatened during a private conversation.")}if(tone==="deceptive"){state.divineReputation.deceitful=(state.divineReputation.deceitful||0)+2;summary=`You misled ${p.name}.`}if(tone==="personal")summary=`${p.name} revealed that they want to ${p.ambition.toLowerCase()}.`;rememberStage8(p.name,`Conversation with ${getGod().name}: ${tone} tone.`,2);closeModal();recordAfterAction({icon:p.icon,title:`Conversation: ${p.name}`,summary,category:p.type==="hero"?"heroes":"gods-titans",target:p.name,changes:[{label:"Tone",value:tone},{label:"Ambition Progress",value:`${p.ambitionProgress}%`},{label:"Grudges",value:`${p.grudges.length}`}],severity:tone==="threatening"?"danger":"success"});renderAll();saveGame(false)}
function renderStage8Factions(){if(!document.getElementById("factions-view")||!state.livingSouls)return;syncStage8Factions();const ls=state.livingSouls;$("#faction-count").textContent=`${ls.factions.length} Factions`;$("#faction-summary").innerHTML=`<p class="eyebrow">POLITICAL BLOCS</p><h3>Gods, Titans, heroes, and demigods organize into factions with shared goals</h3>`;$("#faction-list").innerHTML=ls.factions.map(f=>`<article class="card faction-card"><div class="card-row"><div><h3>${f.icon} ${f.name}</h3><p>${f.goal}</p></div><span class="tag gold">Influence ${f.influence}</span></div><div class="faction-grid"><div class="faction-stat"><strong>${f.members.length}</strong><small>Members</small></div><div class="faction-stat"><strong>${f.leader||"None"}</strong><small>Leader</small></div><div class="faction-stat"><strong>${f.alignment}</strong><small>Alignment</small></div></div><p class="muted">${f.members.slice(0,8).join(", ")||"No members yet"}</p></article>`).join("")}
function renderStage8Offices(){if(!document.getElementById("divine-offices-view")||!state.livingSouls)return;const ls=state.livingSouls;$("#office-count").textContent=`${ls.offices.length} Offices`;$("#office-summary").innerHTML=`<p class="eyebrow">OLYMPIAN POWER</p><h3>Divine offices give characters lasting political influence</h3>`;$("#office-list").innerHTML=ls.offices.map(o=>`<article class="card office-card"><div class="card-row"><div><h3>${o.icon} ${o.name}</h3><p>${o.description}</p></div>${o.holder?`<span class="office-holder">${o.holder}</span>`:"<span class='tag'>Vacant</span>"}</div><div class="office-grid"><div class="office-stat"><strong>${o.support}</strong><small>Support</small></div><div class="office-stat"><strong>${o.power}</strong><small>Political Power</small></div></div>${!o.holder?`<button class="card-button" data-office-fill="${o.id}">Back a Candidate • 10 Prestige</button>`:""}</article>`).join("");$$('[data-office-fill]').forEach(b=>b.onclick=()=>openStage8OfficeChoice(b.dataset.officeFill))}
function openStage8OfficeChoice(id){const o=state.livingSouls.offices.find(x=>x.id===id),cs=state.livingSouls.profiles.filter(p=>p.type==="god"||p.type==="titan").slice(0,12);showModal(`<p class="eyebrow">DIVINE APPOINTMENT</p><h2>${o.icon} ${o.name}</h2><div class="action-grid">${cs.map(p=>`<button class="action-button" data-office-candidate="${p.name}">${p.icon} ${p.name}<br><small>${p.personality} • Legend ${p.legendScore}</small></button>`).join("")}</div>`);$$('[data-office-candidate]').forEach(b=>b.onclick=()=>{closeModal();supportStage8Office(id,b.dataset.officeCandidate)})}
function renderStage8Messages(){if(!document.getElementById("messages-view")||!state.livingSouls)return;const waiting=state.livingSouls.messages.filter(m=>m.status==="Waiting");$("#message-count").textContent=`${waiting.length} Waiting`;$("#message-summary").innerHTML=`<p class="eyebrow">PRIVATE CONTACT</p><h3>Important characters contact you about ambitions, rivalries, family, rumors, and schemes</h3>`;$("#message-list").innerHTML=waiting.length?waiting.map(m=>`<article class="card message-card ${m.urgent?"urgent":""}"><div class="card-row"><div class="council-head"><span class="portrait">${m.icon}</span><div><h3>${m.sender}</h3><p>Year ${m.year}</p></div></div>${m.urgent?'<span class="tag gold">Urgent</span>':""}</div><p>${m.text}</p><div class="message-choice-grid">${["supportive","diplomatic","commanding","threatening","deceptive"].map(t=>`<button class="message-choice" data-message="${m.id}" data-message-tone="${t}">${t}</button>`).join("")}</div></article>`).join(""):'<article class="card"><h3>No messages are waiting</h3><p>Characters will contact you as their ambitions and conflicts develop.</p></article>';$$('[data-message]').forEach(b=>b.onclick=()=>resolveStage8Message(b.dataset.message,b.dataset.messageTone))}
function renderStage8Extras(){renderLivingSouls();renderStage8Factions();renderStage8Offices();renderStage8Messages()}
...
function renderDivineRelationships(){runOlympusIntegrityRepair(true);
...
function openImmortalMeeting(id){
 ...
 $$("[data-divine-talk]").forEach(b=>b.onclick=()=>resolveRelationshipConversation(r,b.dataset.divineTalk));
...
function resolveRelationshipConversation(r,choiceId){
...
function completeLivingMarriage(r){
 ...
 state.divineFamily.familyHistory.unshift(`Year ${state.year}: ${getGod().name} married ${r.name}.`);rememberStage8(r.name,`Married ${getGod().name}.`,4);
 let node=state.divineFamily.familyTree.find(x=>x.name===r.name);
...
function renderDivineFamily(){runOlympusIntegrityRepair(true);synchronizeUnifiedOlympus(true);const df=state.divineFamily,s=df.spouse;$("#divine-family-status").textContent=s?`Married to ${s.name}`:"Unwed";$("#divine-family-summary").innerHTML=`<p class="eyebrow">LIVING PANTHEON</p><h3>Marriage creates immortal children, jealousy, sibling bonds, inheritance and political consequences</h3><div class="stats"><div class="stat"><strong>${s?1:0}</strong><small>Spouse</small></div><div class="stat"><strong>${df.immortalChildren.length}</strong><small>Immortal Children</small></div><div class="stat"><strong>${state.demigods.length}</strong><small>Demigods</small></div></div>`;
...
function resolveAlertChoice(a,choiceId,asInterrupt){
 ...
 if(choice.effect==="holdWedding"){
  const match=state.divineMatchmaking?.arrangedMatches?.find(m=>m.id===a.targetId);
  if(asInterrupt)document.querySelector(".interrupt-overlay")?.remove();else closeModal();
  if(!match){showToast("The betrothal record could not be found.");return}
  holdStage10Wedding(match.id);
  return;
 }
 if(choice.effect==="openMatchmaking"){
  if(asInterrupt)document.querySelector(".interrupt-overlay")?.remove();else closeModal();
  activateView("matchmaking-view");renderStage10Matchmaking();return;
 }
 if(choice.effect==="cancelBetrothal"){
  const match=state.divineMatchmaking?.arrangedMatches?.find(m=>m.id===a.targetId);
  if(match)stage11CancelBetrothal(match.a,match.b);
  if(asInterrupt)document.querySelector(".interrupt-overlay")?.remove();else closeModal();
  renderAll();saveGame(false);showNextInterrupt();return;
 }
 if(choice.cost){
...
function resolveDirectIntervention(a){
 if(a.targetType==="matchmaking")return"Complete this event through the Divine Wedding system.";
...
function openHeraPalace(){
 showModal(`<p class="eyebrow">HERA'S PALACE</p><h2>🦚 Divine Family Court</h2><div class="olympus-action-grid"><button class="action-button" data-hera-action="marriage">💞 Bless Divine Families</button><button class="action-button" data-hera-action="heir">👶 Recognize an Heir</button><button class="action-button" data-hera-action="reconcile">🤝 Reconcile Family</button><button class="action-button" data-hera-action="legitimacy">👑 Grant Legitimacy</button></div>`);
 $$("[data-hera-action]").forEach(b=>b.addEventListener("click",()=>{const a=b.dataset.heraAction;if(a==="marriage"){state.divineRelationships.roster.forEach(r=>{r.affection=clamp((r.affection||0)+3);r.trust=clamp((r.trust||45)+2)});state.prestige+=3}if(a==="heir"){const d=state.demigods[0];if(d)d.councilFavor+=10}if(a==="reconcile"){state.familyDramaEvents=[];state.divineReputation.merciful+=3}if(a==="legitimacy"){const d=state.demigods.find(d=>d.status!=="Immortal");if(d){d.reputation+=10;d.fame+=8}}recordOlympus("Hera's Palace",a);closeModal();renderAll();saveGame(false)}));
}
...
function avatarSpeakCitizen(c,id){
 ...
 $("#citizen-bless").addEventListener("click",()=>{p.happiness=clamp(p.happiness+18);p.loyalty=clamp(p.loyalty+12);state.faith+=5;state.favor+=2;state.divineReputation.merciful+=2;recordAvatar(`Blessed ${p.name}`,`${p.name}'s life was changed by direct divine favor.`,"✨");closeModal();renderAll()});
...
function openDivineConversation(name){
 ...
 $$("[data-conversation-topic]").forEach(b=>b.addEventListener("click",()=>resolvePoliticsConversation(g,b.dataset.conversationTopic)));
}
function resolvePoliticsConversation(g,topic){
...
// Version 2.0.0 — Olympus Reforged
function ensureV200Data(){
 if(!state.v200)state.v200={version:"2.0.0",migrationYear:state.year,renderPasses:0,coreAuthority:"olympusCore",legacyMirrors:"compatibility-only"};
 state.v200.version="2.0.0";
 // Olympus Core is authoritative. Old arrays remain compatibility mirrors only so existing saves stay safe.
 if(state.olympusCore){
  state.v200.coreAuthority="olympusCore";
  state.v200.corePeople=Object.keys(state.olympusCore.people||{}).length;
  state.v200.coreUnions=(state.olympusCore.unions||[]).length;
 }
}
function v200SystemTurn(){
 ensureV200Data();
 // One integrity pass per year is enough; older systems no longer need to compete with each other.
 if(state.olympusCore)runOlympusIntegrityRepair(true);
 state.v200.lastSystemTurn=state.year;
}
function pulseResourceStrip(){
 const el=document.querySelector('.resource-strip');if(!el)return;
 el.classList.remove('resource-update');void el.offsetWidth;el.classList.add('resource-update');
}
function confirmAdvanceYear(){
 if(window.__yearAdvancing)return;
 const critical=(state.alerts||[]).filter(a=>!a.resolved&&a.status!=="resolved"&&a.priority==="critical");
 if(critical.length){showToast(`Resolve ${critical.length} critical crisis${critical.length===1?'':'es'} before time can advance.`);activateView("alerts-view");return}
 window.__yearAdvancing=true;const btn=$("#advance-year-button");if(btn){btn.classList.add("year-running");btn.innerHTML="⏳ Advancing…"}
 const veil=document.createElement("div");veil.className="year-transition";veil.innerHTML=`<div><span>${getGod().symbol}</span><small>THE WORLD MOVES FORWARD</small><strong>Year ${state.year+1}</strong></div>`;document.body.appendChild(veil);
 setTimeout(()=>{try{advanceYear()}finally{setTimeout(()=>{veil.remove();window.__yearAdvancing=false;if(btn){btn.classList.remove("year-running");btn.innerHTML="📅 Advance Year"}},700)}},180);
}
function advanceYear(){
 state.year++;
 let monsterReports=0;
 const income=calculateYearlyIncome();state.gold+=income.total;recordGold(income.total,"Yearly income");expireAlerts();resolveEnterprises();resolveFleetYear();wonderYearTurn();resolveTrainingYear();cityYearTurn();kingdomYearTurn();civilizationYearTurn();autonomousHeroYearTurn();bloodlineYearTurn();divineFamilyYearTurn();divinePoliticsYearTurn();titanCouncilYearTurn();avatarYearTurn();livingOlympusYearTurn();underworldYearTurn();livingAIYearTurn();adventureYearTurn();dynastyYearTurn();livingCitizenYearTurn();templeYearTurn();livingWarYearTurn();creatureEcosystemYearTurn();livingConversationYearTurn();livingWorldStoryYearTurn();autonomousImmortalRelationshipYearTurn();pantheonGenerationsYearTurn();stage3JealousyYearTurn();livingChronicleYearTurn();stage6YearTurn();stage7YearTurn();stage8YearTurn();stage9YearTurn();stage10YearTurn();stage11YearTurn();v150YearTurn();v160YearTurn();v170YearTurn();v180YearTurn();v190YearTurn();v200SystemTurn();divineFamilyStage2YearTurn();fateYearTurn();miracleYearTurn();worldSimulationTurn();
 state.cities.forEach(c=>{
 ...
 closeModal();runOlympusIntegrityRepair(true);syncAutonomousDivineFamilies();renderAll();pulseResourceStrip();saveGame(false);
 const newMarriages=(state.olympusCore?.unions||[]).filter(u=>u.status==="Married"&&u.year===state.year).length;const newBirths=(state.v170?.birthHistory||[]).filter(b=>b.year===state.year).length;
 showToast(`Year ${state.year}: ${newMarriages} marriage${newMarriages===1?'':'s'}, ${newBirths} birth${newBirths===1?'':'s'}, ${monsterReports} monster action${monsterReports===1?'':'s'}.`);
 setTimeout(showNextInterrupt,450);
}
...
// v2.0.0 boot polish
document.documentElement.dataset.olympusVersion="2.0.0";
