const SAVE_KEY="ageOfOlympusSaveV160";
...
function v150AutomaticImmortalBirthTurn(){
 ...
 unions.forEach(u=>{
  ...
  if(existing.length>=5||Math.random()<existing.length*.12)return;
  const a=(state.divineRelationships.roster||[]).find(r=>r.name===u.a),b=(state.divineRelationships.roster||[]).find(r=>r.name===u.b);
  if(!a||!b)return;
  const name=randomUniqueAutonomousChildName(),domain=deriveAutonomousChildDomain(a,b);
  const child={id:uid("npcImmortalChild"),name,icon:randomItem(["✨","🌟","⚡","🌙","🔥","🌊","🦉","🌸"]),parentA:u.a,parentB:u.b,domain,age:0,stage:"Divine Infant",power:rand(20,42),wisdom:rand(20,42),loyalty:65,house:`House of ${u.a} & ${u.b}`,memories:[`Born in Year ${state.year}.`]};
  state.divineRelationships.npcImmortalChildren.push(child);
  stage11Person(child.name,{icon:child.icon,type:"Immortal Child",status:"Divine Infant"});stage11LinkParentChild(u.a,child.name);stage11LinkParentChild(u.b,child.name);
  runV150FamilySync();
  addActivity(child.icon,`Immortal Child Born: ${child.name}`,`${u.a} and ${u.b} welcomed a child of ${domain}.`,"success");
  recordChronicleEvent({icon:child.icon,title:`Immortal Birth: ${child.name}`,text:`Child of ${u.a} and ${u.b}, born into ${child.house} with affinity for ${domain}.`,category:"births",severity:"mythic",major:true,actor:u.a,target:child.name,location:"Mount Olympus",tags:["divine-family","living-olympus"]});
 });
}
...
function v150AutomaticDemigodTurn(){
 ...
 roster.forEach(g=>{
  ...
  if(existing>=5||Math.random()<existing*.1)return;
  const mortal=randomItem(autonomousMortalRomanceNames),name=randomUniqueAutonomousChildName(),city=randomItem(state.cities).name;
  const child={id:uid("npcDemigod"),name,icon:"👤",divineParent:g.name,mortalParent:mortal,city,age:0,status:"Demigod Child",fame:0,reputation:10,corruption:0,domainPotential:randomItem(divineDomainPool),memories:[`Born in ${city} in Year ${state.year}.`]};
  state.divineRelationships.npcDemigods.push(child);
  if(state.v160)syncV160Campers();
  stage11Person(child.name,{icon:"👤",type:"Demigod",status:"Demigod Child"});stage11LinkParentChild(g.name,child.name);
  runV150FamilySync();
  addActivity("👶",`Demigod Born: ${child.name}`,`${g.name} had a demigod child with ${mortal} of ${city}.`,"success");
  recordChronicleEvent({icon:"👶",title:`Demigod Born: ${child.name}`,text:`Child of ${g.name} and mortal ${mortal}.`,category:"births",severity:"normal",actor:g.name,target:child.name,location:city,tags:["divine-family","demigod"]});
 });
}
