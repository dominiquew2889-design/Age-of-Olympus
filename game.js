const SAVE_KEY="ageOfOlympusSaveV130S3";
...
function renderAll(){
 ...
 ensureLivingCityData();ensureV100Data();ensureV110Data();ensureV120Data();ensureV130Data();ensureV130Stage2Data();ensureV130Stage3Data();renderWorld();renderUnderworld();renderLivingOlympus();renderAvatar();renderDivinePolitics();renderTitanCouncil();renderDivineReputation();renderWorldExpansion();renderNaval();renderWonders();renderOlympicGamesV110();renderLibraryOfFate();renderCityDevelopment();renderTraining();renderOlympus();renderFate();renderAges();renderPantheon();renderDivineFamily();renderKingdoms();renderEconomy();renderArtifacts();renderLegacy();renderTitans();renderCouncil();renderPowersPlaceholder();renderHeroes();renderMonsters();renderQuests();renderThrone();renderTreasury();renderLog();
}
...
function advanceYear(){
 ...
 const income=calculateYearlyIncome();state.gold+=income.total;recordGold(income.total,"Yearly income");resolveEnterprises();resolveFleetYear();wonderYearTurn();resolveTrainingYear();cityYearTurn();kingdomYearTurn();civilizationYearTurn();autonomousHeroYearTurn();bloodlineYearTurn();divineFamilyYearTurn();divinePoliticsYearTurn();titanCouncilYearTurn();avatarYearTurn();livingOlympusYearTurn();underworldYearTurn();fateYearTurn();miracleYearTurn();worldSimulationTurn();
 state.cities.forEach(c=>{
 ...
 addLog(`Year ${state.year} Begins`,`${monsterReports} monsters acted. ${getCurrentAge().name} continues. ${state.fleets.filter(f=>f.status==='At Sea').length} fleets sailed, ${state.civilizations.filter(c=>c.known).length} civilizations are known, and Fate balance is ${Math.round(state.fateBalance)}%.`);
