const SAVE_KEY="ageOfOlympusSaveV140S3";
...
function openImmortalMeeting(id){
 ...
 showModal(`<p class="eyebrow">${r.type==="Titan"?"TITAN ENCOUNTER":"DIVINE ENCOUNTER"}</p><h2>${r.icon} ${r.name}</h2><div class="relationship-scene"><strong>${r.location}</strong>${getRelationshipGreeting(r)}</div><div class="conversation-grid">${divineConversationChoices.map(c=>`<button class="action-button" data-divine-talk="${c.id}">${c.icon} ${c.label}${c.costGold?`<br><small>${c.costGold} Gold</small>`:""}</button>`).join("")}</div><div class="relationship-action-grid"><button class="action-button" data-divine-special="court">❤️ Begin Courtship</button><button class="action-button" data-divine-special="proposal" ${!canProposeToImmortal(r)?"disabled":""}>💍 Propose Marriage</button><button class="action-button" data-divine-special="family">🌳 Discuss Demigods & Family</button><button class="action-button" data-divine-special="politics">🏛️ Ask Their Political Goal</button></div>`);
...
function resolveDivineSpecial(r,action){
 if(action==="court"){r.status="Romantic";r.affection=clamp(r.affection+5);r.memories.unshift(`Year ${state.year}: Courtship officially began.`);showToast(`Courtship with ${r.name} has begun.`)}
 if(action==="proposal"){closeModal();proposeToLivingImmortal(r);return}
 if(action==="family"){r.trust=clamp(r.trust+7);r.respect=clamp(r.respect+4);if(state.demigods.length)r.jealousy=clamp(r.jealousy-2);r.memories.unshift(`Year ${state.year}: Discussed demigods and the blended divine family.`)}
 if(action==="politics"){const goal=r.type==="Titan"?randomItem(["Reform Tartarus","Gain Titan representation","Protect Titan descendants","Reduce Olympian control","Restore an ancient sanctuary"]):randomItem(["Gain Council influence","Protect a favored city","Mentor a young god","Expand temple worship","Prevent Titan rebellion"]);r.memories.unshift(`Year ${state.year}: Revealed political goal: ${goal}.`);showToast(`${r.name}: ${goal}`)}
 closeModal();renderAll();saveGame(false);
}
...
function buildDivineHouses(){
 ...
 const spouse=state.divineFamily.spouse;
 if(spouse&&!state.divineRelationships.houses.some(h=>h.founders.includes(getGod().name)&&h.founders.includes(spouse.name))){
  state.divineRelationships.houses.push({id:uid("divineHouse"),name:`House of ${getGod().name} & ${spouse.name}`,icon:"🏛️",founders:[getGod().name,spouse.name],type:spouse.type==="Titan"?"Olympian-Titan House":"Olympian House",children:state.divineFamily.immortalChildren.map(c=>c.name),demigods:state.demigods.map(d=>d.name),influence:55,unity:70,history:[`Founded by marriage in Year ${state.year}.`]});
 }
}
...
function resolveTempleAction(t,action){
 ...
 t.status=t.devotion>t.rivalInfluence?"Devoted":"Contested";t.history.unshift(`Year ${state.year}: ${a.name}.`);closeModal();renderAll();saveGame(false);
}
...
function livingCitizenYearTurn(){
 Object.entries(state.citizens).forEach(([city,people])=>people.forEach(p=>{
  ...
  if(Math.random()<.06&&p.age>65){p.alive=false;state.souls.unshift({id:uid("soul"),name:p.name,role:p.job,city,glory:p.reputation,virtue:p.loyalty,crimes:rand(0,35),icon:getCitizenAvatar(p),status:"Awaiting Judgment",birthYear:state.year-p.age,parents:"Mortal family",deeds:p.memories});state.underworld.soulsArrived++}
