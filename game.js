const SAVE_KEY="ageOfOlympusSaveV200";
...
function v180EarlyMarriageTurn(){
 if(state.year>5)return;
 const roster=state.divineRelationships?.roster||[];
 let free=roster.filter(r=>!state.olympusCore?.people?.[r.name]?.spouse);
 if(free.length<2)return;
 const attempts=state.year<=3?2:1;
 for(let i=0;i<attempts;i++){
  free=roster.filter(r=>!state.olympusCore?.people?.[r.name]?.spouse);if(free.length<2)break;
  const a=randomItem(free),choices=free.filter(x=>x.name!==a.name).sort((x,y)=>v150RelationshipScore(a,y)-v150RelationshipScore(a,x));
  const b=choices[0];if(!b)continue;
  a.affection=clamp((a.affection||0)+rand(8,16));b.affection=clamp((b.affection||0)+rand(8,16));a.trust=clamp((a.trust||45)+rand(5,10));b.trust=clamp((b.trust||45)+rand(5,10));
  const score=v150RelationshipScore(a,b);if(score<60||Math.random()>.78)continue;
  const type=(a.type==='Titan'||b.type==='Titan')?'Olympian-Titan Union':'Divine Union';
  stage11CompleteMarriage(a.name,b.name,{year:state.year,type,source:'v1.8 Founding Era Marriage'});
  state.divineRelationships.npcFamilyEvents.unshift({year:state.year,type:'Marriage',text:`${a.name} and ${b.name} married during the Founding Era.`});
  addActivity('💍',`Early Divine Marriage: ${a.name} & ${b.name}`,`Their union formed during Year ${state.year}, strengthening the first generation of Divine Houses.`,'success');
  recordChronicleEvent({icon:'💍',title:`Founding Marriage: ${a.name} & ${b.name}`,text:`The immortals married during the Founding Era and established House of ${a.name} & ${b.name}.`,category:'marriages',severity:'mythic',major:true,actor:a.name,target:b.name,location:'Mount Olympus',tags:['founding-era','divine-family']});
 }
}
...
function queueV170Birth(config){
 ...
 state.v170.pendingBirths.push(pending);saveGame(false);setTimeout(()=>showNextV170BirthNaming(),50);return pending;
}
...
function finalizeV170Birth(id,name){
 ...
 recordChronicleEvent({icon:identity.icon,title:`Birth of ${name}`,text:`${name}, child of ${b.parentA}${b.parentB?` and ${b.parentB}`:b.mortalParent?` and mortal ${b.mortalParent}`:""}, was named by the player. Inherited domains: ${identity.domains.join(", ")}.`,category:"births",severity:b.type==="Immortal Child"?"mythic":"success",major:b.type==="Immortal Child",actor:b.parentA,target:name,location:b.city,tags:["divine-family","bloodline","births"]});
...
function v150AutomaticMarriageTurn(){
 const roster=state.divineRelationships?.roster||[];
 const free=roster.filter(r=>!state.olympusCore?.people?.[r.name]?.spouse);
 if(free.length<2)return;
 // v1.8: Founding Era marriages are handled by v180EarlyMarriageTurn so Houses form early.
 if(state.year<=5)return;
 if(Math.random()>.34)return;
 const a=randomItem(free),choices=free.filter(x=>x.name!==a.name);if(!choices.length)return;
 const b=choices.sort((x,y)=>v150RelationshipScore(a,y)-v150RelationshipScore(a,x))[0];
 a.affection=clamp((a.affection||0)+rand(3,8));b.affection=clamp((b.affection||0)+rand(3,8));a.trust=clamp((a.trust||45)+rand(2,5));b.trust=clamp((b.trust||45)+rand(2,5));
 if(v150RelationshipScore(a,b)<78||Math.random()>.48)return;
 const type=(a.type==='Titan'||b.type==='Titan')?'Olympian-Titan Union':'Divine Union';
 stage11CompleteMarriage(a.name,b.name,{year:state.year,type,source:'Autonomous Divine Romance'});
 state.divineRelationships.npcFamilyEvents.unshift({year:state.year,type:'Marriage',text:`${a.name} and ${b.name} chose to marry.`});
 addActivity('💍',`Divine Marriage: ${a.name} & ${b.name}`,'The immortals chose one another and their Divine House was founded automatically.','success');
 recordChronicleEvent({icon:'💍',title:`Divine Marriage: ${a.name} & ${b.name}`,text:`Their autonomous union founded House of ${a.name} & ${b.name}.`,category:'marriages',severity:'mythic',major:true,actor:a.name,target:b.name,location:'Mount Olympus',tags:['living-olympus','divine-family']});
}
function v150AutomaticImmortalBirthTurn(){
 const unions=(state.olympusCore?.unions||[]).filter(u=>u.status==="Married");
 unions.forEach(u=>{
  if(Math.random()>.115)return;
  ...
  if(existing.length+queued.length>=5||Math.random()<existing.length*.12)return;
...
function advanceYear(){
 ...
 closeModal();renderAll();pulseResourceStrip();saveGame(false);showToast(`Year ${state.year} has begun.`);
