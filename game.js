const SAVE_KEY="ageOfOlympusSaveV140S11";
...
function renderStage11Core(){
 ...
 $("#core-summary").innerHTML=`<p class="eyebrow">UNIFIED WORLD CORE</p><h3>${Object.keys(core.people).length} characters • ${core.unions.filter(u=>u.status==="Married").length} marriages • ${Object.keys(core.houses).length} divine houses</h3><p>Marriage, genealogy, children, houses, matchmaking, relationships, and family-tree data now rebuild from one shared core.</p><button class="card-button" data-stage11-action="repair-core">🔧 Run Integrity Repair</button>`;
...
function patchStage10Navigation(){
 const map=window.__navCategoryMap;if(!map)return;
 if(map.divine&&!map.divine.items.some(x=>x[0]==="matchmaking-view"))map.divine.items.push(["matchmaking-view","💞","Matchmaking","Arrange immortal unions"]);
}
...
function stage10YearTurn(){
 stage10RepairAllMarriages();stage10ChildbirthTurn();stage10MarriagePoliticsTurn();stage10RepairAllMarriages();syncStage10Dynasties();synchronizeUnifiedOlympus(false);stage10RepairAllMarriages();
}
