const SAVE_KEY="ageOfOlympusSaveV090";
...
function renderAll(){
 ...
 ensureLivingCityData();renderWorld();renderCityDevelopment();renderTraining();renderDivineFamily();renderKingdoms();renderEconomy();renderArtifacts();renderLegacy();renderTitans();renderCouncil();renderPowersPlaceholder();renderHeroes();renderMonsters();renderQuests();renderThrone();renderTreasury();renderLog();
}
...
function ascendDemigod(d){
 ...
 addLog(`${d.name} Ascends to Godhood`,`${d.name} becomes the immortal deity of ${d.domain} and joins the Olympus Council.`);
}
...
function advanceYear(){
 ...
 const income=calculateYearlyIncome();state.gold+=income.total;recordGold(income.total,"Yearly income");resolveEnterprises();resolveTrainingYear();cityYearTurn();kingdomYearTurn();bloodlineYearTurn();divineFamilyYearTurn();
 state.cities.forEach(c=>{
 ...
 addLog(`Year ${state.year} Begins`,`${monsterReports} monsters acted. Tartarus stability is ${Math.round(state.tartarusStability)}%. Titan War level: ${state.titanWar}. Divine descendants: ${state.demigods.length}.`);
